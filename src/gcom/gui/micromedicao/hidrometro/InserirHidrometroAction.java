...
//		 Obtendo uma instancia da sessao
		...
		if ( hidrometroActionForm.getTombamento() != null
			...
			// [FS0010] - Verificar existência do número do tombamento
			Integer quantidadeOcorrencia = fachada
					.pesquisarNumeroHidrometroFaixaCount(hidrometroActionForm.getTombamento());
			
			if (quantidadeOcorrencia != null && quantidadeOcorrencia > 0) {
				throw new ActionServletException(
					"atencao.numero_tombamento.ja_cadastrado");
			}
		...
		Integer anoFabricacao = new Integer(hidrometroActionForm
				.getAnoFabricacao());
		// [FS0009] - Validar ano de fabricação
		...
		// Cria o objeto hidrômetro fator correção e seta o id
		...
		if (hidrometroActionForm.getIdHidrometroFatorCorrecao() != null
			...
			hidrometroFatorCorrecao.setId(new Integer(hidrometroActionForm
					.getIdHidrometroFatorCorrecao()));
		}

		// Cria o objeto hidrômetro classe pressão e seta o id
		...
		if (hidrometroActionForm.getIdHidrometroClassePressao() != null
			...
			hidrometroClassePressao.setId(new Integer(hidrometroActionForm
		...
		if (hidrometroActionForm.getFaixaFinal() != null
			...
			faixaInicial = new Integer(hidrometroActionForm
				.getFaixaInicial());
			
			faixaFinal = new Integer(hidrometroActionForm.getFaixaFinal());
			
			intervalo = new Integer((faixaFinal.intValue() - faixaInicial
					.intValue()) + 1);
		} else {
			intervalo = 1;
		}
