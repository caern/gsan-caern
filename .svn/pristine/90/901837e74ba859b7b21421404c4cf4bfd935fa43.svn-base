...
function validarForm(formulario){
	...
	var submeterFormulario = false;
	
	if(validateAtualizarEmpresaActionForm(formulario)){
		 if (comparaData(formulario.dataInicioContratoCobranca.value, ">", formulario.dataFimContratoCobranca.value )) {

			submeterFormulario = false;		
			alert('Data incial maior que Data final');
					
		} else if(formulario.indicadorEmpresaCobranca[0].checked){
		...
		<tr>
			...
			<table width="100%" border="0">
				...
				<tr>
							...
							<td colspan="2"><strong> <html:text
										property="dataEncerramentoContratoCobranca" size="11" maxlength="10"
										tabindex="10" onkeyup="mascaraData(this, event);"
										onkeypress="mascaraData(this, event);return isCampoNumerico(event);" />
									<a 
									href="javascript:abrirCalendario('AtualizarEmpresaActionForm', 'dataEncerramentoContratoCobranca');">
										<img border="0"
										src="<bean:message key='caminho.imagens'/>calendario.gif"
										width="16" height="15" border="0" alt="Exibir Calendário"
										tabindex="11" />
						...
						<tr>
							...
							<td colspan="2"><span class="style2"> <html:text
										...
										onkeypress="return isCampoNumerico(event);" tabindex="13" /> </span>
							</td>
							<td>&nbsp;</td>
						</tr>
