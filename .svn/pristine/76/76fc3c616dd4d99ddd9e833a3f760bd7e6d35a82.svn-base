...
public class ControladorGerencialCadastroSEJB implements SessionBean {
	...
	public void gerarResumoHistograma(int anoMesReferencia, int idSetor,
		...
		try {
			...
			//[SB1000] – Gerar Histograma para Imóveis Faturados
			//1.	O sistema seleciona as contas do mês de faturamento corrente e gera o histograma de água por ligação, o histograma de água por economia, o histograma de esgoto por ligação e o histograma de esgoto por economia acumulando os valores das ligações e das economias agrupados por:
			...
			for (int i = 0; i < histogramaAguaLigacao.size(); i++) {
				...
				if (obj instanceof Object[]) {
                    ...
                    //2.	O sistema deverá acumular os valores das ligações e das economias 
                    //do histograma de água por ligação e por economia e do histograma de esgoto 
                    //por ligação e por economia conforme a seguir:
					
                    // Criamos um laço com todas as categorias da conta
					...
					while (iteContaCategoria.hasNext()) {                        
						// Verificamos se quantidade de economias dessa conta é
...
//						}				
	 ...
	 *  [Quantidade de ligações cortadas mes  e	Quantidade de religações no mes].
	 */
			...
			for (int i = 0; i < imoveisResumoLigacaoEconomias.size(); i++) {
				...
				if (obj instanceof Object[]) {
					...
					if (listaSimplificada.contains(helper)) {
						...
						//retorno[27] = "data de Corte da ligação de água"
        ...
        try {
            ...
            for (int i = 0; i < imoveisResumoConsumoAgua.size(); i++) {
                ...
                if (obj instanceof Object[]) {
                    ...
                    imovelErro = idImovel;
                    Integer idConta = (Integer) retorno[20]; // Código da conta que esta sendo processada
                    ...
                    // Caso não obtenha as quantidades pela conta, pesquisamos as quantidades de economia do imovel corrente                    
...
//							.intValue())) {
//				throw new ControladorException(
//						"atencao.ano.mes.indicador.maior.igual.ano.mes.resumo", null, "da Comercialização");
//			}
		...
		try {
			...
			for (int i = 0; i < imoveisResumoColetaEsgoto.size(); i++) {
				...
				if (obj instanceof Object[]) {
					...
					// Caso não encontre a categoria pela conta, seleciona pelo imovel
					if ( categoria == null ){					
			...
			while(!flagTerminou){
				...
				for (int i = 0; i < imoveisResumoLigacaoEconomias.size(); i++) {
					...
					if (obj instanceof Object[]) {
						...
						//Se nenhuma linha foi atualizada é inserida uma nova linha na tabela.
...
//                            (Integer) retorno[9], // Numero da quadra
                    ...
                    Integer idImovel = (Integer) retorno[0]; // Codigo do imovel que esta sendo processado
                    imovelErro = idImovel;
                    Integer idConta = (Integer) retorno[17]; // Código da conta que esta sendo processada
                    Conta conta = new Conta();
                    conta.setId( idConta );                 
