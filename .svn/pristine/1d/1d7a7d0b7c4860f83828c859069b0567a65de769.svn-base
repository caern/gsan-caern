	        		...
	        		Imovel imovel = (Imovel)colecaoImovel.iterator().next();        		
			       		...
			       		if((imovel.getLigacaoAguaSituacao().getId().intValue() == LigacaoAguaSituacao.LIGADO.intValue() 
	        				...
	        				if(sitAguaEsgotoCancel){
		        				imovel = excluirImovelSituacaoAguaEsgoto(imovel, fachada, sistemaParametro, 
		        					manutencaoRegistroActionForm);
	        				}
...
//    		}
	...
	private Imovel excluirImovelSituacaoAguaEsgoto(Imovel imovel, Fachada fachada, SistemaParametro sistemaParametro,
		...
		if(!Util.isVazioOrNulo(colecaoLigacaoAgua)){
			ligacaoAguaSituacao = 
					(LigacaoAguaSituacao) Util.retonarObjetoDeColecao(colecaoLigacaoAgua);
		}
		...
		if(!Util.isVazioOrNulo(colecaoLigacaoEsgoto)){
			ligacaoEsgotoSituacao = (LigacaoEsgotoSituacao) 
					Util.retonarObjetoDeColecao(colecaoLigacaoEsgoto);
		}
		...
		imovel.setObservacaoCancelamento(
			manutencaoRegistroActionForm.getMensagemObservacaoExclusao());
