...
//		Integer idFuncionalidadeIniciada = this.getIdFuncionalidadeIniciada();
		...
		if (colecao != null && !colecao.isEmpty()) {		
			...
			while(it.hasNext()){
				RelatorioCertidaoNegativaClienteBean cliente = 
						(RelatorioCertidaoNegativaClienteBean) it.next();
				
				if(cliente.getIndicadorContaVencidas() != null 
						&& cliente.getIndicadorContaVencidas()){
		...
		if(!imovelComParcelamento && !indicadorContaVencidas){
			textoCertidaoNegativa = "Pelo presente instrumento certificamos, para fins de direito, " +
					...
					"até a presente data: " + Util.formatarData(dataAtual) + ".";
		}
		...
		if (sistemaParametro.getCodigoEmpresaFebraban().equals(SistemaParametro.CODIGO_EMPRESA_FEBRABAN_CAEMA)) {
			...
			if(indicadorContaVencidas){
				parametros.put("nomeRelatorio", "CERTIDÃO POSITIVA DE DÉBITOS POR CLIENTE");
			}
			
			if(imovelComParcelamento && !indicadorContaVencidas){
				parametros.put("nomeRelatorio", "CERTIDÃO NEGATIVA DE DÉBITOS POR CLIENTE - COM EFEITO POSITIVO");
			}
