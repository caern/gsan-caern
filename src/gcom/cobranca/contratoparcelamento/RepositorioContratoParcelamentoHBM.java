	...
	public Object[] pesquisarDadosContratoParcelamento(String numeroContrato) throws ErroRepositorioException {
		...
		try{	
			consulta = " SELECT cpar.cpar_id AS idContrato, " //0
					 ...
					 + " WHERE cpar.cpar_tmcancelamento IS NULL AND (upper(cpar.cpar_nncontrato) like upper('"+numeroContrato+"'))"
...
/**
	...
	throws ErroRepositorioException {
		...
		try{	
			consulta = " select count(distinct cpar_nncontrato) AS quantidade "
					 ...
					 + "   cpar.cpar_tmcancelamento IS NULL ";
	...
	public ContratoParcelamento obterDadosContratoParcelamento(Integer idContratoParcelamento) 
		...
		try {
			...
			consulta = " SELECT cpar.cpar_id AS idContrato, " //0
					 ...
					 + "   cpar.cpar_nncontrato AS numeroContrato " //9
					 + " FROM cobranca.contrato_parcel cpar "
					 ...
					 + " WHERE cpar.cpar_id = :idContratoParcelamento ";
			...
			if (dados != null) {
				...
				if (dados[9] != null) {
					contratoParcelamento.setNumero((String) dados[9]);
				}
	...
	/**
	 * [UC1348] Excluir Débito Automático de Parcelamento por Cliente
	 * 
	 * [FS0005] - Verificar se o contrato parcelamento é débito automático para os dados informados
	 * 
	 * @author Mariana Victor
	 * @data 21/06/2012
	 */
	public Boolean verificarDebitoAutomaticoContratoParcelamento(Integer idContratoParcelamento)
		...
		try {
			...
			consulta = " select count(*) as quantidade "
					+ " from arrecadacao.debito_aut_parc_cliente dapc "
					+ " where dapc.cpar_id = :idContratoParcelamento "
					+ "  and dapc.dapc_dtexclusao is null";
	...
	public Boolean verificarDebitoAutomaticoContratoParcelamentoBancoAgencia(Integer idContratoParcelamento,
		...
		try {
			...
			consulta = " select count(*) as quantidade "
					+ " from arrecadacao.debito_aut_parc_cliente dapc "
					+ "  inner join arrecadacao.agencia agen on agen.agen_id = dapc.agen_id "
					+ " where dapc.cpar_id = :idContratoParcelamento "
					+ "  and agen.agen_cdagencia = :codigoAgencia "
					+ "  and agen.bnco_id = :codigoBanco "; 

			retorno = (Integer) session.createSQLQuery(consulta)
					.addScalar("quantidade", Hibernate.INTEGER)
					.setInteger("idContratoParcelamento", idContratoParcelamento)
					.setString("codigoAgencia", codigoAgencia)
					.setString("codigoBanco", codigoBanco)
					.setMaxResults(1).uniqueResult();
	 ...
	 * [UC1348] Excluir Débito Automático de Parcelamento por Cliente
	 * 
	 * [FS0004] - Verificar data de opção posterior já informada
	 * 
	 * @author Mariana Victor
	 * @data 21/06/2012
	 */
	public Boolean verificarDebitoAutomaticoContratoParcelamentoDataOpcao(Integer idContratoParcelamento,
		...
		try {
			...
			if(incluirIgual) {
				consulta = consulta + "  and dapc.dapc_dtopcao >= :dataOpcao "; 
			} else {
				consulta = consulta + "  and dapc.dapc_dtopcao > :dataOpcao ";
			}
	...
	/**
	 * [UC1347] Inserir Débito Automático de Parcelamento por Cliente
	 * 
	...
	public Boolean verificarDebitoAutomaticoContratoParcelamentoDataOpcaoExcluida(
		...
		try {
			...
			consulta = " select count(*) as quantidade "
					+ " from arrecadacao.debito_aut_parc_cliente dapc "
					+ " where dapc.cpar_id = :idContratoParcelamento "
					+ "  and dapc.dapc_dtexclusao is not null "
					+ "  and dapc.dapc_dtopcao = :dataOpcao ";
	 ...
	 * [UC1348] Excluir Débito Automático de Parcelamento por Cliente
	 * 
	 * 2.1.	Remover os movimentos de débitos automáticos que ainda não foram enviados ao banco 
	 * 
	 * @author Mariana Victor
	 * @data 21/06/2012
	 */
	public void removerMovimentosDebitoAutomaticoContratoParcelamento(Integer idContratoParcelamento) 
		...
		try {
			...
			String delete = " delete from arrecadacao.debito_aut_mov_parc_clie damp"
						  + " where damp.damp_tmenviobanco is null"
					      + " and damp.dapc_id in"
					   	  + " (select dapc_id from arrecadacao.debito_aut_parc_cliente dapc where cpar_id = ?" 
						  + " and dapc_dtexclusao is null " 
						  + " and dapc.dapc_id = damp.dapc_id)";
			
			st = jdbcCon.prepareStatement(delete);
			st.setInt(1,idContratoParcelamento);
	 ...
	 * [UC1348] Excluir Débito Automático de Parcelamento por Cliente
	 * 
	 * 2.2.	Atualiza a data da exclusão com a data corrente 
	 * 
	 * @author Mariana Victor
	 * @data 21/06/2012
	 */
	public void atualizarDataExclusaoDebitoAutomaticoContratoParcelamento(Integer idContratoParcelamento)
		...
		try {
			consulta = "update gcom.arrecadacao.debitoautomatico.DebitoAutomaticoParcelamentoCliente "
				+ " set dapc_dtexclusao = :dataAtual "
				+ " where cpar_id = :idContratoParcelamento and dapc_dtexclusao is null ";
	 ...
	 * [UC1348] Excluir Débito Automático de Parcelamento por Cliente
	 * 
	 * 2.3.	Atualiza o indicador de débito automático da tabela CONTRATO_PARCELAMENTO para NÃO 
	 * 
	 * @author Mariana Victor
	 * @data 21/06/2012
	 */
	public void atualizarIndicadorDebitoAutomaticoContratoParcelamento(
		...
		try {
			consulta = "update gcom.cobranca.contratoparcelamento.ContratoParcelamento "
				+ " set cpar_icdebitoatomatico = :indicadorDebitoAutomatico "
				+ " where cpar_id = :idContratoParcelamento ";
		
			session.createQuery(consulta)
					.setInteger("idContratoParcelamento", idContratoParcelamento)
					.setInteger("indicadorDebitoAutomatico", indicadorDebitoAutomatico)
					.executeUpdate();
	 ...
	 * [UC1348] Excluir Débito Automático de Parcelamento por Cliente
	 * 
	 * [FS0001] - Verificar existência de contrato de parcelamento
	 * 
	 * @author Mariana Victor
	 * @data 21/06/2012
	 */
	...
	public Collection<Object[]> obterPrestacoesContratoParcelamento(
		...
		try {
			...
			consulta = " SELECT cppr.cppr_id AS idPrestacao, " //0
					 + "   cppr.cppr_dtvencimento AS dataVencimento, " //1
					 + "   cppr.cppr_vlprestacao AS valorPrestacao " //2
					 + " FROM cobranca.contrato_parcel_prest cppr "
					 + " WHERE cppr.cpar_id = :idContratoParcelamento "
					 + "   and cppr.cppr_dtpagamento is null ";
