...
public class ControladorAcessoSEJB implements SessionBean {
	...
	public void inserirGrupo(Grupo grupo,
			Collection colecaoGrupoFuncionalidadeOperacao, Usuario usuarioLogado, String[] permissoesEspeciaisMarcadas)
			throws ControladorException {
		...
		// 4.2. Caso tenha selecionado alguma permissão especial para o grupo:
		//	4.2.1.  Insere, para cada permissão especial selecionada, os dados na tabela GRUPO_PERMISSAO_ESPECIAL
		if (permissoesEspeciaisMarcadas!=null){
			GrupoPermissaoEspecial grupoPermissaoEspecial = null;
			PermissaoEspecial permissaoEspecial = null;
			for (String permissao:permissoesEspeciaisMarcadas){
				permissaoEspecial = new PermissaoEspecial();
				permissaoEspecial.setId(Integer.parseInt(permissao));
			
				grupoPermissaoEspecial = new GrupoPermissaoEspecial(new GrupoPermissaoEspecialPK(grupo.getId(), Integer.parseInt(permissao)));
				getControladorUtil().inserir(grupoPermissaoEspecial);
			}			
		}
		/*
...
//						"atencao.data_nascimento.incorreta.login", null, login);
	...
	public boolean verificarAcessoPermitidoFuncionalidade(
		...
		if (colecaoFuncionalidade != null && !colecaoFuncionalidade.isEmpty()) {
			...
			Iterator iteratorColecaoFuncionalidade = colecaoFuncionalidade.iterator();
			
			while (iteratorColecaoFuncionalidade.hasNext()) {
			// Recupera a funcionalidade da coleção
			funcionalidade = (Funcionalidade) iteratorColecaoFuncionalidade
					.next();
	...
	public boolean verificarAcessoPermitidoOperacao(Usuario usuarioLogado,
		...
		if (colecaoOperacao != null && !colecaoOperacao.isEmpty()) {
			...
			Iterator it = colecaoOperacao.iterator();
