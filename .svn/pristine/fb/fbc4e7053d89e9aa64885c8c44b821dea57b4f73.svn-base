...
public class TarefaBatchGerarArquivoTextoContasCobrancaEmpresa extends
	...
	public Object executar() throws TarefaException {
		...
		String nomeAbreviadoEmpresa = (String) getParametro("nomeAbreviadoEmpresa");

		if (nomeAbreviadoEmpresa.equalsIgnoreCase(
				SistemaParametro.EMPRESA_COMPESA)) {
			Collection<Integer> idsComandos = (Collection<Integer>) getParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH);
	
			Iterator<Integer> it = idsComandos.iterator();
	
			while (it.hasNext()) {
	
				Integer idComando = (Integer) it.next();

				idsRegistros = new ArrayList<Integer>();
				idsRegistros.add(idComando);
				
				enviarMensagemControladorBatch(
						ConstantesJNDI.BATCH_GERAR_ARQUIVO_TEXTO_CONTAS_COBRANCA_POR_EMPRESA,
						new Object[] { idsRegistros, idEmpresa, idComando,
