			...
			if (action.equals("voltar")) {
				...
				if ( confirmado == null ){
					...
					for (int i = (indice - 1) * 12; i < (indice - 1) * 12 + 12
						...
						// RM 5768 - Inclusão de alertas para não deixar que o
						// leiturista informe possíveis erros
					...
					if (imoveisAnormalidades.size() > 0) {
						...
						// Montar mensagem de confirmação com os dados
						// (matrícula e anormalidade) dos imóveis
...
/*				retorno = naoAcabou(actionMapping, httpServletRequest, retorno,
				...
				// Se houverem anormalidades lançar página de confirmação
				if (imoveisAnormalidades.size() > 0) {
					...
					// Montar mensagem de confirmação com os dados (matrícula e
					// anormalidade) dos imóveis
					Set<Integer> chaves = imoveisAnormalidades.keySet();
					String mensagem = "</br></br>";
					for (Integer in : chaves) {
						if (in != null) {
