...
/**
 * classe responsável por criar o relatório de analise de faturamento por período
 * 
 * @author Davi Menezes
 * @created 06 de Outubro de 2011
 */
public class RelatorioAnaliseFaturamentoPeriodo extends TarefaRelatorio {

	private static final long serialVersionUID = 1L;
	public RelatorioAnaliseFaturamentoPeriodo(Usuario usuario) {
		super(usuario, ConstantesRelatorios.RELATORIO_ANALISE_FATURAMENTO_PERIODO);
	}
	...
	public Object executar() throws TarefaException {
		...
		String mesAnoInicialFaturamento =  (String) getParametro("mesAnoInicialFaturamento");
		String mesAnoFinalFaturamento = (String) getParametro("mesAnoFinalFaturamento");
		String mesAnoFaturamento = (String) getParametro("mesAnoFaturamento");
		int tipoFormatoRelatorio = (Integer) getParametro("tipoFormatoRelatorio");
		InformarDadosGeracaoRelatorioConsultaPeriodoHelper informarDadosGeracaoRelatorioConsultaHelper
			= (InformarDadosGeracaoRelatorioConsultaPeriodoHelper) getParametro("informarDadosGeracaoRelatorioConsultaPeriodoHelper");
		...
		List<Object[]> retornoConsulta = fachada.consultarResumoAnaliseFaturamentoRelatorioPeriodo(informarDadosGeracaoRelatorioConsultaHelper);
		...
		if( retornoConsulta != null && !retornoConsulta.equals("")
			...
			for (Object[] retornoObject : retornoConsulta) {
					...
					String agrupamento = "";
					if(!mesAnoInicialFaturamento.equals(mesAnoFinalFaturamento)){
						if(retornoObject[10] != null){
							agrupamento = (String) retornoObject[10];
						}
						String anoMes = Util.formatarAnoMesParaMesAno(idQuebra);
						String mes = anoMes.substring(0, 2);
						String ano = anoMes.substring(2, 7);
						idQuebra = Util.getNomeMes(mes) + ano;
						
					}
					else{
						agrupamento = this.obterDescricaoDoAgrupamento(informarDadosGeracaoRelatorioConsultaHelper,
					...
					Collection<Object[]> retornoConsultaDetalheCredito = 
			        	fachada.consultarResumoAnaliseFaturamentoPeriodoDetalhe(informarDadosGeracaoRelatorioConsultaHelper);
					...
					for (Object[] dados : retornoConsultaDetalheDebito) {
			        	ResumoFaturamentoSimulacaoDetalheHelper  helper 
			        	 	= new ResumoFaturamentoSimulacaoDetalheHelper((String)dados[0], "", Util.formatarMoedaReal((BigDecimal) dados[1]));
		...
		boolean mesesIguais = false;
		if(!mesAnoInicialFaturamento.equals(mesAnoFinalFaturamento)){
			String anoInicial = mesAnoInicialFaturamento.substring(2, 7);
			String anoFinal = mesAnoFinalFaturamento.substring(2, 7);
			
			mesAnoInicialFaturamento = mesAnoInicialFaturamento.substring(0,2);
			mesAnoFinalFaturamento = mesAnoFinalFaturamento.substring(0,2);
			
			mesAnoInicialFaturamento = Util.getNomeMes(mesAnoInicialFaturamento);
			mesAnoFinalFaturamento = Util.getNomeMes(mesAnoFinalFaturamento);
			
			mesAnoInicialFaturamento = mesAnoInicialFaturamento + anoInicial;
			mesAnoFinalFaturamento = mesAnoFinalFaturamento + anoFinal;
			
			parametros.put("mesAnoInicialFaturamento", mesAnoInicialFaturamento);
			parametros.put("mesAnoFinalFaturamento", mesAnoFinalFaturamento);
		...
		if(!mesesIguais){
			retorno = gerarRelatorio(ConstantesRelatorios.RELATORIO_ANALISE_FATURAMENTO_PERIODO, parametros,
				ds, tipoFormatoRelatorio);
		}else{
			retorno = gerarRelatorio(ConstantesRelatorios.RELATORIO_ANALISE_FATURAMENTO, parametros, 
				ds, tipoFormatoRelatorio);
		}
	...
	private String obterDescricaoDoAgrupamento(InformarDadosGeracaoRelatorioConsultaPeriodoHelper 
