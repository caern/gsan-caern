...
public class ControladorLocalidadeSEJB implements SessionBean {
	...
	public void atualizarQuadra(Quadra quadra, Usuario usuarioLogado, Collection colecaoQuadraFace)
		...
		if (quadra != null) {
			...
			if(quadraNaBase.getRota() != null && quadraNaBase.getRota().getFaturamentoGrupo() != null &&
					quadraNaBase.getRota().getFaturamentoGrupo().getId() != null){
				// Autor: Sávio Luiz
				// Data: 20/08/2013
				// Pesquisa se existem imóveis na tabela de movimento_roteiro_empresa 
				// para a rota e o mês e ano da referência
			    getControladorMicromedicao().verificarComandoNaoRealizadoParaRota(quadraNaBase.getRota().getFaturamentoGrupo().getId());
			}
		...
		if (!getControladorAcesso().verificarAcessoAbrangencia(abrangencia)) {
			...
			if(quadra.getRota() != null && quadra.getRota().getFaturamentoGrupo() != null &&
					quadra.getRota().getFaturamentoGrupo().getId() != null){
				// Autor: Sávio Luiz
				// Data: 20/08/2013
				// Pesquisa se existem imóveis na tabela de movimento_roteiro_empresa 
				// para a rota e o mês e ano da referência	
			    getControladorMicromedicao().verificarComandoNaoRealizadoParaRota(quadra.getRota().getFaturamentoGrupo().getId());
			}
	...
	public Localidade pesquisarLocalidade(Integer idLocalidade) throws ControladorException {
		try{
			return repositorioLocalidade.pesquisarLocalidade(idLocalidade);
		}catch(ErroRepositorioException e){
			e.printStackTrace();
			throw new ControladorException("erro.sistema", e);
		}
	...
	public Collection pesquisarIdLocalidadeCobrancaPorResultado() throws ControladorException {
		try {
			return repositorioLocalidade.pesquisarIdLocalidadeCobrancaPorResultado();
		} catch (ErroRepositorioException ex) {
			sessionContext.setRollbackOnly();
			throw new ControladorException("erro.sistema", ex);
		}
