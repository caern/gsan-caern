			...
			if (login == null || login.trim().equals("")) {
				...
				if (!this.verificarExistenciaLogin(login)) {
					...
					if (usuarioLogado == null) {
						this.reportarErros(httpServletRequest,"atencao.usuario.senha_invalida");
						retorno = actionMapping.findForward("telaLogin");
						...
						if (loginUsuarioSessao.equals(login)) {
							...
							if (numeroTentativasPermitidas != null && numeroTentativas.intValue() > numeroTentativasPermitidas.intValue()) {
...
//						}
					...
					// @autor Thúlio Araújo - 23/11/2011
					// 1. O sistema verifica se está parametrizado na base para utilizar o usuário do GSAN como
					// usuário de banco ( PARM_ICLOGAUSUARIODB da tabela de cadastro.sistema_parametros com valor igual a 1 )
					if(this.getSistemaParametro().getIndicadorUsuarioBd() != null && 
							this.getSistemaParametro().getIndicadorUsuarioBd().intValue() == 1){
						if (usuarioLogado != null){
							// 1.1. Selecionar o login e senha da base de dados para o usuário que foi logado no GSAN 
							// FS-0001 - Validar usuário da base para usuário logado retorne nulo ).
							UsuarioBanco usuarioBanco = Fachada.getInstancia().validarUsuarioBaseLogado(usuarioLogado.getId());
							if (usuarioBanco != null){
								String mensagemRetorno = Fachada.getInstancia().validarUsuarioBaseLogadoDicionarioDados(usuarioBanco.getLogin(), this.getSistemaParametro());
								if (mensagemRetorno == null){
									int codMensagem = HibernateUtil.alterarSessaoDaFabrica(usuarioBanco.getLogin(), usuarioBanco.getSenha(), this.getSistemaParametro().getNomeAbreviadoEmpresa());
									if (codMensagem != UsuarioBanco.MENSAGEM_SUCESSO){
										throw new ActionServletException("atencao.usuario.nao_problema_acesso", usuarioLogado.getLogin(), UsuarioBanco.MENSAGEM_ERRO_0004);
									}
								}else{
									throw new ActionServletException("atencao.usuario.nao_problema_acesso", usuarioLogado.getLogin(), mensagemRetorno);
								}
							// 2  Caso contrário, retorna a seguinte mensagem de erro: "[0001] - Usuário não possui 
							// cadastro na tabela de usuários de banco de dados "
							}else{
								throw new ActionServletException("atencao.usuario.nao_problema_acesso", usuarioLogado.getLogin(), UsuarioBanco.MENSAGEM_ERRO_0001);
							}
						}
					}
				}
