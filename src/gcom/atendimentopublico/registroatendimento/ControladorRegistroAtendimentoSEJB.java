...
//					MunicipioFeriado.class.getName());
		...
		if (colecaoSolicitacaoTipoEspecificacao != null
			...
			if (solicitacaoTipoEspecificacao.getIndicadorVerificarDebito() != null
				...
				.getControladorCobranca().obterDebitoImovelOuCliente(1,
						...
						dataVencimentoFinal, 1, 1, 1, 1, 1, 1, 1, true, 2);
	...
	public Integer inserirTipoSolicitacaoEspecificacao(
		...
		while (iteratorSolicitacaoTipoEspecificacao.hasNext()) {
			...
			/*RM 5924
			* Inseri os ArquivosPOP na base */
			if (solicitacaoTipoEspecificacao.getColecaoArquivosPOP()!=null){
			for (ArquivoProcedimentoOperacionalPadrao pop : solicitacaoTipoEspecificacao.getColecaoArquivosPOP()){
				solicitacaoTipoEspecificacao.setId(idSolicitacaoTipoEspecificacao);
				pop.setSolicitacaoTipoEspecificacao(solicitacaoTipoEspecificacao);
				pop.setUltimaAlteracao(new Date());
				getControladorUtil().inserir(pop);
			}
			}
	...
	public void validarCamposObrigatoriosRA_2ABA(String idImovel,
		...
		if(indicadorInformarPagamentoDuplicidade == ConstantesSistema.SIM.shortValue()){
			...
			if(colecaoPagamento == null || colecaoPagamento.isEmpty()){
				throw new ControladorException("atencao.nao_exite_pagamento_duplicidade");
			}
		}
	...
	throws ControladorException {
		...
		while (iteratorSolicitacaoTipoEspecificacao.hasNext()) {
			...
			if (solicitacaoTipoEspecificacao.getId() == null) {
				...
				//RM 5924 - Amélia Pessoa - Inseri arquivosPOP
				if (solicitacaoTipoEspecificacao.getColecaoArquivosPOP()!=null){
					for (ArquivoProcedimentoOperacionalPadrao pop : solicitacaoTipoEspecificacao.getColecaoArquivosPOP()){
						solicitacaoTipoEspecificacao.setId(idSolicitacaoTipoEspecificacao);
						pop.setSolicitacaoTipoEspecificacao(solicitacaoTipoEspecificacao);
				...
				if (colecaoEspecificacaoServicoTipo != null
					...
					while (iteratorSolicitacaoTipoServico.hasNext()) {
						...
						Object retorno = getControladorUtil().inserir(especificacaoServicoTipo);
				...
				Collection<ArquivoProcedimentoOperacionalPadrao> colecao = null;
				FiltroArquivoProcedimentoOperacionalPadrao filtroArquivoProcedimentoOperacionalPadrao = (FiltroArquivoProcedimentoOperacionalPadrao) (new ArquivoProcedimentoOperacionalPadrao()).retornaFiltro();
				filtroArquivoProcedimentoOperacionalPadrao.limparListaParametros();
				filtroArquivoProcedimentoOperacionalPadrao.adicionarParametro(new ParametroSimples(FiltroArquivoProcedimentoOperacionalPadrao.SOLICITACAO_TIPO_ESPECIFICACAO_ID, solicitacaoTipoEspecificacao.getId()));
				colecao =  getControladorUtil().pesquisar(filtroArquivoProcedimentoOperacionalPadrao,ArquivoProcedimentoOperacionalPadrao.class.getName());
				ArquivoProcedimentoOperacionalPadrao[] excluir = new ArquivoProcedimentoOperacionalPadrao[colecao.size()];
				int auxiliar=0;
				
				//Verifica se algum arquivo foi exluído pelo usuário para excluir também no BD
				loopMain: for (ArquivoProcedimentoOperacionalPadrao popExcluir : colecao){
					for (ArquivoProcedimentoOperacionalPadrao pop : solicitacaoTipoEspecificacao.getColecaoArquivosPOP()){
						if(popExcluir!=null && popExcluir.getId()!=null && pop!=null && pop.getId()!=null && popExcluir.getId().intValue()==pop.getId().intValue()){
							continue loopMain;
						}
					}
					excluir[auxiliar] = popExcluir;
					auxiliar++;
				}
				...
				for (int i=0;i<excluir.length;i++){
					if (excluir[i]!=null){
						//Excluindo
						getControladorUtil().remover(excluir[i]);
					}
				}
				
				//RM 5924 - Amélia Pessoa - Atualiza arquivosPOP
				if (solicitacaoTipoEspecificacao.getColecaoArquivosPOP()!=null){
					for (ArquivoProcedimentoOperacionalPadrao pop : solicitacaoTipoEspecificacao.getColecaoArquivosPOP()){
						if (pop.getId()==null){
							pop.setSolicitacaoTipoEspecificacao(solicitacaoTipoEspecificacao);
			...
			if (colecaoSolicitacaoTipoEspecificacaoRemovidos != null
				...
				while (iteratorSolicitacaoTipoEspecificacaoRemovidos.hasNext()) {
					...
					if (solicitacaoTipoEspecificacaoRemovido.getId() != null) {
						...
						//RM 5924 - Amélia Pessoa - Remove arquivosPOP
						if (solicitacaoTipoEspecificacaoRemovido.getColecaoArquivosPOP()!=null){
							ArquivoProcedimentoOperacionalPadrao[] excluir = new ArquivoProcedimentoOperacionalPadrao[solicitacaoTipoEspecificacaoRemovido.getColecaoArquivosPOP().size()];
							int i=0;
							for (ArquivoProcedimentoOperacionalPadrao pop : solicitacaoTipoEspecificacaoRemovido.getColecaoArquivosPOP()){
								excluir[i]=pop;
								i++;
							}
							for (int j=0;j<excluir.length;j++){
								if (excluir[j]!=null){
									//Excluindo
									getControladorUtil().remover(excluir[j]);
								}
							}
						}
...
//		registradorOperacao.registrarOperacao(raEncerramentoComando);
	...
	public void validarRegistroAtendimentoAnexos(FileItem arquivoAnexo) throws ControladorException{
		...
		if (!arquivoExtensao.equalsIgnoreCase("JPG") &&
			...
			!arquivoExtensao.equalsIgnoreCase("DOCX") &&
			!arquivoExtensao.equalsIgnoreCase("PDF") &&
			!arquivoExtensao.equalsIgnoreCase("XLS") &&
			!arquivoExtensao.equalsIgnoreCase("XLSX")){
		...
		if (sistemaParametro.getTamanhoMaximoAnexoRA() != null && 
			arquivoAnexo.getSize() > (sistemaParametro.getTamanhoMaximoAnexoRA() * 1048576)){
			
			sessionContext.setRollbackOnly();
			throw new ControladorException("atencao.imagem_tamanho_maximo", null, sistemaParametro.getTamanhoMaximoAnexoRA().toString());
		}
	}
	...
	public void atualizarRegistroAtendimentoAnexo(RegistroAtendimento registroAtendimento, 
			...
			if (colecaoRegistroAtendimentoAnexoAtual != null && !colecaoRegistroAtendimentoAnexoAtual.isEmpty()){
				...
				while (itAtual.hasNext()){
					...
					if (colecaoRegistroAtendimentoAnexo.contains(registroAtendimentoAnexoAtual)){
						...
						colecaoRegistroAtendimentoAnexo.remove(registroAtendimentoAnexo);
					}
					else {
		...
		else if (registroAtendimento != null){
			
			try {
				
				repositorioRegistroAtendimento.removerRegistroAtendimentoAnexo(registroAtendimento.getId());
			} 
			catch (ErroRepositorioException e) {
				sessionContext.setRollbackOnly();
				throw new ControladorException("erro.sistema", e);
			...
			if (colecaoRAConta != null && !colecaoRAConta.isEmpty()){
				...
				while (it.hasNext()){
					...
					if (registroAtendimentoConta.getUltimaAlteracao() == null) {
						...
						RegistroAtendimentoContaPK registroAtendimentoContaPK = new RegistroAtendimentoContaPK();
						registroAtendimentoContaPK.setContaId(registroAtendimentoConta.getContaGeral().getId());
						registroAtendimentoContaPK.setRegistroAtendimentoId(idRegistroAtendimento);
						
						registroAtendimentoConta.setComp_id(registroAtendimentoContaPK);
						registroAtendimentoConta.setUltimaAlteracao(new Date());
						
						//Verifica se o registroAtendimentoConta ja existe;
						FiltroRegistroAtendimentoConta filtroRegistroAtendimentoConta = new FiltroRegistroAtendimentoConta();
						filtroRegistroAtendimentoConta.adicionarParametro(new ParametroSimples(
							FiltroRegistroAtendimentoConta.REGISTRO_ATENDIMENTO_ID, registroAtendimentoConta.getComp_id().getRegistroAtendimentoId()));
						filtroRegistroAtendimentoConta.adicionarParametro(new ParametroSimples(
							FiltroRegistroAtendimentoConta.CONTA_ID, registroAtendimentoConta.getComp_id().getContaId()));
						
						Collection colecaoRegistroAtendimentoConta = this.getControladorUtil().pesquisar(
							filtroRegistroAtendimentoConta, RegistroAtendimentoConta.class.getName());
						
						//Caso encontre algum registro na base atualiza o registro caso contrario insere um novo registro
						if(colecaoRegistroAtendimentoConta != null && !colecaoRegistroAtendimentoConta.isEmpty()){
...
//			getControladorFaturamento().inserirCreditoRealizadoCategoria(creditoRealizadoInserir,colecaoCategoria);
	 ...
	 * @throws ControladorException
	 */
	public RegistroAtendimento pesquisarRegistroAtendimentoPorRA(
			Integer id) throws ControladorException{
		try {
			 return repositorioRegistroAtendimento.pesquisarRegistroAtendimento(id);
		} catch (ErroRepositorioException erx) {
			erx.printStackTrace();
			throw new ControladorException("erro.sistema", erx);
		}
	 ...
	 * @throws ControladorException
	 */
	public Object[] pesquisarUnidadeOrganizacionalUsuario(Integer idUsuario) throws ControladorException{
		try {
			 return repositorioRegistroAtendimento.pesquisarUnidadeOrganizacionalUsuario(idUsuario);
		} catch (ErroRepositorioException erx) {
			erx.printStackTrace();
			throw new ControladorException("erro.sistema", erx);
		}
	...
	public Object[] pesquisarDadosRASolicitante(Integer idRegistroAtendimento)
			throws ControladorException {
		try {
			 return repositorioRegistroAtendimento.pesquisarDadosRASolicitante(idRegistroAtendimento);
		} catch (ErroRepositorioException erx) {
			erx.printStackTrace();
			throw new ControladorException("erro.sistema", erx);
		}
	...
	/**
	 * Pesquisar Id Tipo Solicitacao por RA [UC1249] Filtro OS Encerradas por Acompanhamento de Serviço
	 * 
	 * @author Fernanda Almeida
	 * @date 09/11/2011
	 * @throws ControladorException
	 */
	public Object[] pesquisarTipoSolicitacao(Integer idRA) throws ControladorException {
		try {
			 return repositorioRegistroAtendimento.pesquisarTipoSolicitacao(idRA);
		} catch (ErroRepositorioException erx) {
			erx.printStackTrace();
			throw new ControladorException("erro.sistema", erx);
		}
	...
	/**
	 * RM5924 - Adicionar arquivo de procedimentos operacionais padrões POPs
	 ...
	 * @throws ErroRepositorioException
	 */
	public Collection<ArquivoProcedimentoOperacionalPadrao> pesquisarArquivoProcedimentoOperacionalPadrao(FiltrarArquivoProcedimentoOperacionalPadraoHelper helper) throws ControladorException{
		try{
			return repositorioRegistroAtendimento.pesquisarArquivoProcedimentoOperacionalPadrao(helper);
		} catch (ErroRepositorioException erx) {
			erx.printStackTrace();
			throw new ControladorException("erro.sistema", erx);
		}
	...
	/**
	 * [UC01527] Inserir Ocorrencia Operacional
	 * 
	 * @author Rômulo Aurélio
	 * @date 11/07/2013
	 * 
	 * @param ocorrenciaOperacional,
	 * @param usuarioLogado
	 * @throws ControladorException
	 */
	public Integer inserirOcorrenciaOperacional(OcorrenciaOperacional ocorrenciaOperacional,
			Usuario usuarioLogado) throws ControladorException {

		// VALIDANDO OS DADOS Da ocorrencia operacional
		this.validarOcorrenciaOperacional(ocorrenciaOperacional, true);
		...
		ocorrenciaOperacional.setOperacaoEfetuada(operacaoEfetuada);
		ocorrenciaOperacional.adicionarUsuario(usuarioLogado,
				UsuarioAcao.USUARIO_ACAO_EFETUOU_OPERACAO);
		registradorOperacao.registrarOperacao(ocorrenciaOperacional);*/
		// ------------ REGISTRAR TRANSAÇÃO ----------------

		return (Integer) getControladorUtil().inserir(ocorrenciaOperacional);
	...
	public void validarOcorrenciaOperacional(OcorrenciaOperacional ocorrenciaOperacional, boolean acao)
																										throws ControladorException {

		FiltroOcorrenciaOperacional filtroOcorrenciaOperacional = new FiltroOcorrenciaOperacional();
		OcorrenciaOperacional ocorrenciaOperacionalJaCadastrado = null;

		FiltroMunicipio filtroMunicipio = new FiltroMunicipio();

		filtroMunicipio.adicionarParametro(new ParametroSimples(FiltroMunicipio.ID,
																ocorrenciaOperacional.getMunicipio().getId()));
		...
		// Municipio
		if (ocorrenciaOperacional.getMunicipio() == null) {
		...
		// Localidade
		if (ocorrenciaOperacional.getLocalidade() == null
				|| ocorrenciaOperacional.getLocalidade().getId().toString().equalsIgnoreCase(""
		...
		// Bairro
		if (ocorrenciaOperacional.getBairro() == null
				|| ocorrenciaOperacional.getBairro().getId().toString().equalsIgnoreCase(""
						+ ConstantesSistema.NUMERO_NAO_INFORMADO)) {
			throw new ControladorException(	"atencao.required",
											null,
											"Bairro");
		...
		// Descricao
		if (ocorrenciaOperacional.getDescricao() == null || ocorrenciaOperacional.getDescricao().equalsIgnoreCase("")) {
			throw new ControladorException(	"atencao.required",
											null,
											"Descrição");
		}
		...
		// Tipo Ocorrencia
		if (ocorrenciaOperacional.getOcorrenciaOperacionalTipo() == null
				|| ocorrenciaOperacional.getOcorrenciaOperacionalTipo().getId().toString().equalsIgnoreCase(""
						+ ConstantesSistema.NUMERO_NAO_INFORMADO)) {
			throw new ControladorException(	"atencao.required",
											null,
											"Tipo Occorrência");
		...
		// Motivo Ocorrencia
		if (ocorrenciaOperacional.getOcorrenciaOperacionalMotivo() == null
				|| ocorrenciaOperacional.getOcorrenciaOperacionalMotivo().getId().toString().equalsIgnoreCase(""
						+ ConstantesSistema.NUMERO_NAO_INFORMADO)) {
			throw new ControladorException(	"atencao.required",
											null,
											"Motivo Ocorrência");
		...
		// Data Ocorrencia
		if (ocorrenciaOperacional.getDataOcorrencia() == null) {
			throw new ControladorException(	"atencao.required",
											null,
											"Data da Ocorrência");
		}
		...
		// Areas Afetadas
		if (ocorrenciaOperacional.getAreasAfetadas() == null || ocorrenciaOperacional.getAreasAfetadas().equals("")) {
		...
		//COMPARA SE A DATA DE OCORRÊNCIA E MAIOR QUE A DATA DE CONCLUSÃO SE FOR LEVANDA A EXCEPTION
		if(ocorrenciaOperacional.getDataConclusao()!=null && ocorrenciaOperacional.getDataConclusao().toString().trim().equalsIgnoreCase("")){
			if(ocorrenciaOperacional.getDataOcorrencia().compareTo(ocorrenciaOperacional.getDataConclusao())>1){
				
				throw new ControladorException(	"atencao.data_ocorrencia_maior_que_data_conclusao");
			}
		}
	
		// VERIFICANDO SE O USUÁRIO JÁ FOI CADASTRADO
		filtroOcorrenciaOperacional.limparListaParametros();
		...
		filtroOcorrenciaOperacional.adicionarParametro(new ParametroSimples(FiltroOcorrenciaOperacional.OCORRENCIA_OPERACIONAL_TIPO_ID,
																			ocorrenciaOperacional.getOcorrenciaOperacionalTipo().getId()));
		
		filtroOcorrenciaOperacional.adicionarParametro(new ParametroNulo(FiltroOcorrenciaOperacional.DATA_CONCLUSAO));


		Collection colecaoOcorrenciaOperacional = getControladorUtil().pesquisar(filtroOcorrenciaOperacional, OcorrenciaOperacional.class.getName());

		if (colecaoOcorrenciaOperacional != null && !colecaoOcorrenciaOperacional.isEmpty()) {

			ocorrenciaOperacionalJaCadastrado = (OcorrenciaOperacional) colecaoOcorrenciaOperacional.iterator().next();

			if (acao) {

				throw new ControladorException(	"atencao.ocorrencia_operacional_ja_existente",
												null,
												ocorrenciaOperacionalJaCadastrado.getMunicipio().getNome(),
												ocorrenciaOperacionalJaCadastrado.getLocalidade().getDescricao(),
												ocorrenciaOperacionalJaCadastrado.getBairro().getNome(),
												ocorrenciaOperacionalJaCadastrado.getOcorrenciaOperacionalTipo().getDescricao());

			} else if (!ocorrenciaOperacional.getId().toString().equals(ocorrenciaOperacionalJaCadastrado.getId().toString())
					&& ocorrenciaOperacional.getMunicipio().getId().toString().equals(ocorrenciaOperacionalJaCadastrado.getMunicipio().getId().toString())
					&& ocorrenciaOperacional.getLocalidade().getId().toString().equals(ocorrenciaOperacionalJaCadastrado.getLocalidade().getId().toString())
					&& ocorrenciaOperacional.getBairro().getId().toString().equals(ocorrenciaOperacionalJaCadastrado.getBairro().getId().toString())
					&& ocorrenciaOperacional.getOcorrenciaOperacionalTipo().getId().toString().equals(ocorrenciaOperacionalJaCadastrado.getOcorrenciaOperacionalTipo().getId().toString())) {
	...
	/**
	 * [UC01529] Manter Ocorrencia Operacional
	 * 
	 * @author Rômulo Aurélio
	 * @date 15/07/2013
	 * 
	 ...
	 * @throws ControladorException
	 */
	public void atualizarOcorrenciaOperacional(OcorrenciaOperacional ocorrenciaOperacional,
			Usuario usuarioLogado) throws ControladorException {

		// VALIDANDO OS DADOS Da ocorrencia operacional
		this.validarOcorrenciaOperacional(ocorrenciaOperacional, false);

		FiltroOcorrenciaOperacional filtroOcorrenciaOperacional = new FiltroOcorrenciaOperacional();
		filtroOcorrenciaOperacional.adicionarParametro(new ParametroSimples(FiltroOcorrenciaOperacional.ID, ocorrenciaOperacional.getId()));
		
		Collection colecaoOcorrenciaOperacional = getControladorUtil().pesquisar(
			filtroOcorrenciaOperacional, OcorrenciaOperacional.class.getName());
		
		if (colecaoOcorrenciaOperacional == null
				|| colecaoOcorrenciaOperacional.isEmpty()) {
			sessionContext.setRollbackOnly();
			throw new ControladorException("atencao.atualizacao.timestamp");
		}
		
		OcorrenciaOperacional ocorrenciaOperacionalBase = (OcorrenciaOperacional)colecaoOcorrenciaOperacional.
				iterator().next();
		
		if (ocorrenciaOperacionalBase.getUltimaAlteracao().after(
			ocorrenciaOperacional.getUltimaAlteracao())) {
			sessionContext.setRollbackOnly();
			throw new ControladorException("atencao.atualizacao.timestamp");
		}
		
		getControladorUtil().atualizar(ocorrenciaOperacional);

	}

	/**
	 * [UC1530] Inserir Registro de Atendimento Simplificado
	 * [IT0011] Pesquisar RA pelo Número do Protocolo
	 * 
	 * @autor Mariana Victor
	 * @date 15/07/2013
	 * @throws ControladorException
	 */
	public boolean pesquisarRegistroAtendimentoNumeroProtocolo(
			String numeroProtocolo) throws ControladorException {
		try {
			return repositorioRegistroAtendimento.pesquisarRegistroAtendimentoNumeroProtocolo(numeroProtocolo);
		} catch (ErroRepositorioException erx) {
			erx.printStackTrace();
			throw new ControladorException("erro.sistema", erx);
		}
	 ...
	 * @throws ControladorException
	 */
	public String pesquisarNumeroProtocoloRegistroAtendimento(
			Integer idRA) throws ControladorException {
		try {
			return repositorioRegistroAtendimento.pesquisarNumeroProtocoloRegistroAtendimento(idRA);
		} catch (ErroRepositorioException erx) {
			erx.printStackTrace();
			throw new ControladorException("erro.sistema", erx);
		}
