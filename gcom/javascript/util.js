...
/**
 * Verifica se o valor digitado nao tem caracteres especiais
 * 
 * Uso: onkeypress="return isAlfaNumerico(event);"
...
/**
 * Verifica se o valor digitado nao tem caracteres especiais
 * (Aceita espaco, virgula, ponto e virgula, ponto final e dois pontos)
 * Uso: onkeypress="return isAlfaNumericoComSpacePontuacao(event);"
 * @param evento
 * @author Amelia Pessoa
 * @returns {Boolean}
 */
function isAlfaNumericoComSpacePontuacao(evento) {
    ...
    var tecla = null;
	  ...
	  if(tecla == 32 || tecla == 46 || tecla == 59 || tecla == 44 || tecla == 58){ 
		  //espaco, virgula, ponto e virgula, ponto final e dois pontos
		  return true;
	  }
      if (tecla == 8 || tecla == 13){ // backspace ou enter
	        return true;
...
/**
 * Verifica se o valor digitado nao tem caracteres especiais
 * (Aceita virgula, ponto)
 * Uso: onkeypress="return isAlfaNumericoComSpacePontuacao(event);"
 * @param evento
 * @author Fernanda Almeida
 * @returns {Boolean}
 */
function isNumericoComPontuacao(evento) {
    ...
    var tecla = null;
	  ...
	  if(tecla == 46 || tecla == 44 || tecla == 45 ){ 
		  // virgula,ponto final ,hifen
		  return true;
	  }
	  if (tecla == 8 || tecla == 13){ // backspace ou enter
	        return true;
...
function validaEnterComMensagem(tecla, caminhoActionReload, nomeCampoForm, nomeCampoMSG) {
	...
	if (codigo == 13) {
		//Alterado para verificar se o campo esta desabilitado - Raimundo Martins - 30/11/2011
	    if(objetoCampo.readOnly || objetoCampo.disabled){
	    	return false
	    }
...
/**
 * Função que verifica se a dependecia
 * de um campo foi preenchida e se
 * o campo é númerico
 * @autor: Raimundo Martins
 * */
function validaEnterDependenciaVerificaCampoNumerico(tecla, caminhoActionReload, campo, nomeCampo, idDependencia, nomeDependencia){
	//var form = document.forms[0];
    ...
    if (codigo == 13) {
		  if( idDependencia.length < 1 || isNaN(idDependencia) || ((idDependencia * 1) == 0) ||
			  ...
			  validaEnter(tecla,caminhoActionReload, campo.name);
		  }
    }
	else {
...
function calculaValorParcela() {
	...
	*/
	...
	/**
	 * essa função verifica se o campo 
	 * contém apenas números, caso não
	 * contenha, limpa o campo
	 * 
	 * @autor: Raimundo Martins
	 * */
	function validarCampoNumericoComMensagemLimpandoCampo(campo,nomeCampo){
		if( !isBrancoOuNulo(campo.value)){
			var filtroNumero = /^[0-9]*$/;
			if(!filtroNumero.test(campo.value)){
				campo.value = '';
...
function comparaDataComDataAtual(data1, operador ){
	...
	/**
	 * Função que põe a mascara de data enquanto 
	 * verifica se o campo é uma data no formato
	 * válido
	 * @autor: Raimundo Martins
	 * */
	function mascaraDataNova(mydata, tecla){
		var codigo;
		if (window.event) {
		...
		if(!isBrancoOuNulo(mydata.value)){
			var filtroNumero = /^[0-9\(/)]*$/;
			if(!filtroNumero.test(mydata.value)){
				mydata.value = '';
				return false;	
			}
			else{
	...
	/**
	 * Função que verifica se o campo
	 * possui uma data no formato válido
	 * @autor: Raimundo Martins
	 * */
	function validarCampoDataComMensagemLimpandoCampo(campo,nomeCampo){
		if( !isBrancoOuNulo(campo.value)){
			var filtroNumero = /^[0-9\(/)]*$/;
			if(!filtroNumero.test(campo.value)){
				campo.value = '';
	...
	/**
	 * Esta função é equivalente a validaEnterDependenciaComMensagem,
	 * entretanto, verifica se o campo não está desabilitado
	 * 
	 * @author Raimundo Martins
	 * @date 30/11/2011
	 * */
	function validaEnterDependenciaComMensagemVerificandoDesabilitado(tecla, caminhoActionReload, nomeCampo, 
			idDependencia, nomeDependencia, nomeCampoMSG){
	      var form = document.forms[0];
		  var indesejaveis = "~{}^%$[]@|`\\<?\#?!;*>\"\'";
		  var teste = true;
	      ...
	      if (codigo == 13) {
	    	  ...
	    	  if(nomeCampo.readOnly == true || nomeCampo.disabled){
	    		  return false;
	    	  }
	...
	/**
	 * Prototição do objeto implicito Array do
	 * Javascript para a criação do método
	 * contais
	 * 
	 * @author Raimundo Martins
	 * @date 12/12/2011
...
/**
 * Metodo responsavel por validar campo texto.
 * O usuario so pode digitar letras e caso coloque alguma acentuação a mesma é retirada.
 * 
 ...
 * @returns {Boolean}
 */
	function campoTexto(evento, campo) {
		 ...
		 if ( tecla == 231 ){ //ç
	    	 ...
	    	 campo.value = campo.value + "C";
		       return false;
		 } else if ( tecla == 224 || tecla == 225 || tecla == 226 || tecla == 227){ //à á || â || ã
			 campo.value = campo.value + "a";
		       return false;
	     } else if ( tecla == 192 || tecla == 193 || tecla == 194 || tecla == 195) { //À || Á || Â || Ã
	    	 campo.value = campo.value + "A";
		       return false;
		  }else if ( tecla == 232 || tecla == 233 || tecla == 234 ){ //è || é || ê
			 campo.value = campo.value + "e";
		       return false;
	     } else if (tecla == 200 ||tecla == 201 || tecla == 202 ) { //È || É || Ê
	    	 campo.value = campo.value + "E";
		       return false;
		 } else if (  tecla == 236 || tecla == 237 || tecla == 238 ) { // ì || í || î
			 campo.value = campo.value + "i";
		       return false;
	     } else if (tecla == 204 || tecla == 205 || tecla == 206) { //Ì || Í || Î
	    	 campo.value = campo.value + "I";
		       return false;
		 } else if ( tecla == 242 || tecla == 243 || tecla == 244 || tecla == 245 ){ //ò || ó || ô || õ
			 campo.value = campo.value + "o";
		       return false;
	     } else if ( tecla == 210 || tecla == 211 || tecla == 212 || tecla == 213) { //Ò || Ó || Ô || Õ
	    	 campo.value = campo.value + "O";
		       return false;
		 } else if ( tecla == 249 || tecla == 250 || tecla == 251){ //ù || ú || û
			 campo.value = campo.value + "u";
		       return false;
	     } else if (tecla == 217 ||tecla == 218 || tecla== 219 ) { // Ù || Ú || Û 
	    	 campo.value = campo.value + "U";
		       return false;
		 } else if ( tecla == 118 ) { // control v
			 campo.value = campo.value + "v";
			 return false;
		 } else if ( tecla == 86 ) { // control V
			 campo.value = campo.value + "V";
			 return false;
		 }else if(  (tecla >= 65 && tecla <= 90) ||(tecla >= 97 && tecla <= 122)  || (tecla.value == 'undefined')){ // letras minusculas || maiusculas
			    return true;
		 } else if ( tecla == 8 || tecla == 32 ){ // backspace ou space
		        return true;
	     }
	...
	/**
	 * Metodo responsavel por validar campo texto Caso o usuario copie e cole algum caracter indevido ao tirar o foco do campo o caracter é removido.
	 * O usuario so pode digitar letras e caso coloque alguma acentuação a mesma é retirada.
	 * 
	 * @author Arthur Carvalho
	 * @date 23/12/2011
	...
	function validaTexto(campo){
	   	var valorAuxiliar = "";
		digitosValidos = "qazwsxedcrfvtgbyhnujmikolpQAZWSXEDCRFVTGBYHNUJMIKOLP " ;
		var numeroInvalido  = false;
	...
	function testarCampoValorZeroSemMsg(valor) {
	...
	function validaCampoSemCaractereEspecialSemMsg(nomeCampoForm) {
		...
		if (!testarCampoValorZeroSemMsg(objetoCampo, nomeCampoMSG)){
			teste = false;
		}
	...
	// valida se data é numérica
	function validaDataNumerica(mydata) {
		var situacao = true;
		if (mydata != "") {
			dia = mydata.value.substring(0,2);
			mes = mydata.value.substring(3,5); 
	...
	//24 lenght 12dec
	function formataCoordenada(campo,tammax){
		var valorAuxiliar = "";
		digitosValidos = "0123456789" ;
		...
		//insere pontos decimais
		for(i = 1;i<=(digitosNumericos/13);i++){
			//alert(i);
			valor = valor.substring(0,digitosNumericos + 1 - 13*i) +
				(i==1?',':'') +
				valor.substring(digitosNumericos + 1 - 13*i);
			}
			//alert(valor);
	...
	/**
	 * Formata campo 'valor' com o numero de casas decimais informado.
	 * Exemplo: 123456789102,123456789123
	 * 
	 * @param campo
	 * @param tamax
	 * @param dec
	 ...
	 *
	 */
	function formataValorGISComDec(campo,tammax, dec){
		var valorAuxiliar = "";
		digitosValidos = "0123456789" ;
		dec = dec + 1;
		valor = campo.value;
		...
		//insere pontos decimais
		for(i = 1;i<=(digitosNumericos/dec);i++){
			//alert(i);
			valor = valor.substring(0,digitosNumericos + 1 - dec*i) +
				(i==1?',':'') +
				valor.substring(digitosNumericos + 1 - dec*i);
			//alert(valor);
		}
	...
	/**
	 * Validar campo para não aceitar caracteres especiais
	 * */
	...
	function mascaraCaracteres(v){
		 v=v.replace(/[^a-zA-Z\s]/g,"")
		 return v
	}

  	function selecionarRadio(campo, event) {
  		evitarEvento(event);
  ...
  // "utilizado" = Local onde sera apresentado a quantidade de caracteres que foi utilizada
  // "limite" = Local onde sera apresentado a quantidade de caracteres que falta para bater o limite
  //evento = evento do teclado
  // EX: return limitarTextArea(limitField, limitNum, utilizado, limite,event)
  // EX: <span id="utilizado">0</span>/<span id="limite">10</span> limitNum = 10
  function limitarTextArea(limitField, limitNum, utilizado, limite,evento) {
	 var total = limitField.value.length;
	 if(window.event){ // Internet Explorer
			tecla = evento.keyCode;
	}else if(evento.which){ // Nestcape ou Mozilla
			tecla = evento.which;
  ...
  //Autor: Jonathan Marcos
  function mascaraDataValidaDiaMesAno(mydata, tecla){ 
	...
	/**
	 * Verifica se o campo mesAnoInicial é maior que o mesAnoFinal
	 * 
	 *  @author Anderson Cabral
	 *  @since 13/02/2014
	 *
	 ***/
	function validaMesAno(mesAnoInicial, mesAnoFinal, nomeCampo){
		if(mesAnoInicial.value != "" && mesAnoFinal.value != ""){
			var mesAnoInicial = mesAnoInicial.value.replace("/", "");
			var mesAnoFinal   = mesAnoFinal.value.replace("/", "");
			
			mesAnoInicial = mesAnoInicial.substr(2, 4) + mesAnoInicial.substr(0, 2);
			mesAnoFinal   = mesAnoFinal.substr(2, 4) + mesAnoFinal.substr(0, 2);
			
			if ( parseInt(mesAnoInicial) > parseInt(mesAnoFinal) ) {
				alert('O '+nomeCampo+' Inicial deve ser Menor que o '+nomeCampo+' Final.');
				form.mesAnoInicial.focus();
				return;
			}
		}else{
			mesAnoInicial.value = "";
	...
	/**
	 * Replica os dados do campo origem no campo destino
	 * 
	 *  @author Anderson Cabral
	 *  @since 13/02/2014
	 *
