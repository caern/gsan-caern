...
import gcom.atualizacaocadastral.bean.AtualizarClienteFoneAPartirDispositivoMovelHelper;
...
/////////////////////////////////////////////////////////////////////////	
		...
		try {
			...
			if (!helper.getNomeNovo().equals(helper.getNomeAnterior()) || 
				...
				if (cliente != null && cliente.getId() != null && cliente.getNome().equals(helper.getNomeNovo())) {
					...
					// 4.1.2.1. Substitui o cliente usuário do imóvel passando a
					// identificação do cliente pesquisado
					// (CLIE_ID) [SB0003 - Substituir Cliente Usuário do
					// Imóvel];
					...
					Integer idClienteInserido = this.inserirNovoCliente(helper, cliente);
					this.verificarDadosRGCliente(helper, idClienteInserido);
					...
					// 4.1.4. Caso não encontre um cliente com o novo CPF/CNPJ
					// informado
					// 4.1.1. Pesquisa um cliente com o novo CPF/CNPJ informado
					// 4.1.4.1. Insere um novo cliente com o CPF/CNPJ e o novo
					// nome informados
					

					// [SB0002 - Inserir Novo Cliente]
					Integer idCliente = this.inserirNovoCliente(helper, clienteAnterior);
					helper.setAtualizarMovimentoClienteFone(true);
					idClienteInseridoAtualizado = idCliente;
					
					// [SB0008 - Verificar Dados do RG do Cliente]
					this.verificarDadosRGCliente(helper, idClienteInseridoAtualizado);
					// 4.1.4.2. Substitui o cliente usuário do imóvel passando a
					// identificação do cliente inserido
					// (CLIE_ID) [SB0003 - Substituir Cliente Usuário do
					// Imóvel];
			...
			Integer idImovel = Integer.valueOf(helper.getMatricula());// this.repositorioAtendimentoPublico.pesquisarIdImovelAssociadoAOrdemVisitaCampo(idOrdemServico);
			...
			Collection<ClienteImovel> colecaoClienteImovel = this
					.getControladorUtil().pesquisar(filtroClienteImovel,
							ClienteImovel.class.getName());
			ClienteImovel clienteImovel = (ClienteImovel) Util
					.retonarObjetoDeColecao(colecaoClienteImovel);
			...
			if (clienteImovel == null || clienteImovel.getId() == null) {
				...
				inserirClienteImovel
						.setIndicadorNomeConta(ConstantesSistema.SIM);
				inserirClienteImovel.setUltimaAlteracao(new Date());

				Integer idClienteImovel = (Integer) this.getControladorUtil()
						.inserir(inserirClienteImovel);
			...
			// 4.2.1. Caso tenha havido alguma alteração nos dados do RG do
			// cliente
			if (validaAlteracaoRGClienteDispositivoMovel(helper, idCliente)) {
				...
				this.atualizarDadosRGClienteAtual(helper, cliente);
			...
			if (!helper.isRemoverTelefone()) {
				...
				AtualizarClienteFoneAPartirDispositivoMovelHelper clienteFoneAnterior = (AtualizarClienteFoneAPartirDispositivoMovelHelper) Util
						.retonarObjetoDeColecao(helper
								.getClienteFoneAnteriorHelper());

				// 4.2.2. Caso tenha havido alguma alteração nos dados do Fone
				// do cliente
				if ((clienteFoneAnterior == null && clienteFoneNovo != null)
						|| (clienteFoneAnterior != null
								&& clienteFoneNovo != null && (!Util
								.stringsIguais(
										clienteFoneAnterior.getDddFone(),
										clienteFoneNovo.getDddFone())
								|| !Util.stringsIguais(
										clienteFoneAnterior.getNumeroFone(),
										clienteFoneNovo.getNumeroFone()) || !Util
									.stringsIguais(
											clienteFoneAnterior.getRamalFone(),
											clienteFoneNovo.getRamalFone())))) {

					// atualiza os dados do Fone do cliente atual [SB0005 -
					// Atualizar Dados do Fone do Cliente Atual]
					this.atualizarDadosFoneClienteAtual(clienteFoneNovo,
							idCliente, helper.getUsuario());
					helper.setAtualizarMovimentoClienteFone(true);
				}
			} else {
				// atualizar dados telefone cliente - atualizacao cadastral
				getControladorCadastro().atualizarDadosFoneClienteAtual(
						helper.getIdClienteAtualizacaoCadastral(), idCliente,
						helper.getImovelAtualizacaoCadastral());
			}
			
			Short indicadorNomeConta = clienteImovel != null ?clienteImovel.getIndicadorNomeConta():ConstantesSistema.NAO;
	...
	public void substituirClienteProprietarioImovel(Integer idImovel,
			Integer idCliente, Short icNomeConta) {

		Integer idProprietarioGsan = null;
		
		try {
			idProprietarioGsan = this.repositorioClienteImovel.retornaClienteProprietario(idImovel).getId();
			//verificar se o cliente proprietario do imovel é diferente do idcliente fornecido
			if(idProprietarioGsan != null && idProprietarioGsan != idCliente){
				//caso seja diferente
				//associar o novo cliente como proprietario
				this.repositorioClienteImovel.desvincularClienteProprietarioImovel(idImovel, idCliente);
				this.repositorioClienteImovel.associarClienteProprietarioImovel(idImovel, idCliente, icNomeConta);
			}else if(idProprietarioGsan == null){
				this.repositorioClienteImovel.associarClienteProprietarioImovel(idImovel, idCliente, icNomeConta);
			}
		} catch (ErroRepositorioException e) {
	...
	public void substituirClienteResponsavelImovel(Integer idImovel,
		...
		try {
			...
			if(idResponsavelGsan != null && idResponsavelGsan != idCliente){
				//caso seja diferente
				//associar o novo cliente como responsavel
				this.repositorioClienteImovel.desvincularClienteResponsavelImovel(idImovel, idCliente);
				this.repositorioClienteImovel.associarClienteResponsavelImovel(idImovel, idCliente, icNomeConta);
	...
	public Integer pesquisarExistenciaNegativacaoParaClienteImovel(
			Integer idClienteAnterior, Integer idImovel){
		Integer result = null;
		try {
	...
	public void atualizarDadosRGClienteAtual(
			AtualizarClienteAPartirDispositivoMovelHelper helper,
			Cliente clientePesquisa) throws ControladorException, ErroRepositorioException {
	...
	public Integer inserirNovoCliente(
		...
		codigoCliente = this.inserirCliente(cliente,
				colecaoFones, colecaoEnderecos, helper.getUsuario(),
...
//		registradorOperacao.registrarOperacao(clienteFone);
	...
	public boolean atualizarClienteAtualizacaoCadastral(ImovelAtualizacaoCadastralDM imovelAtualizacaoCadastral, ClienteAtualizacaoCadastralDM clienteAtualizacaoCadastral, 
		...
		AtualizarClienteAPartirDispositivoMovelHelper helper = this.montarAtualizarClienteAPartirDispositivoMovelHelper(imovel.getId(),clienteAtualizacaoCadastral, clienteImovel);
	...
	public AtualizarClienteAPartirDispositivoMovelHelper montarAtualizarClienteAPartirDispositivoMovelHelper(
		...
		if (clienteAtualizacaoCadastral != null
				&& clienteAtualizacaoCadastral.getId() != null) {

			// MATRICULA
			helper.setMatricula(String.valueOf(idImovel));

			if (clienteAtualizacaoCadastral.getClienteTipo().getId() != null) {
				helper.setIdClienteTipo(String
						.valueOf(clienteAtualizacaoCadastral.getClienteTipo()
								.getId()));
			}
			...
			if (clienteAtualizacaoCadastral.getCpfCnpj() != null
					&& Util.validacaoCPF(clienteAtualizacaoCadastral
							.getCpfCnpj())) {
			...
			// CNPJ
			if (clienteAtualizacaoCadastral.getCpfCnpj() != null
					&& Util.validacaoCNPJ(clienteAtualizacaoCadastral
							.getCpfCnpj())) {
			...
			if (clienteAtualizacaoCadastral.getUnidadeFederacao() != null) {
				helper.setUnidadeFederacaoNovo(String
						.valueOf(clienteAtualizacaoCadastral
								.getUnidadeFederacao().getId()));
			}
			...
			if (clienteAtualizacaoCadastral.getSexo() != null) {
				helper.setIdPessoaSexo(String
						.valueOf(clienteAtualizacaoCadastral.getSexo().getId()));
			}
			...
			if (clienteImovel != null && clienteImovel.getId() != null) {
				...
				if (clienteImovel.getCliente().getUnidadeFederacao() != null
						&& clienteImovel.getCliente().getUnidadeFederacao()
								.getId() != null) {
					helper.setUnidadeFederacaoAnterior(clienteImovel
							.getCliente().getUnidadeFederacao().getId()
							.toString());
				}

				if (clienteImovel.getCliente().getOrgaoExpedidorRg() != null
						&& clienteImovel.getCliente().getOrgaoExpedidorRg()
								.getId() != null) {
				...
				// ID CLIENTE ANTERIOR
				helper.setIdClienteAnterior(clienteImovel.getCliente().getId()
						.toString());

			}

			// Responsável por atualizar somente o telefone com o indicador
			// principal = false ( dispositivos moveis que so trabalham com
			...
			// dispositivos moveis que trabalham com todos os telefones do
			// cliente)
			helper.setRemoverTelefone(true);

			helper.setIdClienteAtualizacaoCadastral(clienteAtualizacaoCadastral
					.getId());
