...
public class RepositorioLocalidadeHBM implements IRepositorioLocalidade {
	...
	/**
	 * Pesquisa os ids das localidades aptas para atualização cadastral.
	 * 
	 * @author André Miranda
	 * @since 10/10/2014
	 */	
	@SuppressWarnings("unchecked")
	public Collection<Integer> pesquisarIdLocalidadeAtualizacaoCadastral() throws ErroRepositorioException {
		...
		try {
			consulta = "SELECT loc.id FROM ImovelAtualizacaoCadastralDM imac " +
						"JOIN imac.localidade loc " +
						"WHERE imac.indicadorAtualizado = :indicadorAtualizacao " +
						"AND imac.indicadorDadosRetorno = :indicadorDadosRetorno " +
						"GROUP BY loc.id ";

			retorno = session.createQuery(consulta)
						.setShort("indicadorAtualizacao", ConstantesSistema.NAO)
						.setShort("indicadorDadosRetorno", ConstantesSistema.SIM)
						.list();
		} catch (HibernateException e) {
			throw new ErroRepositorioException("Erro no Hibernate");
		} finally {
