...
<!--
	...
	function debitosCreditosSelecionados(form,urlTransferencia,complementoUrl){
		...
		if (!retorno){
			...
			if ( form.idImovel.value != null && form.idImovel.value != "" ) {
				...
				var idsGuiaPagamento = obterValorCheckboxMarcadoPorNome("guiaPagamento");
				form.action = '/gsan/gerarRelatorioExtratoDebitoClienteAction.do?guiaPagamento='+idsGuiaPagamento;
			}
	...
	function reloadCliente() {
		var form = document.forms[0];
		form.action = "/gsan/exibirDebitoCreditoDadosSelecaoExtratoAction.do?pesquisarCliente=SIM";
		submeterFormPadrao(form);
	}
  	
  	function limparImovel(codigo) {

    	var form = document.forms[0];

    	if ( (form.idImovel.value != null && form.idImovel.value != "" && codigo == "1") ||
    	    	(form.codigoCliente.value != null && form.codigoCliente.value != "" && codigo =="2") || codigo == "3" ) {
...
function totalizarDebito(objeto){
	...
	} else if ( objeto.name == 'guiaPagamento' ) {
		myString = new String(objeto.alt);
		splitString = myString.split(";");
		valorSelecionado  = splitString[0];
		valorAcrescimosSelecionado  = splitString[1];
...
function pesquisarImovel() {
	...
	if (form.idImovel.value != null && form.idImovel.value != "" ) {
 		alert("Para realizar uma pesquisa é necessário apagar o imóvel atual.")
 	} else if ( form.codigoCliente.value != null && form.codigoCliente.value != "") {
 		alert("Para realizar uma pesquisa é necessário apagar o cliente atual.")
	} else {
...
function hablitaDesabilitaImovelCliente(){
	var form = document.forms[0];
...
<logic:notPresent name="calcularAcrescimoParaGuia">
...
<table width="770" border="0" cellspacing="5" cellpadding="0">
  <tr>
	...
	<td width="625" valign="top" class="centercoltext">
	  ...
	  <table width="100%" border="0">
		<tr>
			...
			<td>
				<html:text property="idImovel" size="10" maxlength="9" tabindex="2"
				onkeypress="validaEnterComMensagem(event, 'exibirDebitoCreditoDadosSelecaoExtratoAction.do?pesquisarImovel=SIM', 'idImovel', 'Matrícula');hablitaDesabilitaImovelCliente();return isCampoNumerico(event);"
				onkeyup="limparInscricaoImovel();hablitaDesabilitaImovelCliente();"/>
				...
				<logic:present name="corImovel">
					<logic:equal name="corImovel" value="exception">
						<html:text property="inscricaoImovel" size="42" readonly="true" style="background-color:#EFEFEF; border:0; color: #ff0000" />
					</logic:equal>

					<logic:notEqual name="corImovel" value="exception">
						<html:text property="inscricaoImovel" size="42" readonly="true" style="background-color:#EFEFEF; border:0; color: #000000" />
					</logic:notEqual>
				</logic:present> 

				<logic:notPresent name="corImovel">
					<logic:empty name="DebitoCreditoDadosSelecaoExtratoActionForm" property="idImovel">
						<html:text property="inscricaoImovel" value="" size="42" readonly="true" style="background-color:#EFEFEF; border:0; color: #ff0000" />
					</logic:empty>
				...
				<a href="javascript:limparImovel(1);"> 
		...
		<tr>
			...
			<td>
	   			<html:text property="nomeClienteUsuarioImovel" size="52" maxlength="50" readonly="true"
		...
		<tr>
			...
			<td>
	   			<html:text property="descricaoLigacaoAguaSituacaoImovel" size="22" maxlength="20" readonly="true"
				style="background-color:#EFEFEF; border:0; color: #000000"/>
				
				<strong>&nbsp;&nbsp;Sit. da Lig. de Esgoto:&nbsp;&nbsp;</strong>
				
				<html:text property="descricaoLigacaoEsgotoSituacaoImovel" size="22" maxlength="20" readonly="true"
				style="background-color:#EFEFEF; border:0; color: #000000"/>
				
			</td>
		</tr>
		
		<logic:present name="calcularAcrescimoParaGuia">
			<tr>
				<td colspan="5">
				<hr>
				</td>
			...
			<tr>
				...
				<td>
					<html:text property="codigoCliente" 
							   ...
							   maxlength="9" 
							   tabindex="2"
							   onkeyup="hablitaDesabilitaImovelCliente();"
							   onkeypress="validaEnterComMensagem(event, 'exibirDebitoCreditoDadosSelecaoExtratoAction.do?pesquisarCliente=SIM', 'codigoCliente', 'codigoCliente');hablitaDesabilitaImovelCliente();return isCampoNumerico(event);"/>
						<a href="javascript:pesquisarCliente();">
							<img width="23" 
								 height="21"
								 src="<bean:message key="caminho.imagens"/>pesquisa.gif" border="0" title="Pesquisar Cliente"
								 /></a>
					...
					<logic:notPresent name="corCliente">
						<logic:empty name="DebitoCreditoDadosSelecaoExtratoActionForm" property="codigoCliente">
							<html:text property="nomeCliente" value="" size="42" readonly="true" style="background-color:#EFEFEF; border:0; color: #ff0000" />
						</logic:empty>
						
						<logic:notEmpty name="DebitoCreditoDadosSelecaoExtratoActionForm" property="codigoCliente">
							<html:text property="nomeCliente" size="42" readonly="true" style="background-color:#EFEFEF; border:0; color: #000000" />
						</logic:notEmpty>
					</logic:notPresent> 
	
					<a href="javascript:limparImovel(2);"> 
						<img src="<bean:message key='caminho.imagens'/>limparcampo.gif"
							alt="Apagar" 
							title="Apagar"
							border="0">
		...
		<td>
				...
				<td>
					...
					<logic:iterate name="colecaoConta" id="conta" type="ContaValoresHelper">
						...
						<td align="center" width="5%">
							...
							<logic:equal name="conta" property="indicadorGerarLink" value="<%=ConstantesSistema.SIM.toString()%>">
							...
							<logic:equal name="conta" property="indicadorGerarLink" value="<%=ConstantesSistema.NAO.toString()%>">
								
								<INPUT TYPE="checkbox" NAME="contaNaoGerar" disabled="disabled" value="<%="" + conta.getConta().getId().intValue() %>" 
								alt="<%="" + Util.formatarMoedaReal(conta.getValorTotalConta()).trim()%>
								<%=";" + Util.formatarMoedaReal(conta.getValorTotalContaValoresParcelamento()).trim()%>"
								>	
			...
			<tr>
				<td>
					...
					<table width="100%" cellpadding="0" cellspacing="0">
					<tr bgcolor="#79bbfd">
						<td colspan="7" height="20"><strong>Guias de Pagamento</strong></td>
					</tr>
					<tr bgcolor="#90c7fc">
			...
			<%BigDecimal valorTotalAcrescimoGuiaPagamento = new BigDecimal("0.00");%>
			...
			<tr>
				<td>
					...
					<logic:iterate name="colecaoGuiaPagamento" id="guiaPagamentoValoresHelper" type="GuiaPagamentoValoresHelper">
						...
						<%valorTotalAcrescimoGuiaPagamento = valorTotalAcrescimoGuiaPagamento.add(guiaPagamentoValoresHelper.getValorAcrescimosImpontualidade()); %>
						...
						<td align="center" width="5%">
							...
							<%=";" + Util.formatarMoedaReal(guiaPagamentoValoresHelper.getValorAcrescimosImpontualidade()).trim()%>""
							onclick="totalizarDebito(this);">
						...
						<td width="15%">
							<div align="right">
									...
									<a title="<%="Multa: " + Util.formatarMoedaReal(guiaPagamentoValoresHelper.getValorMulta()).trim() + 
									" Juros de Mora: " + Util.formatarMoedaReal(guiaPagamentoValoresHelper.getValorJurosMora()).trim() +
									" Atualização Monetária: " + Util.formatarMoedaReal(guiaPagamentoValoresHelper.getValorAtualizacaoMonetaria()).trim()%>">
						...
						<td width="15%">
							<div align="right">
								<strong>				
								<%="" + Util.formatarMoedaReal(valorTotalAcrescimoGuiaPagamento).trim()%>
								</strong>				
							</div>
						</td>
					...
					<input type="hidden" name="valorTotalAcrescimoGuiaPagamento" ID="valorTotalAcrescimoGuiaPagamento" value="<%="" + valorTotalAcrescimoGuiaPagamento %>">
				</td>
			</tr>
	...
	<%valorTotalDebitosAtualizado = valorTotalDebitosAtualizado.add(valorTotalAcrescimoGuiaPagamento); %>
	...
	<tr>
		<td>
			...
			onclick="javascript:limparImovel(3);">
			<input name="Button" type="button" class="bottonRightCol"
