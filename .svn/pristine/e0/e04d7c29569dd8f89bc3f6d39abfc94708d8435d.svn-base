...
function consultarImovelAjax(){
	...
	if(form.matricula != null && form.matricula.value != ""){
		...
		$.ajax({
				   type: "POST",
				   url: "consultarImovelAjaxAction.do?idImovel="+form.matricula.value,
				   data: "",
				   success: function(msg){
					   if ( msg != null && msg != "" ) {
						   alert(msg + " Não é possivel altera-lo.");
						   msg = "";
						}
				   }
	 	});
...
<body leftmargin="5" topmargin="5" onload="javascript:setarFoco('${requestScope.nomeCampo}');consultarImovelAjax();">
<div id="formDiv">
