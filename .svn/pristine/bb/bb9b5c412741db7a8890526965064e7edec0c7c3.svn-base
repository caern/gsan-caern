...
import gcom.batch.arrecadacao.TarefaBatchBaixaAutomaticaPagamentosNaoClassificados;
...
import gcom.batch.arrecadacao.TarefaBatchGerarArquivoCarteira17;
...
import gcom.batch.atualizacaocadastral.TarefaBatchAtualizarDadosCadastraisImovelRecadastramento;
import gcom.batch.atualizacaocadastral.TarefaBatchGerarRoteiroDispositivoMovel;
...
import gcom.batch.cadastro.TarefaBatchEnviarEmailCobrancaFaturamento;
...
import gcom.batch.cadastro.TarefaBatchRetornoSMSEmLote;
import gcom.batch.cadastro.TarefaBatchSuspenderImovelEmProgramaEspecial;
import gcom.batch.cadastro.TarefaBatchTransmitirSMSEmLoteCobrancaFaturamento;
...
import gcom.batch.cobranca.TarefaBatchGerarDividaAtiva;
...
import gcom.batch.cobranca.TarefaBatchGerarResumoDividaAtivaAnual;
...
import gcom.batch.cobranca.TarefaBatchPrepararDadosSMSEmailAcaoCobranca;
import gcom.batch.cobranca.TarefaBatchPrepararDadosSMSEmailCobrancaConta;
import gcom.batch.cobranca.TarefaBatchProcessarAmortizacaoDividaAtivaArrecadacao;
import gcom.batch.cobranca.TarefaBatchProcessarAmortizacaoDividaAtivaFaturamento;
import gcom.batch.cobranca.TarefaBatchProcessarEncerramentoOSAcaoCobranca;
import gcom.batch.cobranca.TarefaBatchRetirarContasRevisaoExtratoEntradaParcelamento;
...
import gcom.batch.faturamento.TarefaBatchAutomatizarCargaPentaho;
...
import gcom.batch.faturamento.TarefaBatchGerarContas;
import gcom.batch.faturamento.TarefaBatchGerarCreditoARealizarContratoDemandaImovelResidencial;
import gcom.batch.faturamento.TarefaBatchGerarCreditoARealizarPagamentoDuplicidadeVivaAgua;
...
import gcom.batch.faturamento.TarefaBatchGerarFaturaProgramaEspecial;
...
import gcom.batch.faturamento.TarefaBatchReceitaLiquidaIndiretaDeAguaEEsgotoMDB;
...
import gcom.batch.faturamento.TarefaBatchRetificarContasCaern;
...
import gcom.batch.gerencial.TarefaBatchExecutarCargaArrecadacaoGerencialAno;
...
public class ControladorBatchSEJB implements SessionBean {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				try {
					...
					case Funcionalidade.RETIFICAR_CONTAS_INDEVIDAS_CAERN:
						TarefaBatchRetificarContasCaern retificarContasCaern = new TarefaBatchRetificarContasCaern(	processoIniciado.getUsuario(),
																													funcionalidadeIniciada.getId());

						Collection<Imovel> imoveis = getControladorFaturamento().recuperarImoveisRetificacaoCAERN();

						retificarContasCaern.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, imoveis);

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(retificarContasCaern));
					...
					case Funcionalidade.GERAR_RESUMO_DIARIO_NEGATIVACAO:
						TarefaBatchGerarResumoDiarioNegativacao gerarResumoDiarioNegativacao = new TarefaBatchGerarResumoDiarioNegativacao(	processoIniciado.getUsuario(),
																																			funcionalidadeIniciada.getId());

						Collection colSetores = getControladorSpcSerasa().consultarSetorParaGerarResumoDiarioNegativacao();
						...
						// 2.0 O sistema exclui o resumo di�rio das
						// negativa��es
						...
						gerarResumoDiarioNegativacao.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colSetores);
					...
					case Funcionalidade.EXECUTAR_COMANDO_DE_NEGATIVACAO:
						...
						TarefaBatchExecutarComandoNegativacao executarComandoNegativacao = new TarefaBatchExecutarComandoNegativacao(	processoIniciado.getUsuario(),
																																		funcionalidadeIniciada.getId());
						...
						// "H� movimento de exclus�o incompleto. Reinicie o
						// step
						// 2 do processo 135 e ap�s a conclus�o
						...
						NegativacaoComando nComando = getControladorSpcSerasa().consultarNegativacaoComandadoParaExecutar();

						if (nComando != null && !nComando.equals("")) {

							// 1.
							NegativacaoCriterio nCriterio = getControladorSpcSerasa().pesquisarNegativacaoCriterio(nComando.getId());

							// 2.0
							Negativador neg = nComando.getNegativador();

							// 3.0
							NegativadorContrato nContrato = getControladorSpcSerasa().consultarNegativadorContratoVigente(neg.getId());

							Collection colecaoParametroNegCrit = (Collection) getControladorSpcSerasa().pesquisarParametroNegativacaoCriterio(nCriterio.getId());
							...
							// rotas = null;
							// }else
							if (nComando.getIndicadorComandoCriterio() == ConstantesSistema.TIPO_COMANDO_POR_CRITERIO) {

								if (nComando.getComandoSimulacao() != null && !nComando.getComandoSimulacao().equals("")) {
									rotas = getControladorSpcSerasa().pesquisarRotasImoveisComandoSimulacao(nComando.getComandoSimulacao().getId());
								} else if (nCriterio.getCliente() != null) {
									rotas = getControladorSpcSerasa().pesquisarRotasImoveis();
								} else {

									if (colecaoParametroNegCrit != null && !colecaoParametroNegCrit.isEmpty()) {

										for (Iterator iterColecaoParametroNegCrit = colecaoParametroNegCrit.iterator(); iterColecaoParametroNegCrit.hasNext();) {

											parametroNegCrit = (Object[]) iterColecaoParametroNegCrit.next();

											if (parametroNegCrit[0] != null) {
												// Condi��o 1
												rotas = getControladorSpcSerasa().pesquisarRotasPorCobrancaGrupoParaNegativacao(nCriterio);
												break;
											} else if (parametroNegCrit[1] != null) {
												// Condi��o 2
												rotas = getControladorSpcSerasa().pesquisarRotasPorGerenciaRegionalParaNegativacao(nCriterio);
												break;
											} else if (parametroNegCrit[2] != null) {
												// Condi��o 3
												rotas = getControladorSpcSerasa().pesquisarRotasPorUnidadeNegocioParaNegativacao(nCriterio);
												break;
											} else if (parametroNegCrit[3] != null) {
												// Condi��o 4
												rotas = getControladorSpcSerasa().pesquisarRotasPorLocalidadeParaNegativacao(nCriterio);
												break;
											} else if (parametroNegCrit[4] != null && parametroNegCrit[5] != null) {
												// Condi��o 5 ou 6
												rotas = getControladorSpcSerasa().pesquisarRotasPorLocalidadesParaNegativacao(nCriterio);
												break;
											} else {
												// default
												rotas = getControladorSpcSerasa().pesquisarRotasImoveis();
							...
							executarComandoNegativacao.addParametro("nCriterio", nCriterio);

							executarComandoNegativacao.addParametro("neg", neg);

							executarComandoNegativacao.addParametro("nComando", nComando);

							executarComandoNegativacao.addParametro("nContrato", nContrato);

							if (nComando.getIndicadorSimulacao() != NegativacaoComando.SIMULACAO
									&& nComando.getIndicadorComandoCriterio() == ConstantesSistema.TIPO_COMANDO_POR_CRITERIO) {

								int saEnvio = nContrato.getNumeroSequencialEnvio() + 1;
								// 1.2
								// gerando o movimento
								Integer idNegativacaoMovimento = getControladorSpcSerasa().geraRegistroNegativadorMovimento(neg.getId(), saEnvio, nComando.getId());

								// 1.3
								// [SB0008] - Gerar Registro do tipo Hearder
								getControladorSpcSerasa().gerarRegistroDeInclusaoTipoHeader(ConstantesSistema.TIPO_COMANDO_POR_CRITERIO, 1, neg, nContrato, nComando, nCriterio, idNegativacaoMovimento);
							}
					...
					case Funcionalidade.ATUALIZAR_LIGACAO_AGUA_LIGADO_ANALISE_PARA_LIGADO:
						TarefaBatchAtualizarLigacaoAguaLigadoAnaliseParaLigado atualizarLigacaoAguaLigadoAnaliseParaLigado = new TarefaBatchAtualizarLigacaoAguaLigadoAnaliseParaLigado(processoIniciado.getUsuario(),
																																														funcionalidadeIniciada.getId());
					...
					case Funcionalidade.ATUALIZAR_NUMERO_EXECUCAO_RESUMO_NEGATIVACAO:
						...
						TarefaBatchAtualizarNumeroExecucaoResumoNegativacao atualizarNumeroExecucaoResumoNegativacao = new TarefaBatchAtualizarNumeroExecucaoResumoNegativacao(	processoIniciado.getUsuario(),
																																												funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_MOVIMENTO_EXCLUSAO_NEGATIVACAO:
						...
						TarefaBatchGerarMovimentoExclusaoNegativacao gerarMovimentoExclusaoNegativacao = new TarefaBatchGerarMovimentoExclusaoNegativacao(	processoIniciado.getUsuario(),
																																							funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_MOVIMENTO_RETORNO_NEGATIVACAO:
						...
						TarefaBatchGerarMovimentoRetornoNegativacao gerarMovimentoRetornoNegativacao = new TarefaBatchGerarMovimentoRetornoNegativacao(	processoIniciado.getUsuario(),
																																						funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_DADOS_DIARIOS_ARRECADACAO:
						...
						Collection<Integer> colecaoIdsLocalidadesGerarDadosDiariosArrecadacao = getControladorArrecadacao().pesquisarIdsLocalidadeComPagamentosOuDevolucoes();
					...
					case Funcionalidade.GERAR_DEBITOS_A_COBRAR_ACRESCIMOS_IMPONTUALIDADE:
						TarefaBatchGerarDebitosACobrarAcrescimosImpontualidade impontualidade = new TarefaBatchGerarDebitosACobrarAcrescimosImpontualidade(	processoIniciado.getUsuario(),
																																							funcionalidadeIniciada.getId());
					...
					case Funcionalidade.CLASSIFICAR_PAGAMENTOS_DEVOLUCOES:
						TarefaBatchClassificarPagamentosDevolucoes dadosClassificarPagamentosDevolucoes = new TarefaBatchClassificarPagamentosDevolucoes(	processoIniciado.getUsuario(),
																																							funcionalidadeIniciada.getId());

						Collection<Integer> colecaoIdsLocalidadesClassificarPagamentosDevolucoes = getControladorArrecadacao().pesquisarIdsLocalidadeComPagamentosOuDevolucoes();

						// Seta os parametros para rodar a funcionalidade
						dadosClassificarPagamentosDevolucoes.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesClassificarPagamentosDevolucoes);
					...
					case Funcionalidade.ENCERRAR_ARRECADACAO_MES:
						...
						// Seta os parametros para rodar a funcionalidade
						dadosEncerrarArrecadacaoMes.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesEncerrarArrecadacaoMes);
					...
					case Funcionalidade.GERAR_HISTORICO_PARA_ENCERRAR_ARRECADACAO_MES:
						...
						TarefaBatchGerarHistoricoParaEncerrarArrecadacaoMes dadosGerarHistoricoEncerrarArrecadacaoMes = new TarefaBatchGerarHistoricoParaEncerrarArrecadacaoMes(processoIniciado.getUsuario(),
																																												funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_HISTORICO_CONTA:
						 ...
						 * ARRECADACAO do mes
						 */
						Collection<Integer> colecaoIdsSetoresEncerrarArrecadacaoMes = getControladorArrecadacao().pesquisarIdsSetoresComPagamentosOuDevolucoes();
					...
					case Funcionalidade.ENCERRAR_FATURAMENTO_MES:
						...
						Collection<Integer> colecaoIdsLocalidadesEncerrarFaturamentoMes = getControladorFaturamento().pesquisarIdsLocalidadeParaEncerrarFaturamento();

						// Seta os parametros para rodar a funcionalidade
						dadosEncerrarFaturamentoMes.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesEncerrarFaturamentoMes);
					...
					case Funcionalidade.GERAR_HISTORICO_PARA_ENCERRAR_FATURAMENTO_MES:
						TarefaBatchGerarHistoricoParaEncerrarFaturamentoMes dadosGerarHistoricoParaEncerrarFaturamentoMes = new TarefaBatchGerarHistoricoParaEncerrarFaturamentoMes(processoIniciado.getUsuario(),
																																													funcionalidadeIniciada.getId());
						...
						Collection<Integer> colecaoIdsSetoresEncerrarFaturamentoMes = getControladorFaturamento().pesquisarIdsSetorComercial();

						dadosGerarHistoricoParaEncerrarFaturamentoMes.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsSetoresEncerrarFaturamentoMes);
					...
					case Funcionalidade.GERAR_RESUMO_LIGACOES_ECONOMIAS:
						TarefaBatchGerarResumoLigacoesEconomias gerarResumoLigacoesEconomias = new TarefaBatchGerarResumoLigacoesEconomias(	processoIniciado.getUsuario(),
																																			funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_REGISTRO_ATENDIMENTO:
						TarefaBatchGerarResumoRegistroAtendimento gerarResumoRegistroAtendimento = new TarefaBatchGerarResumoRegistroAtendimento(	processoIniciado.getUsuario(),
																																					funcionalidadeIniciada.getId());
						...
						gerarResumoRegistroAtendimento.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesGerarResumoRegistroAtendimentoMes);
					...
					case Funcionalidade.GERAR_DEBITO_A_COBRAR_CONTRATO_DEMANDA_RESIDENCIAL:
						...
						TarefaBatchGerarDebitoACobrarContratoDemandaImovelResidencial dadosDebitoACobrarContratoDemandaImovelResidencial = new TarefaBatchGerarDebitoACobrarContratoDemandaImovelResidencial(	processoIniciado.getUsuario(),
																																																				funcionalidadeIniciada.getId());

						FiltroLocalidade filtroLocalidadeDebitoACobrarContrato = new FiltroLocalidade();

						Collection<Localidade> colecaoIdsLocalidadesGerarDebitoACobrarContrato = getControladorUtil().pesquisar(filtroLocalidadeDebitoACobrarContrato, Localidade.class.getName());

						dadosDebitoACobrarContratoDemandaImovelResidencial.addParametro("anoMesArrecadacao", sistemaParametros.getAnoMesArrecadacao());

						// Seta os parametros para rodar a funcionalidade
						dadosDebitoACobrarContratoDemandaImovelResidencial.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesGerarDebitoACobrarContrato);

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(dadosDebitoACobrarContratoDemandaImovelResidencial));
					...
					case Funcionalidade.GERAR_LANCAMENTOS_CONTABEIS_ARRECADACAO:
						...
						TarefaBatchGerarLancamentosContabeisArrecadacao dadosGerarLancamentosContabeisArrecadacao = new TarefaBatchGerarLancamentosContabeisArrecadacao(processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());

						FiltroLocalidade filtroLocalidadeLancamentoContabeisArrecadacao = new FiltroLocalidade();

						Collection<Localidade> colecaoIdsLocalidadesGerarLancamentosContabeisArrecadacao = getControladorUtil().pesquisar(filtroLocalidadeLancamentoContabeisArrecadacao, Localidade.class.getName());

						dadosGerarLancamentosContabeisArrecadacao.addParametro("anoMesArrecadacao", sistemaParametros.getAnoMesArrecadacao());

						// Seta os parametros para rodar a funcionalidade
						dadosGerarLancamentosContabeisArrecadacao.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesGerarLancamentosContabeisArrecadacao);
					...
					case Funcionalidade.GERAR_LANCAMENTOS_CONTABEIS_AVISO_BANCARIO:
						...
						TarefaBatchGerarLancamentosContabeisAvisosBancarios tarefaBatch = new TarefaBatchGerarLancamentosContabeisAvisosBancarios(	processoIniciado.getUsuario(),
																																					funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_SITUACAO_ESPECIAL_FATURAMENTO:
						TarefaBatchGerarResumoSituacaoEspecialFaturamento gerarResumoSituacaoEspecialFaturamento = new TarefaBatchGerarResumoSituacaoEspecialFaturamento(	processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());
						...
						gerarResumoSituacaoEspecialFaturamento.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoLocalidadeFaturamento);
					...
					case Funcionalidade.GERAR_RESUMO_HISTOGRAMA_AGUA_ESGOTO:
						TarefaBatchGerarResumoHistogramaAguaEsgoto gerarResumoAguaEsgoto = new TarefaBatchGerarResumoHistogramaAguaEsgoto(	processoIniciado.getUsuario(),
																																			funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_ACOES_COBRANCA_CRONOGRAMA:
						TarefaBatchGerarResumoAcoesCobrancaCronograma dadosGerarResumoAcoesCobrancaCronograma = new TarefaBatchGerarResumoAcoesCobrancaCronograma(	processoIniciado.getUsuario(),
																																									funcionalidadeIniciada.getId());
						...
						if (colecaoCobrancaGrupoCronogramaMes != null) {
							...
							while (iteratorColecaoCobrancaGrupoCronogramaMes.hasNext()) {
								...
								if (colecaoCobrancaAcaoCronograma != null && !colecaoCobrancaAcaoCronograma.isEmpty()) {
									...
									while (iteratorColecaoCobrancaAcaoCronograma.hasNext()) {
										...
										// tabela
										// COBRANCA_ATIVIDADE
										colecaoCobrancaAtividadeAcaoCronogramaEmitir = getControladorCobranca().pesquisarDataRelizacaoCobrancaAtividadeAcaoConograma(idCobrancaAcaoCronograma, CobrancaAtividade.EMITIR);
										...
										colecaoCobrancaAtividadeAcaoCronogramaEncerrar = getControladorCobranca().pesquisarDataRelizacaoCobrancaAtividadeAcaoConograma(idCobrancaAcaoCronograma, CobrancaAtividade.ENCERRAR);
							...
							dadosGerarResumoAcoesCobrancaCronograma.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoDadosCobrancaAcaoAtividadeCronograma);
					...
					case Funcionalidade.GERAR_RESUMO_ACOES_COBRANCA_CRONOGRAMA_ENCERRAR_OS:
						TarefaBatchGerarResumoAcoesCobrancaCronogramaEncerrarOS dadosGerarResumoAcoesCobrancaCronogramaEncerrarOS = new TarefaBatchGerarResumoAcoesCobrancaCronogramaEncerrarOS(processoIniciado.getUsuario(),
																																																funcionalidadeIniciada.getId());
						...
						if (colecaoCobrancaGrupoCronogramaMes != null) {
							...
							dadosGerarResumoAcoesCobrancaCronogramaEncerrarOS.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoDadosCobrancaAcaoAtividadeCronograma);
					...
					case Funcionalidade.GERAR_RESUMO_ACOES_COBRANCA_EVENTUAL:
						TarefaBatchGerarResumoAcoesCobrancaEventual tarefaBatchGerarResumoAcoesCobrancaEventual = new TarefaBatchGerarResumoAcoesCobrancaEventual(	processoIniciado.getUsuario(),
																																									funcionalidadeIniciada.getId());
						...
						if (colecaoAcaoCobrancaEventual != null && !colecaoAcaoCobrancaEventual.isEmpty()) {
							tarefaBatchGerarResumoAcoesCobrancaEventual.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoAcaoCobrancaEventual);
					...
					case Funcionalidade.INSERIR_RESUMO_ACOES_COBRANCA_CRONOGRAMA:
						TarefaBatchInserirResumoAcoesCobrancaCronograma dadosInserirResumoAcoesCobrancaCronograma = new TarefaBatchInserirResumoAcoesCobrancaCronograma(processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());
						...
						if (colecaoCobrancaGrupoCronogramaMesInserir != null) {
							...
							dadosInserirResumoAcoesCobrancaCronograma.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoDadosCobrancaAcaoAtividadeCronogramaParaInserir);
					...
					case Funcionalidade.INSERIR_RESUMO_ACOES_COBRANCA_EVENTUAL:
						TarefaBatchInserirResumoAcoesCobrancaEventual tarefaBatchInserirResumoAcoesCobrancaEventual = new TarefaBatchInserirResumoAcoesCobrancaEventual(processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());
						...
						if (colecaoAcaoCobrancaEventualParaInserir != null
							...
							tarefaBatchInserirResumoAcoesCobrancaEventual.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoAcaoCobrancaEventualParaInserir);
					...
					case Funcionalidade.GERAR_RESUMO_PENDENCIA:
						...
						Collection<SetorComercial> colecaoSetorPendencia = getControladorUtil().pesquisar(novoFiltroSetorComercial, SetorComercial.class.getName());
					...
					case Funcionalidade.GERAR_GUIA_PAGAMENTO_POR_CLIENTE_RESUMO_PENDENCIA:
						TarefaBatchGerarGuiaPagamentoPorClienteResumoPendencia dados = new TarefaBatchGerarGuiaPagamentoPorClienteResumoPendencia(	processoIniciado.getUsuario(),
																																					funcionalidadeIniciada.getId());

						FiltroLocalidade filtroLocalidadeResumoPendencia = new FiltroLocalidade();
						Collection<Localidade> colecaoLocalidadePendencia = getControladorUtil().pesquisar(filtroLocalidadeResumoPendencia, Localidade.class.getName());
					...
					case Funcionalidade.GERAR_RESUMO_ANORMALIDADES:
						TarefaBatchGerarResumoAnormalidades tarefaBatchGerarResumoAnormalidades = new TarefaBatchGerarResumoAnormalidades(	processoIniciado.getUsuario(),
																																			funcionalidadeIniciada.getId());

						Collection<Integer> colecaoIdsLocalidadesAnormalidades = getControladorFaturamento().pesquisarIdsLocalidadeParaGerarResumoLigacoesEconomias();

						// Seta os parametros para rodar a funcionalidade
						tarefaBatchGerarResumoAnormalidades.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesAnormalidades);
					...
					case Funcionalidade.GERAR_RESUMO_SITUACAO_ESPECIAL_COBRANCA:
						TarefaBatchGerarResumoSituacaoEspecialCobranca tarefaBatchGerarResumoSituacaoEspecialCobranca = new TarefaBatchGerarResumoSituacaoEspecialCobranca(	processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());

						Collection<Integer> colecaoIdsLocalidadesCobranca = getControladorFaturamento().pesquisarIdsLocalidadeParaGerarResumoLigacoesEconomias();

						// Seta os parametros para rodar a funcionalidade
						tarefaBatchGerarResumoSituacaoEspecialCobranca.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesCobranca);
					...
					case Funcionalidade.GERAR_FATURA_CLIENTE_RESPONSAVEL:
						TarefaBatchGerarFaturaClienteResponsavel dadosGerarFaturaClienteResponsavel = new TarefaBatchGerarFaturaClienteResponsavel(	processoIniciado.getUsuario(),
																																					funcionalidadeIniciada.getId());
					...
					case Funcionalidade.DESFAZER_PARCELAMENTO_POR_ENTRADA_NAO_PAGA:
						TarefaBatchDesfazerParcelamentoPorEntradaNaoPaga dadosDesfazerParcelamentoPorEntradaNaoPaga = new TarefaBatchDesfazerParcelamentoPorEntradaNaoPaga(	processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_INSTALACOES_HIDROMETROS:
						TarefaBatchGerarResumoInstalacoesHidrometros dadosGerarResumoInstalacoesHidrometros = new TarefaBatchGerarResumoInstalacoesHidrometros(	processoIniciado.getUsuario(),
																																								funcionalidadeIniciada.getId());
						 ...
						 */
						Collection<Integer> colecaoIdsSetoresComercial = getControladorGerencialMicromedicao().pesquisarIdsSetorComercialParaGerarResumoInstalacaoHidrometro(anoMesFaturamentoSistemaParametro);
					...
					case Funcionalidade.GERAR_RESUMO_LEITURA_ANORMALIDADE:
						TarefaBatchGerarResumoLeituraAnormalidade dadosGerarResumoLeituraAnormalidade = new TarefaBatchGerarResumoLeituraAnormalidade(	processoIniciado.getUsuario(),
																																						funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_ARRECADACAO:
						...
						Collection<Localidade> colLocalidadeResumoArrecadacao = getControladorUtil().pesquisar(filtroLocalidadeResumoArrecadacao, Localidade.class.getName());
					...
					case Funcionalidade.GERAR_RESUMO_PARCELAMENTO:
						TarefaBatchGerarResumoParcelamento dadosGerarResumoParcelamento = new TarefaBatchGerarResumoParcelamento(	processoIniciado.getUsuario(),
																																	funcionalidadeIniciada.getId());
						// 2222
						Collection<Integer> colecaoLocalidadesParcelamento = getControladorFaturamento().pesquisarIdsLocalidadeParaGerarResumoLigacoesEconomias();
					...
					case Funcionalidade.GERAR_RESUMO_FATURAMENTO_AGUA_ESGOTO:
						TarefaBatchGerarResumoFaturamentoAguaEsgoto dadosGerarResumoFaturamentoAguaEsgoto = new TarefaBatchGerarResumoFaturamentoAguaEsgoto(processoIniciado.getUsuario(),
																																							funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_REFATURAMENTO:
						TarefaBatchGerarResumoReFaturamento dadosGerarResumoReFaturamento = new TarefaBatchGerarResumoReFaturamento(processoIniciado.getUsuario(),
																																	funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_REFATURAMENTO_OLAP:
						TarefaBatchGerarResumoReFaturamentoOlap dadosGerarResumoReFaturamentoOlap = new TarefaBatchGerarResumoReFaturamentoOlap(processoIniciado.getUsuario(),
																																				funcionalidadeIniciada.getId());

						FiltroSetorComercial filtroReFaturamentoOlap = new FiltroSetorComercial();

						Collection<SetorComercial> colSetorReFaturamentoOlap = getControladorUtil().pesquisar(filtroReFaturamentoOlap, SetorComercial.class.getName());
					...
					case Funcionalidade.GERAR_LANCAMENTOS_CONTABEIS_FATURAMENTO:
						...
						TarefaBatchGerarLancamentosContabeisFaturamento dadosGerarLancamentosContabeisFaturamento = new TarefaBatchGerarLancamentosContabeisFaturamento(processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());

						FiltroLocalidade filtroLocalidadeLancamentoContabeisFaturamento = new FiltroLocalidade();

						Collection<Localidade> colecaoIdsLocalidadesGerarLancamentosContabeisFaturamento = getControladorUtil().pesquisar(filtroLocalidadeLancamentoContabeisFaturamento, Localidade.class.getName());

						dadosGerarLancamentosContabeisFaturamento.addParametro("anoMesFaturamento", sistemaParametros.getAnoMesFaturamento());

						// Seta os parametros para rodar a funcionalidade
						dadosGerarLancamentosContabeisFaturamento.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesGerarLancamentosContabeisFaturamento);
					...
					case Funcionalidade.GERAR_RESUMO_DEVEDORES_DUVIDOSOS:
						TarefaBatchGerarResumoDevedoresDuvidosos dadosGerarResumoDevedoresDuvidosos = new TarefaBatchGerarResumoDevedoresDuvidosos(	processoIniciado.getUsuario(),
																																					funcionalidadeIniciada.getId());

						Collection<Integer> colecaoIdsLocalidadesGerarResumoDevedoresDuvidosos = getControladorFinanceiro().pesquisarIdsLocalidadesParaGerarResumoDevedoresDuvidosos(sistemaParametros.getAnoMesFaturamento());

						dadosGerarResumoDevedoresDuvidosos.addParametro("anoMesFaturamento", sistemaParametros.getAnoMesFaturamento());

						// Seta os parametros para rodar a funcionalidade
						dadosGerarResumoDevedoresDuvidosos.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesGerarResumoDevedoresDuvidosos);
					...
					case Funcionalidade.GERAR_RESUMO_METAS:
						...
						if (colecaoResumoMetas != null && !colecaoResumoMetas.isEmpty()) {
							throw new ControladorException(	"atencao.dados.existente.resumo.metas",
															null,
					...
					case Funcionalidade.GERAR_RESUMO_METAS_ACUMULADO:
						TarefaBatchGerarResumoMetasAcumulado tarefaBatchGerarResumoMetasAcumulado = new TarefaBatchGerarResumoMetasAcumulado(	processoIniciado.getUsuario(),
																																				funcionalidadeIniciada.getId());
					...
					case Funcionalidade.EMITIR_CONTAS_ORGAO_PUBLICO:
						TarefaBatchEmitirContasOrgaoPublico tarefaBatchEmitirContasOrgaoPublico = new TarefaBatchEmitirContasOrgaoPublico(	processoIniciado.getUsuario(),
																																			funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_CONTAS_A_RECEBER_CONTABIL:
						TarefaBatchGerarContasAReceberContabil gerarContaAReceberContabil = new TarefaBatchGerarContasAReceberContabil(	processoIniciado.getUsuario(),
																																		funcionalidadeIniciada.getId());
					...
					case Funcionalidade.ATUALIZA_QUANTIDADE_PARCELA_PAGA_CONSECUTIVA_PARCELA_BONUS:
						TarefaBatchAtualizaQuantidadeParcelaPagaConsecutivaParcelaBonus dadosAtualizarParcela = new TarefaBatchAtualizaQuantidadeParcelaPagaConsecutivaParcelaBonus(processoIniciado.getUsuario(),
																																													funcionalidadeIniciada.getId());
					...
					case Funcionalidade.EXECUTAR_COMANDO_DE_ENCERRAMENTO_RA:
						TarefaBatchExecutarComandoEncerramentoRA executarComandoEncerramentoRA = new TarefaBatchExecutarComandoEncerramentoRA(	processoIniciado.getUsuario(),
																																				funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_VALOR_VOLUMES_CONSUMIDOS_NAO_FATURADOS:
						TarefaBatchGerarValorVolumesConsumidosNaoFaturados gerarValorVolumesConsumidosNaoFaturados = new TarefaBatchGerarValorVolumesConsumidosNaoFaturados(processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_LANCAMENTOS_CONTABEIS_VOLUMES_CONSUMIDOS_NAO_FATURADOS:
						TarefaBatchGerarLancamentosContabeisVolumesConsumidosNaoFaturados dadosGerarLancamentosContabeisVolumesConsumidos = new TarefaBatchGerarLancamentosContabeisVolumesConsumidosNaoFaturados(	processoIniciado.getUsuario(),
																																																					funcionalidadeIniciada.getId());

						Collection<Integer> colecaoIdsLocalidade = getControladorLocalidade().pesquisarIdsLocalidadesImoveis();

						// Seta os parametros para rodar a funcionalidade
						dadosGerarLancamentosContabeisVolumesConsumidos.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidade);

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(dadosGerarLancamentosContabeisVolumesConsumidos));
					...
					case Funcionalidade.GERAR_RESUMO_INDICADORES_COMERCIALIZACAO:
						...
						TarefaBatchGerarResumoIndicadoresComercializacao gerarResumoIndicadoresComercializacao = new TarefaBatchGerarResumoIndicadoresComercializacao(	processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_INDICADORES_MICROMEDICAO:
						...
						TarefaBatchGerarResumoIndicadoresMicromedicao gerarResumoIndicadoresMicromedicao = new TarefaBatchGerarResumoIndicadoresMicromedicao(	processoIniciado.getUsuario(),
																																								funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_INDICADORES_FATURAMENTO:
						...
						TarefaBatchGerarResumoIndicadoresFaturamento gerarResumoIndicadoresFaturamento = new TarefaBatchGerarResumoIndicadoresFaturamento(	processoIniciado.getUsuario(),
																																							funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_INDICADORES_COBRANCA:
						...
						TarefaBatchGerarResumoIndicadoresCobranca gerarResumoIndicadoresCobranca = new TarefaBatchGerarResumoIndicadoresCobranca(	processoIniciado.getUsuario(),
																																					funcionalidadeIniciada.getId());
					...
					case Funcionalidade.INCLUIR_DEBITO_A_COBRAR_ENTRADA_PARCELAMENTO_NAO_PAGA:
						...
						TarefaBatchIncluirDebitoACobrarEntradaParcelamentoNaoPaga dadosIncluirDebitoACobrarEntradaParcelamentoNaoPaga = new TarefaBatchIncluirDebitoACobrarEntradaParcelamentoNaoPaga(	processoIniciado.getUsuario(),
																																																		funcionalidadeIniciada.getId());
					...
					case Funcionalidade.ATUALIZAR_PAGAMENTOS_CONTAS_COBRANCA:
						...
						TarefaBatchAtualizarPagamentosContasCobranca atualizarPagamentosContasCobranca = new TarefaBatchAtualizarPagamentosContasCobranca(	processoIniciado.getUsuario(),
																																							funcionalidadeIniciada.getId());

						Collection<Integer> colecaoIdsLocalidadesAtualizarPagamentos = getControladorLocalidade().pesquisarIdLocalidadeCobrancaPorResultado();
						...
						atualizarPagamentosContasCobranca.addParametro("anoMesArrecadacaoSistemaParametro", sistemaParametros.getAnoMesArrecadacao());
					...
					case Funcionalidade.GERAR_MOVIMENTO_CONTAS_COBRANCA_POR_EMPRESA:
						...
						TarefaBatchGerarMovimentoContasCobrancaPorEmpresa gerarMovimentoContasCobrancaPorEmpresa = new TarefaBatchGerarMovimentoContasCobrancaPorEmpresa(	processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_MOVIMENTO_EXTENSAO_CONTAS_COBRANCA_POR_EMPRESA:
						...
						TarefaBatchGerarMovimentoExtensaoContasCobrancaPorEmpresa gerarMovimentoExtensaoContasCobrancaPorEmpresa = new TarefaBatchGerarMovimentoExtensaoContasCobrancaPorEmpresa(	processoIniciado.getUsuario(),
																																																	funcionalidadeIniciada.getId());
						...
						filtroComandoEmpresaCobrancaContaExtensao.adicionarParametro(new ParametroNulo(FiltroComandoEmpresaCobrancaContaExtensao.DATA_EXECUCAO));
						filtroComandoEmpresaCobrancaContaExtensao.adicionarCaminhoParaCarregamentoEntidade(FiltroComandoEmpresaCobrancaContaExtensao.COMANDO_EMPRESA_COBRANCA_CONTA);
						filtroComandoEmpresaCobrancaContaExtensao.adicionarCaminhoParaCarregamentoEntidade("comandoEmpresaCobrancaConta.empresa");

						Collection colecaoComandoEmpresaCobrancaContaExtensao = getControladorUtil().pesquisar(filtroComandoEmpresaCobrancaContaExtensao, ComandoEmpresaCobrancaContaExtensao.class.getName());
						...
						gerarMovimentoExtensaoContasCobrancaPorEmpresa.addParametro("colecaoComandoEmpresaCobrancaContaExtensao", colecaoComandoEmpresaCobrancaContaExtensao);
					...
					case Funcionalidade.ATUALIZAR_AUTOS_INFRACAO_PRAZO_RECURSO_VENCIDO:
						TarefaBatchAtualizarAutosInfracaoPrazoRecursoVencido tarefaBatchAtualizarAutosInfracaoPrazoRecursoVencido = new TarefaBatchAtualizarAutosInfracaoPrazoRecursoVencido(	processoIniciado.getUsuario(),
																																																funcionalidadeIniciada.getId());

						tarefaBatchAtualizarAutosInfracaoPrazoRecursoVencido.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, Collections.singletonList(0));
					...
					case Funcionalidade.EXCLUIR_IMOVEIS_DA_TARIFA_SOCIAL:
						TarefaBatchExcluirImoveisDaTarifaSocial excluirImoveisDaTarifaSocial = new TarefaBatchExcluirImoveisDaTarifaSocial(	processoIniciado.getUsuario(),
																																			funcionalidadeIniciada.getId());
					...
					case Funcionalidade.ACOMPANHAR_PAGAMENTO_DO_PARCELAMENTO:
						...
						TarefaBatchAcompanharPagamentoDoParcelamento acompanharPagamentoDoParcelamento = new TarefaBatchAcompanharPagamentoDoParcelamento(	processoIniciado.getUsuario(),
																																							funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_DOCUMENTOS_A_RECEBER:
						...
						TarefaBatchGerarResumoDocumentosAReceber tarefaBatchGerarResumoDocumentosAReceber = new TarefaBatchGerarResumoDocumentosAReceber(	processoIniciado.getUsuario(),
																																							funcionalidadeIniciada.getId());

						FiltroLocalidade filtroLocalidadeGerarResumoDocumentosAReceber = new FiltroLocalidade();

						Collection<Localidade> colLocalidadeGerarResumoDocumentosAReceber = getControladorUtil().pesquisar(filtroLocalidadeGerarResumoDocumentosAReceber, Localidade.class.getName());

						tarefaBatchGerarResumoDocumentosAReceber.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colLocalidadeGerarResumoDocumentosAReceber);
					...
					case Funcionalidade.BATCH_EMITIR_ORDEM_FISCALIZAO:
						...
						TarefaBatchEmitirOrdemDeFiscalizacao tarefaBatchEmitirOrdemDeFiscalizacao = new TarefaBatchEmitirOrdemDeFiscalizacao(	processoIniciado.getUsuario(),
																																				funcionalidadeIniciada.getId());

						FiltroSetorComercial filtroSetores = new FiltroSetorComercial();

						filtroSetores.adicionarParametro(new ParametroSimples(	FiltroSetorComercial.INDICADORUSO,
																				ConstantesSistema.INDICADOR_USO_ATIVO));
					...
					case Funcionalidade.BATCH_GERAR_ARQUIVO_ORDEM_FISCALIZAO:
						...
						TarefaBatchGerarArquivoOrdemDeFiscalizacaoMDB tarefaBatchGerarArquivoOrdemDeFiscalizacaoMDB = new TarefaBatchGerarArquivoOrdemDeFiscalizacaoMDB(processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());
						// Seta o objeto para ser serializado no banco, onde
						// depois ser� executado por uma thread.
					...
					case Funcionalidade.DETERMINAR_CONFIRMACAO_DA_NEGATIVACAO:
						TarefaBatchDeterminarConfirmacaoDaNegativacao determinarConfirmacaoDaNegativacao = new TarefaBatchDeterminarConfirmacaoDaNegativacao(	processoIniciado.getUsuario(),
																																								funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_PENDENCIA_POR_ANO:
						TarefaBatchGerarResumoPendenciaPorAno dadosGerarResumoPendenciaPorAno = new TarefaBatchGerarResumoPendenciaPorAno(	processoIniciado.getUsuario(),
																																			funcionalidadeIniciada.getId());

						FiltroSetorComercial filtroPendenciaPorAno = new FiltroSetorComercial();
						Collection<SetorComercial> colecaoSetorComercialPendenciaPorAno = getControladorUtil().pesquisar(filtroPendenciaPorAno, SetorComercial.class.getName());

						// Seta os parametros para rodar a funcionalidade
						dadosGerarResumoPendenciaPorAno.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoSetorComercialPendenciaPorAno);
					...
					case Funcionalidade.BATCH_ATUALIZAR_CODIGO_DEBITO_AUTOMATICO:
						...
						TarefaBatchAtualizarCodigoDebitoAutomatico tarefaBatchAtualizarCodigoDebitoAutomatico = new TarefaBatchAtualizarCodigoDebitoAutomatico(	processoIniciado.getUsuario(),
																																								funcionalidadeIniciada.getId());

						FiltroSetorComercial filtroSetoresParaAtualizarCodigoDebitoAutomatico = new FiltroSetorComercial();

						Collection<SetorComercial> colecaoSetoresParaAtualizar = getControladorUtil().pesquisar(filtroSetoresParaAtualizarCodigoDebitoAutomatico, SetorComercial.class.getName());

						tarefaBatchAtualizarCodigoDebitoAutomatico.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoSetoresParaAtualizar);
					...
					case Funcionalidade.GERAR_RESUMO_LIGACOES_ECONOMIAS_POR_ANO:
						TarefaBatchGerarResumoLigacoesEconomiasPorAno gerarResumoLigacoesEconomiasPorAno = new TarefaBatchGerarResumoLigacoesEconomiasPorAno(	processoIniciado.getUsuario(),
																																								funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_INDICADORES_MICROMEDICAO_POR_ANO:
						...
						TarefaBatchGerarResumoIndicadoresMicromedicaoPorAno gerarResumoIndicadoresMicromedicaoPorAno = new TarefaBatchGerarResumoIndicadoresMicromedicaoPorAno(	processoIniciado.getUsuario(),
																																												funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_CONSUMO_AGUA_POR_ANO:
						TarefaBatchGerarResumoConsumoAguaPorAno gerarResumoConsumoAguaPorAno = new TarefaBatchGerarResumoConsumoAguaPorAno(	processoIniciado.getUsuario(),
																																			funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_FATURAMENTO_POR_ANO:
						...
						TarefaBatchGerarResumoFaturamentoPorAno dadosGerarResFaturamento = new TarefaBatchGerarResumoFaturamentoPorAno(	processoIniciado.getUsuario(),
																																		funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_ARRECADACAO_POR_ANO:
						TarefaBatchGerarResumoArrecadacaoPorAno dadosGerarResumoArrecadacaoAno = new TarefaBatchGerarResumoArrecadacaoPorAno(	processoIniciado.getUsuario(),
																																				funcionalidadeIniciada.getId());
						...
						Collection<Localidade> colLocalidadeResumoArrecadacaoAno = getControladorUtil().pesquisar(filtroLocalidadeResumoArrecadacaoAno, Localidade.class.getName());
					...
					case Funcionalidade.GERAR_RESUMO_COLETA_ESGOTO_POR_ANO:
						TarefaBatchGerarResumoColetaEsgotoPorAno gerarResumoColetaEsgotoPorAno = new TarefaBatchGerarResumoColetaEsgotoPorAno(	processoIniciado.getUsuario(),
																																				funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_REGISTRO_ATENDIMENTO_POR_ANO:
						TarefaBatchGerarResumoRegistroAtendimentoPorAno gerarResumoRegistroAtendimentoPorAno = new TarefaBatchGerarResumoRegistroAtendimentoPorAno(	processoIniciado.getUsuario(),
																																									funcionalidadeIniciada.getId());

						Collection<Integer> colecaoIdsLocalidadesGerarResumoRegistroAtendimentoMesPorAno = getControladorLocalidade().pesquisarTodosIdsLocalidade();

						gerarResumoRegistroAtendimentoPorAno.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesGerarResumoRegistroAtendimentoMesPorAno);
					...
					case Funcionalidade.GERAR_RESUMO_INSTALACOES_HIDROMETROS_POR_ANO:
						TarefaBatchGerarResumoInstalacoesHidrometrosPorAno dadosGerarResumoInstalacoesHidrometrosPorAno = new TarefaBatchGerarResumoInstalacoesHidrometrosPorAno(	processoIniciado.getUsuario(),
																																													funcionalidadeIniciada.getId());
						 ...
						 * para o ano/mes de referancia do faturamento.
						 */
						Collection<Integer> colecaoIdsSetoresComercialPorAno = getControladorGerencialMicromedicao().pesquisarIdsSetorComercialParaGerarResumoInstalacaoHidrometro(anoMesFaturamentoSistemaParametro);

						// Seta os parametros para rodar a funcionalidade
						dadosGerarResumoInstalacoesHidrometrosPorAno.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsSetoresComercialPorAno);
					...
					case Funcionalidade.GERAR_RESUMO_PARCELAMENTO_POR_ANO:
						TarefaBatchGerarResumoParcelamentoPorAno dadosGerarResumoParcelamentoPorAno = new TarefaBatchGerarResumoParcelamentoPorAno(	processoIniciado.getUsuario(),
																																					funcionalidadeIniciada.getId());
						// 2222
						Collection<Integer> colecaoLocalidadesParcelamentoPorAno = getControladorFaturamento().pesquisarIdsLocalidadeParaGerarResumoLigacoesEconomias();

						dadosGerarResumoParcelamentoPorAno.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoLocalidadesParcelamentoPorAno);
					...
					case Funcionalidade.ALTERAR_INSCRICOES_IMOVEIS:
						...
						TarefaBatchAlterarInscricaoImovel tarefaBatchAlterarInscricaoImovel = new TarefaBatchAlterarInscricaoImovel(processoIniciado.getUsuario(),
																																	funcionalidadeIniciada.getId());

						FiltroLocalidade filtroLocalidadeAlterarInscricoesImoveis = new FiltroLocalidade();

						Collection<Localidade> colecaoLocalidadeAlterarInscricoesImoveis = getControladorUtil().pesquisar(filtroLocalidadeAlterarInscricoesImoveis, Localidade.class.getName());

						// Seta os parametros para rodar a funcionalidade
						tarefaBatchAlterarInscricaoImovel.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoLocalidadeAlterarInscricoesImoveis);
					...
					case Funcionalidade.GERAR_PRESCREVER_DEBITOS_DE_IMOVEIS:
						TarefaBatchGerarPrescreverDebitosDeImoveis dadosGerarPrescreverDebitosDeImoveis = new TarefaBatchGerarPrescreverDebitosDeImoveis(	processoIniciado.getUsuario(),
																																							funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RESUMO_REFATURAMENTO_NOVO:
						...
						if (existeResumo == null || existeResumo > 0) {
							...
							throw new ControladorException(	"atencao.resumo.refaturamento.ja.existe",
															null,
															sistemaParametros.getAnoMesFaturamento().toString());

						}

						TarefaBatchGerarResumoReFaturamentoNovo dadosGerarResumoReFaturamentoNovo = new TarefaBatchGerarResumoReFaturamentoNovo(processoIniciado.getUsuario(),
																																				funcionalidadeIniciada.getId());

						FiltroSetorComercial filtroReFaturamentoNovo = new FiltroSetorComercial();

						Collection<SetorComercial> colSetorReFaturamentoNovo = getControladorUtil().pesquisar(filtroReFaturamentoNovo, SetorComercial.class.getName());

						/*
						 * Adicionamos um setor comercial fict�cio que ser�
						 * utilizado para selecionarmos as guias de pagamento
						 * sem imovel.
						 */
					...
					case Funcionalidade.GERAR_RESUMO_HISTOGRAMA_AGUA_ESGOTO_SEM_QUADRA:
						TarefaBatchGerarResumoHistogramaAguaEsgotoSemQuadra gerarResumoAguaEsgotoSemQuadra = new TarefaBatchGerarResumoHistogramaAguaEsgotoSemQuadra(	processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());

						FiltroSetorComercial filtroHistogramaSemQuadra = new FiltroSetorComercial();
						Collection<SetorComercial> colSetorHistogramaSemQuadra = getControladorUtil().pesquisar(filtroHistogramaSemQuadra, SetorComercial.class.getName());
					...
					case Funcionalidade.RELIGAR_IMOVEIS_CORTADOS_COM_CONSUMO_REAL:
						...
						TarefaBatchReligarImoveisCortadosComConsumoReal dadosReligarImoveisCortadosComConsumoReal = new TarefaBatchReligarImoveisCortadosComConsumoReal(processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());

						FiltroLocalidade filtroLocalidadeReligarImoveisCortadosComConsumoReal = new FiltroLocalidade();

						Collection<Localidade> colecaoLocalidadesReligarImoveisCortadosComConsumoReal = getControladorUtil().pesquisar(filtroLocalidadeReligarImoveisCortadosComConsumoReal, Localidade.class.getName());

						dadosReligarImoveisCortadosComConsumoReal.addParametro("anoMesFaturamento", sistemaParametros.getAnoMesFaturamento());

						// Seta os parametros para rodar a funcionalidade
						dadosReligarImoveisCortadosComConsumoReal.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoLocalidadesReligarImoveisCortadosComConsumoReal);
					...
					case Funcionalidade.PRESCREVER_DEBITOS_IMOVEIS_PUBLICOS:
						TarefaBatchPrescreverDebitosImoveisPublicosAutomatico prescricao = new TarefaBatchPrescreverDebitosImoveisPublicosAutomatico(	processoIniciado.getUsuario(),
																																						funcionalidadeIniciada.getId());
					...
					case Funcionalidade.SELECIONAR_COMANDO_RETIRAR_IMOVEL_TARIFA_SOCIAL:
						...
						TarefaBatchRetirarImovelTarifaSocial tarefaBatchRetirarImovelTarifaSocial = new TarefaBatchRetirarImovelTarifaSocial(	processoIniciado.getUsuario(),
																																				funcionalidadeIniciada.getId());
					...
					case Funcionalidade.AUTOMATIZAR_PERFIS_DE_GRANDES_CONSUMIDORES:
						...
						TarefaBatchAutomatizarPerfisDeGrandesConsumidores dadosAutomatizarPerfisDeGrandesConsumidores = new TarefaBatchAutomatizarPerfisDeGrandesConsumidores(	processoIniciado.getUsuario(),
																																												funcionalidadeIniciada.getId());

						FiltroLocalidade filtroLocalidadeAutomatizarPerfis = new FiltroLocalidade();

						Collection<Localidade> colecaoIdsLocalidadesAutomatizarPerfisDeGrandesConsumidores = getControladorUtil().pesquisar(filtroLocalidadeAutomatizarPerfis, Localidade.class.getName());

						// Seta os parametros para rodar a funcionalidade
						dadosAutomatizarPerfisDeGrandesConsumidores.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesAutomatizarPerfisDeGrandesConsumidores);
					...
					case Funcionalidade.GERAR_TXT_IMPRESSAO_CONTAS_FORMATO_BRAILLE:
						...
						Collection colecaoGrupoNaoFaturados = getControladorFaturamento().pesquisarGrupoFaturamentoGrupoNaoFaturados(anoMesFaturamentoSistemaParametro);
						// Caso haja algum grupo de faturamento que n�o tenha
						// sido faturado
						...
						TarefaBatchGerarTxtImpressaoContasBraille gerarTxtContasBraille = new TarefaBatchGerarTxtImpressaoContasBraille(processoIniciado.getUsuario(),
																																		funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_ARQUIVO_TXT_OS_CONTAS_PAGAS_PARCELADAS:
						...
						TarefaBatchGerarArquivoTextoOSContasPagasParceladas gerarArqvTxtOSContasPagasParceladas = new TarefaBatchGerarArquivoTextoOSContasPagasParceladas(	processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());
					...
					case Funcionalidade.ENVIO_EMAIL_CONTA_PARA_CLIENTE:
						TarefaBatchEnvioEmailContaParaCliente tarefaBatchEnvioEmailContaParaCliente = new TarefaBatchEnvioEmailContaParaCliente(processoIniciado.getUsuario(),
																																				funcionalidadeIniciada.getId());

						// Seta os parametros para rodar a funcionalidade
						FiltroLocalidade filtroLocalidadeEnvioEmail = new FiltroLocalidade();
						filtroLocalidadeEnvioEmail.adicionarParametro(new ParametroSimples(	FiltroLocalidade.INDICADORUSO,
																							ConstantesSistema.INDICADOR_USO_ATIVO));
						...
						tarefaBatchEnvioEmailContaParaCliente.addParametro("faturamentoGrupo", null);
					...
					case Funcionalidade.PROGRAMACAO_AUTO_ROTEIRO_ACOMPANHAMENTO_OS:
						TarefaBatchProgramacaoAutoRoteiroAcompanhamentoOS tarefaBatchProgramacaoAutoRoteiroAcompanhamentoOS = new TarefaBatchProgramacaoAutoRoteiroAcompanhamentoOS(processoIniciado.getUsuario(),
																																													funcionalidadeIniciada.getId());
						...
						tarefaBatchProgramacaoAutoRoteiroAcompanhamentoOS.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, collIdsUnidadesOrganizacionais);
					...
					case Funcionalidade.GERAR_DADOS_ARQUIVO_ACOMPANHAMENTO_SERVICO:
						TarefaBatchGerarDadosArquivoAcompanhamentoServico tarefaBatchGerarDadosArquivoAcompanhamentoServico = new TarefaBatchGerarDadosArquivoAcompanhamentoServico(processoIniciado.getUsuario(),
																																													funcionalidadeIniciada.getId());
						...
						tarefaBatchGerarDadosArquivoAcompanhamentoServico.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, collIdsUnidadesOrganizacionais2);
					...
					case Funcionalidade.PROCESSAR_ENCERRAMENTO_OS_FISCALIZACAO_DECURSO_PRAZO:
						...
						TarefaBatchProcessarEncerramentoOSFiscalizacaoDecursoPrazo tarefaBatchProcessarEncerramentoOSFiscalizacaoDecursoPrazo = new TarefaBatchProcessarEncerramentoOSFiscalizacaoDecursoPrazo(	processoIniciado.getUsuario(),
																																																				funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_BOLETIM_COBRANCA_POR_RESULTADO:
						TarefaBatchGerarBoletimCobrancaPorResultado tarefaBatchGerarBoletimCobrancaPorResultado = new TarefaBatchGerarBoletimCobrancaPorResultado(	processoIniciado.getUsuario(),
																																									funcionalidadeIniciada.getId());

						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(tarefaBatchGerarBoletimCobrancaPorResultado));
					...
					case Funcionalidade.INCLUIR_CONTAS_EM_COBRANCA:
						TarefaBatchIncluirContasEmCobranca tarefaBatchIncluirContasEmCobranca = new TarefaBatchIncluirContasEmCobranca(	processoIniciado.getUsuario(),
																																		funcionalidadeIniciada.getId());

						Collection<Integer> colecaoIdsLocalidadesIncluirContas = getControladorLocalidade().pesquisarIdLocalidadeCobrancaPorResultado();

						tarefaBatchIncluirContasEmCobranca.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesIncluirContas);

						tarefaBatchIncluirContasEmCobranca.addParametro("anoMesArrecadacaoSistemaParametro", sistemaParametros.getAnoMesArrecadacao());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(tarefaBatchIncluirContasEmCobranca));
					...
					case Funcionalidade.BATCH_GERAR_FATURAMENTO_BRUTO_LIQUIDO:
						...
						TarefaBatchGerarFaturamentoBrutoLiquido gerarFaturamentoBrutoLiquido = new TarefaBatchGerarFaturamentoBrutoLiquido(	processoIniciado.getUsuario(),
																																			funcionalidadeIniciada.getId());
						...
						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarFaturamentoBrutoLiquido));
					...
					case Funcionalidade.BATCH_GERAR_RESUMO_ARRECADACAO_ANALITICO_DADOS:
						...
						TarefaBatchGerarResumoArrecadacaoAnaliticoDados gerarResumoArrecadacaoAnaliticoDados = new TarefaBatchGerarResumoArrecadacaoAnaliticoDados(	processoIniciado.getUsuario(),
																																									funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarResumoArrecadacaoAnaliticoDados));
					...
					case Funcionalidade.BATCH_GERAR_RESUMO_ARRECADACAO_ANALITICO_TOTAIS_1:
						...
						TarefaBatchGerarResumoArrecadacaoAnaliticoTotais1 gerarResumoArrecadacaoAnaliticoTotais1 = new TarefaBatchGerarResumoArrecadacaoAnaliticoTotais1(	processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarResumoArrecadacaoAnaliticoTotais1));
					...
					case Funcionalidade.BATCH_GERAR_RESUMO_ARRECADACAO_ANALITICO_TOTAIS_2:
						...
						TarefaBatchGerarResumoArrecadacaoAnaliticoTotais2 gerarResumoArrecadacaoAnaliticoTotais2 = new TarefaBatchGerarResumoArrecadacaoAnaliticoTotais2(	processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarResumoArrecadacaoAnaliticoTotais2));
					...
					case Funcionalidade.EXECUTAR_CARGA_REGISTRO_ATENDIMENTO_POR_ANO:
						...
						TarefaBatchExecutarCargaRegistroAtendimentoPorAno executarCargaRegistroAtendimentoPorAno = new TarefaBatchExecutarCargaRegistroAtendimentoPorAno(	processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaRegistroAtendimentoPorAno));
					...
					case Funcionalidade.BATCH_EXECUTAR_CARGA_ARRECADACAO_GERENCIAL_POR_ANO:
						...
						TarefaBatchExecutarCargaArrecadacaoGerencialAno executarCargaArrecadacaoGerencialAno = new TarefaBatchExecutarCargaArrecadacaoGerencialAno(	processoIniciado.getUsuario(),
																																									funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaArrecadacaoGerencialAno));
					...
					case Funcionalidade.EXECUTAR_CARGA_RESUMO_FATURAMENTO_POR_ANO:
						...
						TarefaBatchExecutarCargaResumoFaturamentoPorAno executarCargaResumoFaturamentoPorAno = new TarefaBatchExecutarCargaResumoFaturamentoPorAno(	processoIniciado.getUsuario(),
																																									funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaResumoFaturamentoPorAno));
					...
					case Funcionalidade.EXECUTAR_CARGA_INDICADORES_DESEMPENHO_FATURAMENTO_POR_ANO:
						...
						TarefaBatchExecutarCargaIndicadoresDesempenhoFaturamentoPorAno executarCargaIndicadoresDesempenhoFaturamentoPorAno = new TarefaBatchExecutarCargaIndicadoresDesempenhoFaturamentoPorAno(processoIniciado.getUsuario(),
																																																				funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaIndicadoresDesempenhoFaturamentoPorAno));
					...
					case Funcionalidade.EXECUTAR_CARGA_INDICADORES_DESEMPENHO_FATURAMENTO_VARIOS_ANOS:
						...
						TarefaBatchExecutarCargaIndicadoresDesempenhoFaturamentoVariosAnos executarCargaIndicadoresDesempenhoFaturamentoVariosAnos = new TarefaBatchExecutarCargaIndicadoresDesempenhoFaturamentoVariosAnos(processoIniciado.getUsuario(),
																																																							funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaIndicadoresDesempenhoFaturamentoVariosAnos));
					...
					case Funcionalidade.EXECUTAR_CARGA_COLETA_ESGOTO_POR_ANO:
						...
						TarefaBatchExecutarCargaColetaEsgotoPorAno executarCargaColetaEsgotoPorAno = new TarefaBatchExecutarCargaColetaEsgotoPorAno(processoIniciado.getUsuario(),
																																					funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaColetaEsgotoPorAno));
					...
					case Funcionalidade.EXECUTAR_CARGA_CONSUMO_AGUA_POR_ANO:
						...
						TarefaBatchExecutarCargaConsumoAguaPorAno executarCargaConsumoAguaPorAno = new TarefaBatchExecutarCargaConsumoAguaPorAno(	processoIniciado.getUsuario(),
																																					funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaConsumoAguaPorAno));
					...
					case Funcionalidade.EXECUTAR_CARGA_LEITURA_ANORMALIDADE_POR_ANO:
						...
						TarefaBatchExecutarCargaLeituraAnormalidadePorAno executarCargaLeituraAnormalidadePorAno = new TarefaBatchExecutarCargaLeituraAnormalidadePorAno(	processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaLeituraAnormalidadePorAno));
					...
					case Funcionalidade.EXECUTAR_CARGA_INDICADORES_DESEMPENHO_MICROMEDICAO_POR_ANO:
						...
						TarefaBatchExecutarCargaIndicadoresDesempenhoMicromedicaoPorAno executarCargaIndicadoresDesempenhoMicromedicaoPorAno = new TarefaBatchExecutarCargaIndicadoresDesempenhoMicromedicaoPorAno(	processoIniciado.getUsuario(),
																																																					funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaIndicadoresDesempenhoMicromedicaoPorAno));
					...
					case Funcionalidade.EXECUTAR_CARGA_INDICADORES_DESEMPENHO_MICROMEDICAO_VARIOS_ANOS:
						...
						TarefaBatchExecutarCargaIndicadoresDesempenhoMicromedicaoVariosAnos executarCargaIndicadoresDesempenhoMicromedicaoVariosAnos = new TarefaBatchExecutarCargaIndicadoresDesempenhoMicromedicaoVariosAnos(	processoIniciado.getUsuario(),
																																																								funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaIndicadoresDesempenhoMicromedicaoVariosAnos));
					...
					case Funcionalidade.EXECUTAR_CARGA_LIGACOES_ECONOMIAS_POR_ANO:
						...
						TarefaBatchExecutarCargaLigacoesEconomiasPorAno executarCargaLigacoesEconomiasPorAno = new TarefaBatchExecutarCargaLigacoesEconomiasPorAno(	processoIniciado.getUsuario(),
																																									funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaLigacoesEconomiasPorAno));
					...
					case Funcionalidade.EXECUTAR_CARGA_INDICADOR_LIGACAO_ECONOMIA_POR_ANO:
						...
						TarefaBatchExecutarCargaIndicadorLigacaoEconomiaPorAno executarCargaIndicadorLigacaosEconomiaPorAno = new TarefaBatchExecutarCargaIndicadorLigacaoEconomiaPorAno(	processoIniciado.getUsuario(),
																																															funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaIndicadorLigacaosEconomiaPorAno));
					...
					case Funcionalidade.EXECUTAR_CARGA_RESUMO_PARCELAMENTO_POR_ANO:
						...
						TarefaBatchExecutarCargaResumoParcelamentoPorAno executarCargaResumoParcelamentoPorAno = new TarefaBatchExecutarCargaResumoParcelamentoPorAno(	processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaResumoParcelamentoPorAno));
					...
					case Funcionalidade.EXECUTAR_CARGA_RESUMO_PENDENCIA_POR_ANO:
						...
						TarefaBatchExecutarCargaResumoPendenciaPorAno executarCargaResumoPendenciaPorAno = new TarefaBatchExecutarCargaResumoPendenciaPorAno(	processoIniciado.getUsuario(),
																																								funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaResumoPendenciaPorAno));
					...
					case Funcionalidade.EXECUTAR_CARGA_PENDENCIA_SEM_QUADRA_VARIOS_ANOS:
						...
						TarefaBatchExecutarCargaPendenciaSemQuadraVariosAnos executarCargaPendenciaSemQuadraVariosAnos = new TarefaBatchExecutarCargaPendenciaSemQuadraVariosAnos(	processoIniciado.getUsuario(),
																																													funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(executarCargaPendenciaSemQuadraVariosAnos));
					...
					case Funcionalidade.GERAR_RESUMO_PENDENCIA_BD:
						...
						TarefaBatchGerarResumoPendenciaBD tarefaBatchGerarResumoPendenciaBD = new TarefaBatchGerarResumoPendenciaBD(processoIniciado.getUsuario(),
																																	funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(tarefaBatchGerarResumoPendenciaBD));

						getControladorUtil().atualizar(funcionalidadeIniciada);
						break;

					case Funcionalidade.RETIRAR_CONTAS_REVISAO_EXTRATO_ENTRADA_PARCELAMENTO:

						TarefaBatchRetirarContasRevisaoExtratoEntradaParcelamento retirarContasRevisaoEntradaParcelamento = new TarefaBatchRetirarContasRevisaoExtratoEntradaParcelamento(	processoIniciado.getUsuario(),
																																															funcionalidadeIniciada.getId());

						FiltroLocalidade filtroLocalidadeContasRevisao = new FiltroLocalidade();
						Collection<Localidade> colecaoLocalidadeContasRevisao = getControladorUtil().pesquisar(filtroLocalidadeContasRevisao, Localidade.class.getName());

						// Seta os parametros para rodar a funcionalidade
						retirarContasRevisaoEntradaParcelamento.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoLocalidadeContasRevisao);

						// pesquisa numero de dias para cancelar a entrada de
						// parcelamento
						Integer numeroDiasCancelarEntParc = sistemaParametros.getNumeroDiasCancelamentoEntradaParcelamento();

						Date dataLimiteValidade = new Date();
						if (numeroDiasCancelarEntParc != null) {
							dataLimiteValidade = Util.subtrairNumeroDiasDeUmaData(new Date(), numeroDiasCancelarEntParc);
						}

						retirarContasRevisaoEntradaParcelamento.addParametro("dataLimiteValidade", dataLimiteValidade);

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(retirarContasRevisaoEntradaParcelamento));

						getControladorUtil().atualizar(funcionalidadeIniciada);
						break;

					case Funcionalidade.GERAR_FATURA_PROGRAMA_ESPECIAL:

						TarefaBatchGerarFaturaProgramaEspecial tarefaBatchGerarFaturaProgramaEspecialBD = new TarefaBatchGerarFaturaProgramaEspecial(	processoIniciado.getUsuario(),
																																						funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(tarefaBatchGerarFaturaProgramaEspecialBD));

						getControladorUtil().atualizar(funcionalidadeIniciada);
						break;

					case Funcionalidade.RECEITA_LIQUIDA_INDIRETA_DE_AGUA_E_ESGOTO:

						TarefaBatchReceitaLiquidaIndiretaDeAguaEEsgotoMDB receitaLiquidaIndiretaDeAguaEEsgotoMDB = new TarefaBatchReceitaLiquidaIndiretaDeAguaEEsgotoMDB(	processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(receitaLiquidaIndiretaDeAguaEEsgotoMDB));
					...
					case Funcionalidade.BATCH_GERAR_RESUMO_FATURAMENTO_ANALITICO_PARTE_1:
						...
						TarefaBatchGerarResumoFaturamentoAnaliticoParte1 gerarResumoFaturamentoAnaliticoParte1 = new TarefaBatchGerarResumoFaturamentoAnaliticoParte1(	processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarResumoFaturamentoAnaliticoParte1));
					...
					case Funcionalidade.BATCH_GERAR_RESUMO_FATURAMENTO_ANALITICO_PARTE_2:
						...
						TarefaBatchGerarResumoFaturamentoAnaliticoParte2 gerarResumoFaturamentoAnaliticoParte2 = new TarefaBatchGerarResumoFaturamentoAnaliticoParte2(	processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarResumoFaturamentoAnaliticoParte2));
					...
					case Funcionalidade.BATCH_GERAR_RESUMO_FATURAMENTO_ANALITICO_TOTAIS:
						...
						TarefaBatchGerarResumoFaturamentoAnaliticoTotais gerarResumoFaturamentoAnaliticoTotais = new TarefaBatchGerarResumoFaturamentoAnaliticoTotais(	processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarResumoFaturamentoAnaliticoTotais));

						getControladorUtil().atualizar(funcionalidadeIniciada);
						break;

					case Funcionalidade.BATCH_GERAR_CREDITO_REALIZAR_PAGAMENTO_DUPLICIDADE_VIVA_AGUA:

						TarefaBatchGerarCreditoARealizarPagamentoDuplicidadeVivaAgua gerarCreditoARealizarPagamentoDuplicidadeVivaAgua = new TarefaBatchGerarCreditoARealizarPagamentoDuplicidadeVivaAgua(	processoIniciado.getUsuario(),
																																																			funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarCreditoARealizarPagamentoDuplicidadeVivaAgua));

						getControladorUtil().atualizar(funcionalidadeIniciada);
						break;

					/* Am�lia Pessoa */
					case Funcionalidade.AUTOMATIZAR_CARGA_MENSAL_PENTAHO:

						TarefaBatchAutomatizarCargaPentaho automatizarCargaPentaho = new TarefaBatchAutomatizarCargaPentaho(processoIniciado.getUsuario(),
																															funcionalidadeIniciada.getId());

						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(automatizarCargaPentaho));
					...
					case Funcionalidade.BATCH_GERAR_CONTAS_A_RECEBER_CONTABIL_ANALITICO_TABELA_AUXILIAR:
						...
						TarefaBatchGerarContasAReceberContabilAnaliticoTabelaAuxiliar gerarContasAReceberContabilAnaliticoTabelaAuxiliar = new TarefaBatchGerarContasAReceberContabilAnaliticoTabelaAuxiliar(	processoIniciado.getUsuario(),
																																																				funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarContasAReceberContabilAnaliticoTabelaAuxiliar));
					...
					case Funcionalidade.BATCH_GERAR_CONTAS_A_RECEBER_CONTABIL_ANALITICO:
						...
						TarefaBatchGerarContasAReceberContabilAnalitico gerarContasAReceberContabilAnalitico = new TarefaBatchGerarContasAReceberContabilAnalitico(	processoIniciado.getUsuario(),
																																									funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarContasAReceberContabilAnalitico));
					...
					case Funcionalidade.BATCH_GERAR_CONTAS_A_RECEBER_CONTABIL_SINTETICO:
						...
						TarefaBatchGerarContasAReceberContabilSintetico gerarContasAReceberContabilSintetico = new TarefaBatchGerarContasAReceberContabilSintetico(	processoIniciado.getUsuario(),
																																									funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarContasAReceberContabilSintetico));

						getControladorUtil().atualizar(funcionalidadeIniciada);
						break;

					/**
					 * @author Rafael Corr�ea
					 * @since 17/04/2013
					 */
					case Funcionalidade.GERAR_UN_RESUMO_ARRECADACAO_BD:

						TarefaBatchGerarUnResumoArrecadacaoBD gerarUnResumoArrecadacaoBD = new TarefaBatchGerarUnResumoArrecadacaoBD(	processoIniciado.getUsuario(),
																																		funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarUnResumoArrecadacaoBD));
					 ...
					 * @since 08/07/2013
					 */
					case Funcionalidade.BATCH_BAIXA_AUTOMATICA_PAGAMENTOS_NAO_CLASSIFICADOS:

						TarefaBatchBaixaAutomaticaPagamentosNaoClassificados baixaAutomaticaPagamentosNaoClassificados = new TarefaBatchBaixaAutomaticaPagamentosNaoClassificados(	processoIniciado.getUsuario(),
																																													funcionalidadeIniciada.getId());

						colecaoTodasRotas = getControladorMicromedicao().pesquisarListaRotasCarregadas();

						baixaAutomaticaPagamentosNaoClassificados.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoTodasRotas);

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(baixaAutomaticaPagamentosNaoClassificados));

						getControladorUtil().atualizar(funcionalidadeIniciada);
						break;

					case Funcionalidade.BATCH_PROCESSAR_AMORTIZACAO_DIVIDA_ATIVA_ARRECADACAO:

						TarefaBatchProcessarAmortizacaoDividaAtivaArrecadacao tarefaBatchProcessarAmortizacaoDividaAtivaArrecadacao = new TarefaBatchProcessarAmortizacaoDividaAtivaArrecadacao(processoIniciado.getUsuario(),
																																																funcionalidadeIniciada.getId());

						Collection colecaoIdsLocalidadeProcessarAmortizacaoDividaAtivaArrecadacao = getControladorCadastro().pesquisarLocalidade();

						tarefaBatchProcessarAmortizacaoDividaAtivaArrecadacao.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadeProcessarAmortizacaoDividaAtivaArrecadacao);
						tarefaBatchProcessarAmortizacaoDividaAtivaArrecadacao.addParametro("anoMesReferenciaArrecadacao", anoMesArrecadacaoSistemaParametro);

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(tarefaBatchProcessarAmortizacaoDividaAtivaArrecadacao));

						getControladorUtil().atualizar(funcionalidadeIniciada);

						break;

					case Funcionalidade.BATCH_PROCESSAR_AMORTIZACAO_DIVIDA_ATIVA_FATURAMENTO:

						TarefaBatchProcessarAmortizacaoDividaAtivaFaturamento tarefaBatchProcessarAmortizacaoDividaAtivaFaturamento = new TarefaBatchProcessarAmortizacaoDividaAtivaFaturamento(processoIniciado.getUsuario(),
																																																funcionalidadeIniciada.getId());

						Collection colecaoIdsLocalidadeProcessarAmortizacaoDividaAtivaFaturamento = getControladorCadastro().pesquisarLocalidade();

						tarefaBatchProcessarAmortizacaoDividaAtivaFaturamento.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadeProcessarAmortizacaoDividaAtivaFaturamento);
						tarefaBatchProcessarAmortizacaoDividaAtivaFaturamento.addParametro("anoMesReferenciaFaturamento", anoMesFaturamentoSistemaParametro);

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(tarefaBatchProcessarAmortizacaoDividaAtivaFaturamento));

						getControladorUtil().atualizar(funcionalidadeIniciada);

						break;

					case Funcionalidade.GERAR_DIVIDA_ATIVA:

						TarefaBatchGerarDividaAtiva gerardividaativa = new TarefaBatchGerarDividaAtiva(	processoIniciado.getUsuario(),
																										funcionalidadeIniciada.getId());

						// Seta o objeto para ser serializado no banco,
						// onde depois seria executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerardividaativa));

						getControladorUtil().atualizar(funcionalidadeIniciada);

						break;

					case Funcionalidade.GERAR_RESUMO_DIVIDA_ATIVA_ANUAL:

						TarefaBatchGerarResumoDividaAtivaAnual tarefaBatchGerarResumoDividaAtivaAnual = new TarefaBatchGerarResumoDividaAtivaAnual(	processoIniciado.getUsuario(),
																																					funcionalidadeIniciada.getId());
						...
						tarefaBatchGerarResumoDividaAtivaAnual.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, idsSetorComercial);

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(tarefaBatchGerarResumoDividaAtivaAnual));

						getControladorUtil().atualizar(funcionalidadeIniciada);

						break;

						case Funcionalidade.BATCH_ENVIAR_EMAIL_COBRANCA_FATURAMENTO:
							
							TarefaBatchEnviarEmailCobrancaFaturamento tarefaBatchEnviarEmailCobrancaFaturamento = new TarefaBatchEnviarEmailCobrancaFaturamento(
							...
							funcionalidadeIniciada
									.setTarefaBatch(IoUtil
											.transformarObjetoParaBytes(tarefaBatchEnviarEmailCobrancaFaturamento));
						...
						case Funcionalidade.RETORNO_SMS_EM_LOTE:
							
							TarefaBatchRetornoSMSEmLote tarefaBatchRetornoSMSEmLote = new TarefaBatchRetornoSMSEmLote(
							processoIniciado.getUsuario(), funcionalidadeIniciada.getId());
							
							
							funcionalidadeIniciada.setTarefaBatch(IoUtil
								.transformarObjetoParaBytes(tarefaBatchRetornoSMSEmLote));
						...
						case Funcionalidade.BATCH_TRANSMITIR_SMS_EM_LOTE_COBRANCA_FATURAMENTO:
							
							TarefaBatchTransmitirSMSEmLoteCobrancaFaturamento tarefaBatchTransmitirSMSEmLoteCobrancaFaturamento = new TarefaBatchTransmitirSMSEmLoteCobrancaFaturamento(
									processoIniciado.getUsuario(), funcionalidadeIniciada.getId());
							...
							funcionalidadeIniciada
									.setTarefaBatch(IoUtil
											.transformarObjetoParaBytes(tarefaBatchTransmitirSMSEmLoteCobrancaFaturamento));
	...
	public Integer inserirProcessoIniciadoFaturamentoComandado(Collection<Integer> idsFaturamentoAtividadeCronograma,
		...
		try {
			...
			while (iteratorFaturamentoAtividadeCronograma.hasNext()) {
				...
				Collection colecaoFaturamentoAtivCronRota = repositorioBatch.pesquisarRotasProcessamentoBatchFaturamentoComandado(codigoFaturamentoAtividadeCronograma);
				...
				filtroFaturamentoAtividadeCronograma.adicionarCaminhoParaCarregamentoEntidade("faturamentoAtividade.cobrancaAcao");
				...
				FaturamentoAtividadeCronograma faturamentoAtividadeCronograma = (FaturamentoAtividadeCronograma) Util.retonarObjetoDeColecao(getControladorUtil().pesquisar(filtroFaturamentoAtividadeCronograma, FaturamentoAtividadeCronograma.class.getName()));
				...
				if (processo.getId().intValue() == Processo.FATURAR_GRUPO_FATURAMENTO) {
					Collection<Rota> colecaoRota = this.repositorioMicromedicao.pesquisaRotasNaoTransmitidas(faturamentoGrupo.getAnoMesReferencia(), faturamentoGrupo.getId());
					...
					if (colecaoRota != null && !colecaoRota.isEmpty()) {
						...
						throw new ControladorException(	"atencao.rotas_nao_transmitidas",
														null,
														mensagemAlerta);
				...
				if (processo.getId().equals(new Integer("273"))) {
				...
				while (iterator.hasNext()) {
					...
					case Funcionalidade.GERAR_DADOS_PARA_LEITURA:
						...
						// Seta os parametros para rodar a funcionalidade
						dadosParaLeitura.addParametro("anoMesFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getAnoMesReferencia());
						dadosParaLeitura.addParametro("idFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getId());
					...
					case Funcionalidade.GERAR_ARQUIVO_TEXTO_PARA_LEITURISTA:
						TarefaBatchGerarArquivoTextoParaLeiturista dadosGerarArquivoTextoParaLeiturista = new TarefaBatchGerarArquivoTextoParaLeiturista(	processoIniciado.getUsuario(),
																																							funcionalidadeIniciada.getId());

						// Seta os parametros para rodar a funcionalidade
						dadosGerarArquivoTextoParaLeiturista.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoFaturamentoAtivCronRota);
						...
						dadosGerarArquivoTextoParaLeiturista.addParametro("anoMesFaturamento", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getAnoMesReferencia());
						dadosGerarArquivoTextoParaLeiturista.addParametro("dataComando", faturamentoAtividadeCronograma.getComando());
						dadosGerarArquivoTextoParaLeiturista.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());
					...
					case Funcionalidade.CONSISTIR_LEITURAS_E_CALCULAR_CONSUMOS:
						...
						TarefaBatchConsistirLeiturasCalcularConsumos calcularConsumos = new TarefaBatchConsistirLeiturasCalcularConsumos(	processoIniciado.getUsuario(),
																																			funcionalidadeIniciada.getId());
						...
						// --------------------------------------------------------------
						getControladorMicromedicao().processarLeiturasNaoResgistradas(faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());
						// --------------------------------------------------------------

						// Seta os parametros para rodar a funcionalidade
						calcularConsumos.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());
					...
					case Funcionalidade.GERAR_BONUS_TARIFA_SOCIAL:
						...
						TarefaBatchGerarBonusTarifaSocial tarefaBatchGerarBonusTarifaSocial = new TarefaBatchGerarBonusTarifaSocial(processoIniciado.getUsuario(),
																																	funcionalidadeIniciada.getId());

						// Seta os parametros para rodar a funcionalidade
						tarefaBatchGerarBonusTarifaSocial.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());
					...
					case Funcionalidade.EFETUAR_RATEIO_CONSUMO:
						...
						// Seta os parametros para rodar a funcionalidade
						rateioConsumo.addParametro("anoMesFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getAnoMesReferencia());
					...
					case Funcionalidade.FATURAR_GRUPO_FATURAMENTO:
						...
						// Seta os parametros para rodar a funcionalidade
						faturarGrupoFaturamento.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());
					...
					case Funcionalidade.GERAR_TAXA_ENTREGA_CONTA_OUTRO_ENDERECO:
						TarefaBatchGerarTaxaEntregaContaOutroEndereco taxaEntrega = new TarefaBatchGerarTaxaEntregaContaOutroEndereco(	processoIniciado.getUsuario(),
																																		funcionalidadeIniciada.getId());

						// Seta os parametros para rodar a funcionalidade
						taxaEntrega.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());

						taxaEntrega.addParametro("anoMesFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getAnoMesReferencia());
					...
					case Funcionalidade.EMITIR_CONTAS:
						...
						// Seta os parametros para rodar a funcionalidade
						emitirContas.addParametro("anoMesFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getAnoMesReferencia());

						emitirContas.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());
					...
					case Funcionalidade.EMITIR_EXTRATO_CONSUMO_IMOVEL_CONDOMINIO:
						TarefaBatchEmitirExtratoConsumoImovelCondominio emitirExtratoConsumoImovelCondominio = new TarefaBatchEmitirExtratoConsumoImovelCondominio(	processoIniciado.getUsuario(),
																																									funcionalidadeIniciada.getId());

						// Seta os parametros para rodar a funcionalidade
						emitirExtratoConsumoImovelCondominio.addParametro("anoMesFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getAnoMesReferencia());

						emitirExtratoConsumoImovelCondominio.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());
					...
					case Funcionalidade.GERAR_CREDITO_SITUACAO_ESPECIAL_FATURAMENTO:
						...
						TarefaBatchGerarCreditoSituacaoEspecialFaturamento gerarCreditoSituacaoespecialFaturamento = new TarefaBatchGerarCreditoSituacaoEspecialFaturamento(processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());

						// FATURAMENTO_GRUPO
						gerarCreditoSituacaoespecialFaturamento.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());

						// ROTAS SELECIONADAS
						gerarCreditoSituacaoespecialFaturamento.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoFaturamentoAtivCronRota);
					...
					case Funcionalidade.SUPENDER_IMOVEL_EM_PROGRAMA_ESPECIAL:
						...
						TarefaBatchSuspenderImovelEmProgramaEspecial tarefaBatchSuspenderImovelEmProgramaEspecial = new TarefaBatchSuspenderImovelEmProgramaEspecial(	processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());

						tarefaBatchSuspenderImovelEmProgramaEspecial.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoFaturamentoAtivCronRota);
					...
					case Funcionalidade.GERAR_RESUMO_SIMULACAO_FATURAMENTO:
						TarefaBatchGerarResumoSimulacaoFaturamento tarefaBatchGerarResumoSimulacaoFaturamento = new TarefaBatchGerarResumoSimulacaoFaturamento(	processoIniciado.getUsuario(),
																																								funcionalidadeIniciada.getId());

						// Seta os parametros para rodar a funcionalidade
						tarefaBatchGerarResumoSimulacaoFaturamento.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());
						...
						tarefaBatchGerarResumoSimulacaoFaturamento.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoFaturamentoAtivCronRota);
					...
					case Funcionalidade.GERAR_DECLARACAO_QUITACAO_ANUAL_DEBITOS:
						...
						TarefaBatchGerarDadosDeclaracaoQuitacaoAnualDebitos tarefaBatchGerarDadosDeclaracaoQuitacaoAnualDebitos = new TarefaBatchGerarDadosDeclaracaoQuitacaoAnualDebitos(	processoIniciado.getUsuario(),
																																															funcionalidadeIniciada.getId());

						ArrayList<Rota> colecaoRota = new ArrayList<Rota>();
						for (Object o : colecaoFaturamentoAtivCronRota) {
							Object[] array = (Object[]) o;
							colecaoRota.add((Rota) array[1]);
						}

						tarefaBatchGerarDadosDeclaracaoQuitacaoAnualDebitos.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoRota);

						SistemaParametro sistemaParametroQuitacao = getControladorUtil().pesquisarParametrosDoSistema();

						// Altera o ano mes de referencia para que ele seja
						// igual ao ano mes referencia do grupo
						sistemaParametroQuitacao.setAnoMesFaturamento(faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getAnoMesReferencia());
					...
					case Funcionalidade.GERAR_ARQUIVO_TEXTO_DECLARACAO_QUITACAO_ANUAL_DEBITOS:
						...
						TarefaBatchGerarArquivoTextoDeclaracaoQuitacaoAnualDebitos tarefaBatchGerarArquivoTextoDeclaracaoQuitacaoAnualDebitos = new TarefaBatchGerarArquivoTextoDeclaracaoQuitacaoAnualDebitos(	processoIniciado.getUsuario(),
																																																				funcionalidadeIniciada.getId());

						tarefaBatchGerarArquivoTextoDeclaracaoQuitacaoAnualDebitos.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());
					...
					case Funcionalidade.GERAR_TAXA_PERCENTUAL_TARIFA_MINIMA_CORTADO:
						...
						TarefaBatchGerarTaxaPercentualTarifaMinimaCortado tarefaBatchGerarTaxaPercentualTarifaMinimaCortado = new TarefaBatchGerarTaxaPercentualTarifaMinimaCortado(processoIniciado.getUsuario(),
																																													funcionalidadeIniciada.getId());

						// FATURAMENTO_GRUPO
						tarefaBatchGerarTaxaPercentualTarifaMinimaCortado.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());

						// ROTAS SELECIONADAS
						tarefaBatchGerarTaxaPercentualTarifaMinimaCortado.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoFaturamentoAtivCronRota);

						tarefaBatchGerarTaxaPercentualTarifaMinimaCortado.addParametro("atividade", faturamentoAtividadeCronograma.getFaturamentoAtividade().getId());
					...
					case Funcionalidade.VERIFICAR_FATURAMENTO_IMOVEIS_CORTADOS:
						...
						TarefaBatchVerificarFaturamentoImoveisCortados tarefaBatchVerificarFaturamentoImoveisCortados = new TarefaBatchVerificarFaturamentoImoveisCortados(	processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());

						// FATURAMENTO_GRUPO
						tarefaBatchVerificarFaturamentoImoveisCortados.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());

						// ROTAS SELECIONADAS
						tarefaBatchVerificarFaturamentoImoveisCortados.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoFaturamentoAtivCronRota);

						tarefaBatchVerificarFaturamentoImoveisCortados.addParametro("atividade", faturamentoAtividadeCronograma.getFaturamentoAtividade().getId());
					...
					case Funcionalidade.GERAR_CREDITO_A_REALIZAR_CONTRATO_DEMANDA_RESIDENCIAL:
						/**
						 * [UC1236] Gerar Cr�dito a Realizar Contrato de
						 * Demanda Im�vel Residencial
						 * 
						 * @author Rafael Pinto
						 * @date 06/10/2011
						 */
						TarefaBatchGerarCreditoARealizarContratoDemandaImovelResidencial tarefaBatchGerarCreditoARealizarContratoDemandaImovelResidencial = new TarefaBatchGerarCreditoARealizarContratoDemandaImovelResidencial(	processoIniciado.getUsuario(),
																																																									funcionalidadeIniciada.getId());

						// FATURAMENTO_GRUPO
						tarefaBatchGerarCreditoARealizarContratoDemandaImovelResidencial.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());

						// ROTAS SELECIONADAS
						tarefaBatchGerarCreditoARealizarContratoDemandaImovelResidencial.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoFaturamentoAtivCronRota);

						tarefaBatchGerarCreditoARealizarContratoDemandaImovelResidencial.addParametro("anoMesFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getAnoMesReferencia());

						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(tarefaBatchGerarCreditoARealizarContratoDemandaImovelResidencial));
					...
					case Funcionalidade.SUSPENDER_LEITURA_PARA_IMOVEL_COM_HIDROMETRO_RETIRADO:
						/**
						 * [UC1216] Suspender Leitura para Im�vel com
						 * Hidr�metro Retirado
						 * 
						 * @author Vivianne Sousa
						 * @date 23/08/2011
						 */
						TarefaBatchSuspenderLeituraParaImovelComHidrometroRetirado tarefaBatchSuspenderLeituraParaImovelComHidrometroRetirado = new TarefaBatchSuspenderLeituraParaImovelComHidrometroRetirado(	processoIniciado.getUsuario(),
																																																				funcionalidadeIniciada.getId());

						// FATURAMENTO_GRUPO
						tarefaBatchSuspenderLeituraParaImovelComHidrometroRetirado.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());

						// ROTAS SELECIONADAS
						tarefaBatchSuspenderLeituraParaImovelComHidrometroRetirado.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoFaturamentoAtivCronRota);

						tarefaBatchSuspenderLeituraParaImovelComHidrometroRetirado.addParametro("anoMesFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getAnoMesReferencia());
					...
					case Funcionalidade.SUSPENDER_LEITURA_PARA_IMOVEL_COM_CONSUMO_REAL_NAO_SUPERIOR_A_10:
						 ...
						 * @date 23/08/2011
						 */
						TarefaBatchSuspenderLeituraParaImovelComConsumoRealNaoSuperiorA10 tarefaBatchSuspenderLeituraParaImovelComConsumoRealNaoSuperiorA10 = new TarefaBatchSuspenderLeituraParaImovelComConsumoRealNaoSuperiorA10(processoIniciado.getUsuario(),
																																																									funcionalidadeIniciada.getId());

						// FATURAMENTO_GRUPO
						tarefaBatchSuspenderLeituraParaImovelComConsumoRealNaoSuperiorA10.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());

						// ROTAS SELECIONADAS
						tarefaBatchSuspenderLeituraParaImovelComConsumoRealNaoSuperiorA10.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoFaturamentoAtivCronRota);

						tarefaBatchSuspenderLeituraParaImovelComConsumoRealNaoSuperiorA10.addParametro("anoMesFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getAnoMesReferencia());

						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(tarefaBatchSuspenderLeituraParaImovelComConsumoRealNaoSuperiorA10));
					...
					case Funcionalidade.GERAR_ATIVIDADE_ACAO_COBRANCA:
						...
						if (faturamentoAtividadeCronograma.getFaturamentoAtividade().getCobrancaAcao() != null) {

							filtroCobrancaAcao.adicionarParametro(new ParametroSimples(	FiltroCobrancaAcao.ID,
																						faturamentoAtividadeCronograma.getFaturamentoAtividade().getCobrancaAcao().getId()));
						} else {
						...
						Collection colecaoCobrancaAtividade = this.getControladorUtil().pesquisar(filtroCobrancaAtividade, CobrancaAtividade.class.getName());
						...
						filtroCobrancaAcaoAtividadeComando.adicionarParametro(new ParametroSimples(	FiltroCobrancaAcaoAtividadeComando.FATURAMENTO_ATIVIDADE_CRONOGRAMA_ID,
						...
						filtroCobrancaAcaoAtividadeComando.adicionarCaminhoParaCarregamentoEntidade(FiltroCobrancaAcaoAtividadeComando.IMOVEL);

						CobrancaAcaoAtividadeComando cobrancaAcaoAtividadeComandoNaBase = (CobrancaAcaoAtividadeComando) Util.retonarObjetoDeColecao(getControladorUtil().pesquisar(filtroCobrancaAcaoAtividadeComando, CobrancaAcaoAtividadeComando.class.getName()));
						if (cobrancaAcaoAtividadeComandoNaBase == null) {
							// inserir comando de cobran�a eventual
							...
							// Data encerramento prevista ser� igual a data
							// corrente mais o n�mero de dias de validade da
							// a��o de cobran�a
							if (cobrancaAcao.getNumeroDiasValidade() != null) {

								cobrancaAcaoAtividadeComando.setDataEncerramentoPrevista(Util.adicionarNumeroDiasDeUmaData(new Date(), cobrancaAcao.getNumeroDiasValidade()));
							} else {

								cobrancaAcaoAtividadeComando.setDataEncerramentoPrevista(new Date());
							}

							// usuario
							cobrancaAcaoAtividadeComando.setUsuario(Usuario.USUARIO_BATCH);
							cobrancaAcaoAtividadeComando.setDescricaoTitulo("GRUPO FATURAMENTO "
									+ faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getId()
									+ ", M�S/ANO "
						...
						CobrancaDocumentoControleGeracao cobrancaDocumentoControleGeracao = new CobrancaDocumentoControleGeracao(	0,
																																	0,
																																	new BigDecimal("0.00"),
																																	new Date(),
																																	processoIniciado,
																																	null,
																																	cobrancaAcaoAtividadeComandoNaBase);

						Integer idCobrancaDocumentoControleGeracao = (Integer) getControladorUtil().inserir(cobrancaDocumentoControleGeracao);
						// //////////////////////////////////////////////////////////

						// Seta os parametros para rodar a funcionalidade
						// acaoCobranca.addParametro("atividade",
						...
						acaoCobranca.addParametro("anoMesReferenciaFinal", (new Integer(Util.subtrairMesDoAnoMes(getControladorUtil().pesquisarParametrosDoSistema().getAnoMesFaturamento(), 1))));
						...
						acaoCobranca.addParametro("dataVencimentoFinal", Util.subtrairNumeroDiasDeUmaData(new Date(), getControladorUtil().pesquisarParametrosDoSistema().getNumeroDiasVencimentoCobranca()));
					...
					case Funcionalidade.ATUALIZAR_COMANDO_ATIVIDADE_ACAO_COBRANCA:
						TarefaBatchAtualizarComandoAtividadeAcaoCobranca atualizarComandoAtividadeAcaoCobranca = new TarefaBatchAtualizarComandoAtividadeAcaoCobranca(	processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());
						...
						filtroCobrancaAcaoAtividadeComand.adicionarParametro(new ParametroSimples(	FiltroCobrancaAcaoAtividadeComando.FATURAMENTO_ATIVIDADE_CRONOGRAMA_ID,
																									faturamentoAtividadeCronograma.getId()));
						...
						CobrancaAcaoAtividadeComando cobrancaAcaoAtividadeComando = (CobrancaAcaoAtividadeComando) Util.retonarObjetoDeColecao(getControladorUtil().pesquisar(filtroCobrancaAcaoAtividadeComand, CobrancaAcaoAtividadeComando.class.getName()));

						atualizarComandoAtividadeAcaoCobranca.addParametro("anoMesReferenciaCicloCobranca", getControladorUtil().pesquisarParametrosDoSistema().getAnoMesFaturamento());

						atualizarComandoAtividadeAcaoCobranca.addParametro("comandoAtividadeAcaoComando", cobrancaAcaoAtividadeComando);

						atualizarComandoAtividadeAcaoCobranca.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, Collections.singletonList(cobrancaAcaoAtividadeComando.getId()));
					...
					case Funcionalidade.PARALISAR_FATURAMENTO:
						...
						TarefaBatchParalisarFaturamento paralisarFaturamento = new TarefaBatchParalisarFaturamento(	processoIniciado.getUsuario(),
																													funcionalidadeIniciada.getId());

						// Seta os parametros para rodar a funcionalidade
						paralisarFaturamento.addParametro("anoMesFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getAnoMesReferencia());
						paralisarFaturamento.addParametro("idFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getId());

						paralisarFaturamento.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoFaturamentoAtivCronRota);

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(paralisarFaturamento));
					...
					case Funcionalidade.ENVIO_EMAIL_CONTA_PARA_CLIENTE:
						...
						// Seta os parametros para rodar a funcionalidade
						// tarefaBatchEnvioEmailContaParaCliente.addParametro("anoMesFaturamentoGrupo",
						// faturamentoAtividadeCronograma
						// .getFaturamentoGrupoCronogramaMensal()
						// .getFaturamentoGrupo()
						// .getAnoMesReferencia());

						tarefaBatchEnvioEmailContaParaCliente.addParametro("faturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo());
					...
					case Funcionalidade.PREPARAR_DADOS_SMS_EMAIL_COBRANCA_CONTA:

						TarefaBatchPrepararDadosSMSEmailCobrancaConta tarefaBatchPrepararDadosSMSEmailCobrancaConta = new TarefaBatchPrepararDadosSMSEmailCobrancaConta(processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());

						// Seta os parametros para rodar a funcionalidade
						tarefaBatchPrepararDadosSMSEmailCobrancaConta.addParametro("anoMesFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getAnoMesReferencia());
						tarefaBatchPrepararDadosSMSEmailCobrancaConta.addParametro("idFaturamentoGrupo", faturamentoAtividadeCronograma.getFaturamentoGrupoCronogramaMensal().getFaturamentoGrupo().getId());

						// Consultas
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(tarefaBatchPrepararDadosSMSEmailCobrancaConta));
	...
	public Integer inserirProcessoIniciadoCobrancaComandado(Collection<Integer> idsCronograma,
			Collection<Integer> idsEventuais, Usuario usuario) throws ControladorException {
		Integer codigoProcessoIniciadoGerado = null;
		try {
	...
	private Integer inserirProcessoCobrancaAtividadeCronograma(Usuario usuario, Integer codigoProcessoIniciadoGerado,
			Iterator<Integer> iteratorCronograma) throws ControladorException, ParseException, IOException {
		...
		while (iteratorCronograma.hasNext()) {
			...
			CobrancaAcaoAtividadeCronograma cobrancaAcaoAtividadeCronograma = getControladorCobranca().pesquisarCobrancaAcaoAtividadeCronogramaId(codigoCobrancaAcaoAtividadeCronograma);
			...
			processoIniciado.setCodigoGrupoProcesso(cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaGrupoCronogramaMes().getCobrancaGrupo().getId());
			...
			while (iterator.hasNext()) {
				...
				filtroRota.adicionarParametro(new ParametroSimples(	FiltroRota.COBRANCA_GRUPO_ID,
																	cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaGrupoCronogramaMes().getCobrancaGrupo().getId()));
				if (cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaAcao().getId() != null
					...
					filtroRota.adicionarParametro(new ParametroSimplesDiferenteDe(	FiltroRota.EMPRESA_COBRANCA_ID,
																					1));
				}
				...
				case Funcionalidade.GERAR_ATIVIDADE_ACAO_COBRANCA:
					...
					// Seta os parametros para rodar a funcionalidade
					acaoCobranca.addParametro("grupoCobranca", cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaGrupoCronogramaMes().getCobrancaGrupo());

					acaoCobranca.addParametro("anoMesReferenciaCicloCobranca", cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaGrupoCronogramaMes().getAnoMesReferencia());
					...
					if (colecaoRotas != null && !colecaoRotas.isEmpty()) {
						...
						acaoCobranca.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, Collections.singletonList(cobrancaAcaoAtividadeCronograma.getId()));
					}
				...
				case Funcionalidade.EMITIR_BOLETIM_CADASTRO:
					...
					tarefaBatchEmitirBoletimCadastro.addParametro("acaoCobranca", cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaAcao());
				...
				case Funcionalidade.ATUALIZAR_COMANDO_ATIVIDADE_ACAO_COBRANCA:
					...
					// Seta os parametros para rodar a funcionalidade
					atualizarComandoAtividadeAcaoCobranca.addParametro("grupoCobranca", cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaGrupoCronogramaMes().getCobrancaGrupo());

					atualizarComandoAtividadeAcaoCobranca.addParametro("anoMesReferenciaCicloCobranca", cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaGrupoCronogramaMes().getAnoMesReferencia());

					atualizarComandoAtividadeAcaoCobranca.addParametro("comandoAtividadeAcaoCobranca", cobrancaAcaoAtividadeCronograma);

					atualizarComandoAtividadeAcaoCobranca.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, Collections.singletonList(cobrancaAcaoAtividadeCronograma.getId()));

					atualizarComandoAtividadeAcaoCobranca.addParametro("acaoCobranca", cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaAcao());
				...
				case Funcionalidade.EMITIR_DOCUMENTO_COBRANCA:
					...
					// Seta os parametros para rodar a funcionalidade
					emitirDocumentoCobranca.addParametro("grupoCobranca", cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getCobrancaGrupoCronogramaMes().getCobrancaGrupo());

					emitirDocumentoCobranca.addParametro("comandoAtividadeAcaoCobranca", cobrancaAcaoAtividadeCronograma);

					emitirDocumentoCobranca.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, Collections.singletonList(cobrancaAcaoAtividadeCronograma.getId()));
				...
				case Funcionalidade.ENCERRAR_ORDENS_SERVICO_ACAO_COBRANCA:
					...
					TarefaBatchProcessarEncerramentoOSAcaoCobranca tarefaBatchProcessarEncerramentoOSAcaoCobranca = new TarefaBatchProcessarEncerramentoOSAcaoCobranca(	processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());

					Collection<Integer> colecaoOrdemServicoParaEncerrar = getControladorCobranca().pesquisarOrdemServicoParaEncerrar(cobrancaAcaoAtividadeCronograma.getCobrancaAcaoCronograma().getId());

					if (colecaoOrdemServicoParaEncerrar != null && !colecaoOrdemServicoParaEncerrar.isEmpty()) {

						tarefaBatchProcessarEncerramentoOSAcaoCobranca.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoOrdemServicoParaEncerrar);
	...
	private Integer inserirProcessoCobrancaAtividadeEventual(Usuario usuario, Integer codigoProcessoIniciadoGerado,
			Iterator<Integer> iteratorAtividade) throws ControladorException, ParseException, IOException {
		...
		try {
			...
			while (iteratorAtividade.hasNext()) {
				...
				filtroCobrancaAcaoAtividadeComando.adicionarCaminhoParaCarregamentoEntidade("cobrancaAcao.cobrancaAcaoPredecessoraAlternativa");
				filtroCobrancaAcaoAtividadeComando.adicionarCaminhoParaCarregamentoEntidade("cobrancaAcao.cobrancaAcaoPredecessoraAlternativa.documentoTipo");
				filtroCobrancaAcaoAtividadeComando.adicionarCaminhoParaCarregamentoEntidade("cobrancaCriterio.resolucaoDiretoria");
				filtroCobrancaAcaoAtividadeComando.adicionarCaminhoParaCarregamentoEntidade(FiltroCobrancaAcaoAtividadeComando.IMOVEL);

				CobrancaAcaoAtividadeComando cobrancaAcaoAtividadeComando = (CobrancaAcaoAtividadeComando) Util.retonarObjetoDeColecao(getControladorUtil().pesquisar(filtroCobrancaAcaoAtividadeComando, CobrancaAcaoAtividadeComando.class.getName()));
				...
				if (cobrancaAcaoAtividadeComando.getCobrancaGrupo() != null
						&& !cobrancaAcaoAtividadeComando.getCobrancaGrupo().equals("")) {
				...
				while (iterator.hasNext()) {
					...
					if (cobrancaAcaoAtividadeComando.getCobrancaGrupo() != null) {
						...
						FiltroRota filtroRota = new FiltroRota();
						filtroRota.adicionarParametro(new ParametroSimples(	FiltroRota.COBRANCA_GRUPO_ID,
																			cobrancaAcaoAtividadeComando.getCobrancaGrupo().getId()));
					...
					case Funcionalidade.GERAR_ATIVIDADE_ACAO_COBRANCA:
						...
						acaoCobranca.addParametro("quantidadeDiasVencimento", cobrancaAcaoAtividadeComando.getQuantidadeDiasVencimento());
					...
					case Funcionalidade.EMITIR_DOCUMENTO_COBRANCA:
						...
						emitirDocumentoCobranca.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, Collections.singletonList(cobrancaAcaoAtividadeComando.getId()));
					...
					case Funcionalidade.PREPARAR_DADOS_SMS_EMAIL_ACAO_COBRANCA:	
						
						TarefaBatchPrepararDadosSMSEmailAcaoCobranca tarefaBatchPrepararDadosSMSEmailAcaoCobranca = new TarefaBatchPrepararDadosSMSEmailAcaoCobranca(
								processoIniciado.getUsuario(),
								funcionalidadeIniciada.getId()		
						);
						
						
						tarefaBatchPrepararDadosSMSEmailAcaoCobranca.addParametro("cobrancaAcaoAtividadeComando",cobrancaAcaoAtividadeComando.getId());
						tarefaBatchPrepararDadosSMSEmailAcaoCobranca.addParametro("acaoCobranca", cobrancaAcaoAtividadeComando.getCobrancaAcao().getId());
						
						funcionalidadeIniciada.setTarefaBatch(IoUtil
								.transformarObjetoParaBytes(tarefaBatchPrepararDadosSMSEmailAcaoCobranca));
			...
			if (colecaoFuncionaldadesIniciadasFalha != null && !colecaoFuncionaldadesIniciadasFalha.isEmpty()) {
				...
				while (iterator.hasNext()) {
					...
					filtroFuncionalidadeIniciada.adicionarParametro(new ParametroSimples(	FiltroFuncionalidadeIniciada.ID,
																							funcionalidadeIniciadaPesquisa.getId()));
					...
					Collection<FuncionalidadeIniciada> colFuin = getControladorUtil().pesquisar(filtroFuncionalidadeIniciada, FuncionalidadeIniciada.class.getName());
				...
				if (colecaoFuncionaldadesIniciadasParaEncerramento != null) {
					...
					while (iterator.hasNext()) {
						...
						// Chama o metodo para atualizar a situacao sem
						// transacao
						// para nao
						...
						// Finaliza com alguma lagica de negacio depois que
						// todas as
	 ...
	 * @return
	 */
	public int iniciarUnidadeProcessamentoBatch(int idFuncionalidadeIniciada, int idUnidadeProcessamento,
			int codigoRealUnidadeProcessamento) throws ControladorException {
		...
		if (idFuncionalidadeIniciada > 0) {
			...
			try {
				...
				if (funcionalidadesIniciadasComErro > 0) {
					throw new ControladorException("Esta Unidade n�o ser� executada porque o processo est� conclu�do com erro");
				}
				FiltroFuncionalidadeIniciada filtroFuncionalidadeIniciada = new FiltroFuncionalidadeIniciada();
				filtroFuncionalidadeIniciada.adicionarParametro(new ParametroSimples(	FiltroFuncionalidadeIniciada.ID,
	...
	public void encerrarUnidadeProcessamentoBatch(Throwable excecao, int idUnidadeIniciada, boolean executouComErro)
		...
		if (idUnidadeIniciada != 0) {
			...
			filtroUnidadeIniciada.adicionarParametro(new ParametroSimples(	FiltroFuncionalidadeIniciada.ID,
																			idUnidadeIniciada));

			UnidadeIniciada unidadeIniciada = (UnidadeIniciada) Util.retonarObjetoDeColecao(getControladorUtil().pesquisar(filtroUnidadeIniciada, UnidadeIniciada.class.getName()));
			...
			if ((!executouComErro)
					|| (excecao != null && excecao.getMessage() != null && excecao.getMessage().equals("Unidade j� executada"))) {
	 ...
	 * @throws ControladorException
	 */
	public void atualizarColecaoObjetoParaBatchSemTransacao(Collection<? extends Object> colecaoObjetos)
																										throws ControladorException {
	...
	public Collection<FuncionalidadeIniciada> verificarFuncionalidadesIniciadasProntasParaExecucao()
		...
		try {
			...
			for (Object[] objects : funcionalidadesIniciadasExecucao) {
				...
				int quantidadeFuncionalidadesForaOrdem = repositorioBatch.pesquisarQuantidadeFuncionaldadesIniciadasForaOrdemExecucao((int) ((Short) objects[1]), funcionalidadeIniciada.getProcessoIniciado().getId());
	 ...
	 * @throws ControladorException
	 */
	public void encerrarFuncionalidadeIniciadaRelatorio(int idFuncionalidadeIniciada, boolean concluiuComErro)
																												throws ControladorException {
	...
	/**
	 * Remove uma cole��o de objetos gen�ricos na base com um flush para
	 * cada 50 registros removidos.
	 * 
	 * @author S�vio Luiz
	 * @date 31/03/2010
	 * 
			...
			while (iterator.hasNext()) {
				...
				try {
					...
					case Funcionalidade.APAGAR_RESUMO_DEVEDORES_DUVIDOSOS:
						TarefaBatchApagarResumoDevedoresDuvidosos dadosApagarResumoDevedoresDuvidosos = new TarefaBatchApagarResumoDevedoresDuvidosos(	processoIniciado.getUsuario(),
																																						funcionalidadeIniciada.getId());
						...
						Collection<Integer> colecaoIdsLocalidadesApagarResumoDevedoresDuvidosos = getControladorFinanceiro().pesquisarIdsLocalidadesParaGerarResumoDevedoresDuvidosos(anoMesReferenciaContabilApagar);
						...
						// Seta os parametros para rodar a funcionalidade
						dadosApagarResumoDevedoresDuvidosos.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesApagarResumoDevedoresDuvidosos);
					...
					case Funcionalidade.GERAR_RESUMO_DEVEDORES_DUVIDOSOS:
						...
						Integer idTipoPerda = new Integer((String) dadosProcessamento.get("idTipoPerda"));

						Collection<Integer> colecaoIdsLocalidadesGerarResumoDevedoresDuvidosos = getControladorFinanceiro().pesquisarIdsLocalidadesParaGerarResumoDevedoresDuvidosos(anoMesReferenciaContabil);

						dadosGerarResumoDevedoresDuvidosos.addParametro("anoMesReferenciaContabil", anoMesReferenciaContabil);

						dadosGerarResumoDevedoresDuvidosos.addParametro("idTipoPerda", idTipoPerda);
					...
					case Funcionalidade.ATUALIZAR_RESUMO_DEVEDORES_DUVIDOSOS:
						TarefaBatchAtualizarResumoDevedoresDuvidosos dadosAtualizarResumoDevedoresDuvidosos = new TarefaBatchAtualizarResumoDevedoresDuvidosos(	processoIniciado.getUsuario(),
																																								funcionalidadeIniciada.getId());
						...
						dadosAtualizarResumoDevedoresDuvidosos.addParametro("idTipoPerda", idTipoPerdaAtualizar);
					...
					case Funcionalidade.GERAR_LANCAMENTOS_CONTABEIS_DEVEDORES_DUVIDOSOS:
						TarefaBatchGerarLancamentosContabeisDevedoresDuvidosos dadosGerarLancamentosContabeisDevedoresDuvidosos = new TarefaBatchGerarLancamentosContabeisDevedoresDuvidosos(	processoIniciado.getUsuario(),
																																																funcionalidadeIniciada.getId());

						Integer anoMesReferenciaContabilLancamentosContabeis = new Integer((String) dadosProcessamento.get("anoMesReferenciaContabil"));

						String tipoPerda = (String) dadosProcessamento.get("idTipoPerda");
						Integer idTipoPerdaLancamentos = tipoPerda != null ? new Integer(tipoPerda) : null;

						Collection<Integer> colecaoIdsLocalidadesGerarLancamentosContabeisDevedoresDuvidosos = getControladorFinanceiro().pesquisarIdsLocalidadesParaGerarResumoDevedoresDuvidosos(anoMesReferenciaContabilLancamentosContabeis);
						...
						dadosGerarLancamentosContabeisDevedoresDuvidosos.addParametro("idTipoPerdaLancamentos", idTipoPerdaLancamentos);

						// Seta os parametros para rodar a funcionalidade
						dadosGerarLancamentosContabeisDevedoresDuvidosos.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoIdsLocalidadesGerarLancamentosContabeisDevedoresDuvidosos);
					...
					case Funcionalidade.GERAR_LANCAMENTOS_CONTABEIS_FATURAMENTO:
						...
						Integer anoMesReferenciaFaturamentoLancamentosContabeis = new Integer((String) dadosProcessamento.get("anoMesReferenciaContabil"));

						FiltroLocalidade filtroLocalidadeLancamentoContabeisFaturamento = new FiltroLocalidade();

						Collection<Localidade> colecaoIdsLocalidadesGerarLancamentosContabeisFaturamento = getControladorUtil().pesquisar(filtroLocalidadeLancamentoContabeisFaturamento, Localidade.class.getName());

						dadosGerarLancamentosContabeisFaturamento.addParametro("anoMesFaturamento", anoMesReferenciaFaturamentoLancamentosContabeis);
					...
					case Funcionalidade.GERAR_LANCAMENTOS_CONTABEIS_ARRECADACAO:
						...
						Integer anoMesReferenciaArrecadacaoLancamentosContabeis = new Integer((String) dadosProcessamento.get("anoMesReferenciaContabil"));

						FiltroLocalidade filtroLocalidadeLancamentoContabeisArrecadacao = new FiltroLocalidade();

						Collection<Localidade> colecaoIdsLocalidadesGerarLancamentosContabeisArrecadacao = getControladorUtil().pesquisar(filtroLocalidadeLancamentoContabeisArrecadacao, Localidade.class.getName());

						dadosGerarLancamentosContabeisArrecadacao.addParametro("anoMesArrecadacao", anoMesReferenciaArrecadacaoLancamentosContabeis);
	...
	public void reiniciarFuncionalidadesIniciadas(String[] idsFuncionalidadesIniciadas, Integer idProcessoIniciado)
		...
		while (i < idsFuncionalidadesIniciadas.length) {
			...
			// Limpa a cole��o das funcionalidades j� executadas da
			// continua��o
			try {
				Object tarefa = IoUtil.transformarBytesParaObjeto(funcionalidadeIniciada.getTarefaBatch());
		...
		FiltroProcessoIniciado filtroProcessoIniciado = new FiltroProcessoIniciado();
		filtroProcessoIniciado.adicionarParametro(new ParametroSimples(	FiltroProcessoIniciado.ID,
																		idProcessoIniciado));
	...
	public void removerColecaoGuiaPagamentoCategoriaParaBatch(
			Collection<GuiaPagamentoCategoria> colecaoGuiaPagamentoCategoria) throws ControladorException {
	 ...
	 * @throws ControladorException
	 */
	public void removerColecaoClienteGuiaPagamentoParaBatch(Collection<ClienteGuiaPagamento> colecaoClienteGuiaPagamento)
																															throws ControladorException {
	 ...
	 * @throws ControladorException
	 */
	public void removerColecaoGuiaPagamentoParaBatch(Collection<GuiaPagamento> colecaoGuiaPagamento)
																									throws ControladorException {
	 ...
	 * @throws ControladorException
	 */
	public void removerColecaoDebitoACobrarParaBatch(Collection<DebitoACobrar> colecaoDebitoACobrar)
																									throws ControladorException {
	...
	public void removerColecaoDebitoACobrarCategoriaParaBatch(
			Collection<DebitoACobrarCategoria> colecaoDebitoACobrarCategoria) throws ControladorException {
	 ...
	 * @throws ControladorException
	 */
	public void removerColecaoContaCategoriaParaBatch(Collection<ContaCategoria> colecaoContaCategoria)
																										throws ControladorException {
	...
	public void removerColecaoContaCategoriaConsumoFaixaParaBatch(
			Collection<ContaCategoriaConsumoFaixa> colecaoContaCategoriaConsumoFaixa) throws ControladorException {
	 ...
	 * @throws ControladorException
	 */
	public void removerColecaoCreditoRealizadoParaBatch(Collection<CreditoRealizado> colecaoCreditoRealizado)
																												throws ControladorException {
	 ...
	 * @throws ControladorException
	 */
	public void removerColecaoDebitoCobradoParaBatch(Collection<DebitoCobrado> colecaoDebitoCobrado)
																									throws ControladorException {
	...
	public void removerColecaoDebitoCobradoCategoriaParaBatch(
			Collection<DebitoCobradoCategoria> colecaoDebitoCobradoCategoria) throws ControladorException {
	...
	public void removerColecaoCreditoRealizadoCategoriaParaBatch(
			Collection<CreditoRealizadoCategoria> colecaoCreditoRealizadoCategoria) throws ControladorException {
	...
	public void removerColecaoContaImpostosDeduzidosParaBatch(
			Collection<ContaImpostosDeduzidos> colecaoContaImpostosDeduzidos) throws ControladorException {
	 ...
	 * @throws ControladorException
	 */
	public void removerColecaoClienteContaParaBatch(Collection<ClienteConta> colecaoClienteConta)
																									throws ControladorException {
	 ...
	 * @throws ControladorException
	 */
	public void removerColecaoCreditoARealizarCategoriaParaBatch(Collection<Integer> colecaoIdsCreditoARealizar)
																												throws ControladorException {
	 ...
	 * @throws ControladorException
	 */
	public Integer inserirProcessoIniciadoParametrosLivres(Map parametros, int idProcesso, Usuario usuario)
																											throws ControladorException {
		Integer codigoProcessoIniciadoGerado = null;
		...
		try {
			...
			// Todos os processo serao iniciados com a situaaao EM_ESPE	RA para q
			// sejam executados o mais cedo possavel
			ProcessoSituacao processoSituacao = new ProcessoSituacao();
			...
			if (parametros.get("complementoProcesso") != null) {
				processoIniciado.setComplementoProcesso((String) parametros.get("complementoProcesso"));
			...
			while (iterator.hasNext()) {
				...
				try {
					...
					case Funcionalidade.GERAR_RELATORIO_CONTAS_BAIXADAS_CONTABILMENTE:
						TarefaBatchRelatorioContasBaixadasContabilmente txtContasBaixadasContabilmente = new TarefaBatchRelatorioContasBaixadasContabilmente(	processoIniciado.getUsuario(),
																																								funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_CARTAS_CAMPANHA_SOLIDARIEDADE_CRIANCA_PARA_NEGOCIACAO_A_VISTA:
						...
						if (acao.equals("1")) {
							// se gerar cartas
							TarefaBatchGerarCartasCampanhaSolidariedadeCriancaParaNegociacaoAVista cartasGerar = new TarefaBatchGerarCartasCampanhaSolidariedadeCriancaParaNegociacaoAVista(processoIniciado.getUsuario(),
																																															funcionalidadeIniciada.getId());
							...
							// se emitir cartas
							TarefaBatchEmitirCartasCampanhaSolidariedadeCriancaParaNegociacaoAVista cartasEmitir = new TarefaBatchEmitirCartasCampanhaSolidariedadeCriancaParaNegociacaoAVista(	processoIniciado.getUsuario(),
																																																funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_TALELAS_TEMPORARIAS_ATUALIZACAO_CADASTRAL:
						...
						TarefaBatchGerarTabelasTemporariasAtualizacaoCadastral tabela = new TarefaBatchGerarTabelasTemporariasAtualizacaoCadastral(	processoIniciado.getUsuario(),
																																					funcionalidadeIniciada.getId());
						...
						if (helper.getColecaoMatriculas() == null || helper.getColecaoMatriculas().isEmpty()) {
							...
							idsSetor = getControladorCadastro().pesquisarSetorComercialGeracaoTabelasTemporarias(helper);
						}
					...
					case Funcionalidade.GERAR_ARQUIVO_TEXTO_ATUALIZACAO_CADASTRAL:
						TarefaBatchGerarArquivoTextoAtualizacaoCadastral arquivoTexto = new TarefaBatchGerarArquivoTextoAtualizacaoCadastral(	processoIniciado.getUsuario(),
																																				funcionalidadeIniciada.getId());
					...
					case Funcionalidade.GERAR_RELATORIO_CONTAS:
						TarefaBatchGerarContas contas = new TarefaBatchGerarContas(	usuario,
							funcionalidadeIniciada.getId());
						
						contas.addParametro("relatorio", parametros.get("relatorio"));
						
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(contas));
					...
					case Funcionalidade.PRESCREVER_DEBITOS_IMOVEIS_PUBLICOS:
						TarefaBatchPrescreverDebitosImoveisPublicosManual prescricao = new TarefaBatchPrescreverDebitosImoveisPublicosManual(	processoIniciado.getUsuario(),
																																				funcionalidadeIniciada.getId());
					...
					case Funcionalidade.PROCESSAR_COMANDO_GERADO:
						...
						TarefaBatchProcessarComandoGerado tarefaBatchProcessarComandoGerado = new TarefaBatchProcessarComandoGerado(processoIniciado.getUsuario(),
																																	funcionalidadeIniciada.getId());
						...
						if (tarifaSocialComandoCarta.getLocalidade() != null
								&& tarifaSocialComandoCarta.getLocalidade().getId() != null) {
							...
							idsLocalidade = getControladorCadastro().pesquisarLocalidadesPorGerenciaEUnidade(tarifaSocialComandoCarta.getGerenciaRegional().getId(), tarifaSocialComandoCarta.getUnidadeNegocio().getId());
					...
					case Funcionalidade.ENCERRAR_COMANDO_DE_COBRANCA_POR_EMPRESA:
						TarefaBatchEncerrarComandosDeCobrancaPorEmpresa dadosEncerrarComandos = new TarefaBatchEncerrarComandosDeCobrancaPorEmpresa(processoIniciado.getUsuario(),
																																					funcionalidadeIniciada.getId());
					...
					case Funcionalidade.ENCERRAR_OS_VISITA_COBRANCA:
						...
						TarefaBatchEncerrarOSVisitaCobranca dadosEncerrarOS = new TarefaBatchEncerrarOSVisitaCobranca(	processoIniciado.getUsuario(),
																														funcionalidadeIniciada.getId());
						...
						dadosEncerrarOS.addParametro("colecaoOrdemServico", colecaoOrdemServico);

						/*
						 * dadosEncerrarOS.addParametro(ConstantesSistema.
						 * COLECAO_UNIDADES_PROCESSAMENTO_BATCH,
						 * colecaoUnidadeProcessamento);
						 */

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(dadosEncerrarOS));
					...
					/**
					 * [UC1182] Recepcionar Arquivo TXT Encerramento OS
					 * Cobran�a Mariana Victor 21/06/2011
					 **/
					case Funcionalidade.RECEPCIONAR_ARQUIVO_TXT_ENCERRAMENTO_OS_COBRANCA:
						TarefaBatchProcessarArquivoTxtEncerramentoOSCobranca tarefaBatchProcessarArquivoTxtEncerramentoOSCobranca = new TarefaBatchProcessarArquivoTxtEncerramentoOSCobranca(	processoIniciado.getUsuario(),
																																																funcionalidadeIniciada.getId());
						...
						break;
					/**
					 * [UC1197] Encerrar Comandos de OS Seletiva de Inspe��o
					 * de Anormalidade
					 * 
					 * @author Vivianne Sousa
					 * @created 21/07/2011
					...
					case Funcionalidade.ENCERRAR_COMANDO_OS_SELETIVA_INSPECAO_ANORMALIDADE:
						TarefaBatchEncerrarComandoOSSeletivaInspecaoAnormalidade dadosEncerrarComando = new TarefaBatchEncerrarComandoOSSeletivaInspecaoAnormalidade(	processoIniciado.getUsuario(),
																																										funcionalidadeIniciada.getId());
					...
					case Funcionalidade.CONSULTAR_ORDENS_DE_SERVICO_DO_ARQUIVO_TEXTO:
						TarefaBatchProcessarAtualizacaoDasOrdensServicoDoArquivoTexto dadosProcessarAtualizarOSArqTxt = new TarefaBatchProcessarAtualizacaoDasOrdensServicoDoArquivoTexto(	processoIniciado.getUsuario(),
																																															funcionalidadeIniciada.getId());

						Collection<ProcessarAtualizacaoOSArquivoTextoHelper> colecaoHelper = (Collection<ProcessarAtualizacaoOSArquivoTextoHelper>) parametros.get("colecaoProcessarAtualizacaoOSArquivoTextoHelper");
						Usuario usuarioLogado = (Usuario) parametros.get("usuarioLogado");

						dadosProcessarAtualizarOSArqTxt.addParametro("colecaoProcessarAtualizacaoOSArquivoTextoHelper", colecaoHelper);

						dadosProcessarAtualizarOSArqTxt.addParametro("usuarioLogado", usuarioLogado);

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(dadosProcessarAtualizarOSArqTxt));
					...
					case Funcionalidade.RETIRAR_IMOVEIS_CONTAS_EMPRESAS_COBRANCA:
						TarefaBatchRetirarImoveisContasEmpresaCobranca tarefaBatchRetirarImoveisContasEmpresaCobranca = new TarefaBatchRetirarImoveisContasEmpresaCobranca(	processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());
						...
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(tarefaBatchRetirarImoveisContasEmpresaCobranca));
					 ...
					 * @since 12/11/2013
					 */
					case Funcionalidade.GERAR_ARQUIVO_BANCOS_CARTEIRA_17:

						TarefaBatchGerarArquivoCarteira17 gerarArquivoBancoCarteira17 = new TarefaBatchGerarArquivoCarteira17(	processoIniciado.getUsuario(),
																																funcionalidadeIniciada.getId());

						DebitoCarteiraMovimentoHelper debitoCarteiraMovimentoHelper = (DebitoCarteiraMovimentoHelper) parametros.get("helper");

						Collection colecaoGrupoFaturamento = debitoCarteiraMovimentoHelper.getColecaoGrupoFaturamento();

						gerarArquivoBancoCarteira17.addParametro("colecaoGrupoFaturamento", colecaoGrupoFaturamento);

						gerarArquivoBancoCarteira17.addParametro("anoMesReferencia", debitoCarteiraMovimentoHelper.getAnoMesReferencia());

						gerarArquivoBancoCarteira17.addParametro("idBanco", debitoCarteiraMovimentoHelper.getBanco());

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(gerarArquivoBancoCarteira17));
					...
					case Funcionalidade.REGISTRAR_MOVIMENTO_ARRECADADORES:
						...
						TarefaBatchRegistrarMovimentoArrecadadores registrarMovimentosArrecadadores = new TarefaBatchRegistrarMovimentoArrecadadores(	processoIniciado.getUsuario(),
																																						funcionalidadeIniciada.getId());

						// Par�metros
						StringBuilder stringBuilderTxt = (StringBuilder) parametros.get("stringBuilderTxt");
						Short codigoAgente = (Short) parametros.get("codigoAgente");
						String nomeArrecadador = (String) parametros.get("nomeArrecadador");
						String idTipoMovimento = (String) parametros.get("idTipoMovimento");
						int quantidadeRegistros = (Integer) parametros.get("quantidadeRegistros");
						Integer idArrecadador = (Integer) parametros.get("idArrecadador");
						ArrecadadorContrato arrecadadorContrato = (ArrecadadorContrato) parametros.get("arrecadadorContrato");

						Collection<Integer> colecaoLinhas = Util.retornarListaSequencial(quantidadeRegistros, ConstantesSistema.QUEBRA_MOVIMENTO_ARRECADADORES);

						registrarMovimentosArrecadadores.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoLinhas);
						registrarMovimentosArrecadadores.addParametro("stringBuilderTxt", stringBuilderTxt);
						registrarMovimentosArrecadadores.addParametro("stringBuilderTxt", stringBuilderTxt);
						registrarMovimentosArrecadadores.addParametro("codigoAgente", codigoAgente);
						registrarMovimentosArrecadadores.addParametro("nomeArrecadador", nomeArrecadador);
						registrarMovimentosArrecadadores.addParametro("idTipoMovimento", idTipoMovimento);
						registrarMovimentosArrecadadores.addParametro("quantidadeRegistros", quantidadeRegistros);
						registrarMovimentosArrecadadores.addParametro("idArrecadador", idArrecadador);
						registrarMovimentosArrecadadores.addParametro("arrecadadorContrato", arrecadadorContrato);

						// Seta o objeto para ser serializado no banco, onde
						// depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(registrarMovimentosArrecadadores));
					...
					case Funcionalidade.GERAR_ROTEIRO_DISPOSITIVO_MOVEL:
						...
						ComandoAtualizacaoCadastralDMHelper comandoHelper = (ComandoAtualizacaoCadastralDMHelper) 
						 ...
						 *
						 */
					case Funcionalidade.BATCH_ATUALIZAR_DADOS_CADASTRAIS_IMOVEL_RECADASTRAMENTO:
						
						TarefaBatchAtualizarDadosCadastraisImovelRecadastramento atualizarDadosCadastraisImovelRecadastramento = new TarefaBatchAtualizarDadosCadastraisImovelRecadastramento(
								processoIniciado.getUsuario(), funcionalidadeIniciada.getId());
						
						// Seta o objeto para ser serializado no banco, onde depois sera executado por uma thread
						funcionalidadeIniciada.setTarefaBatch(IoUtil.transformarObjetoParaBytes(atualizarDadosCadastraisImovelRecadastramento));
						getControladorUtil().atualizar(funcionalidadeIniciada);
						break;	
					}
	 ...
	 * @param idProcessoIniciado
	 */
	public void continuarFuncionalidadesIniciadas(String[] idsFuncionalidadesIniciadas, Integer idProcessoIniciado)
																													throws ControladorException {
	...
	public Integer inserirProcessoIniciadoContasCobranca(Collection ids, Integer idEmpresa, Usuario usuario)
																											throws ControladorException {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				TarefaBatchGerarArquivoTextoContasCobrancaEmpresa tarefaBatchGerarArquivoTextoContasCobrancaEmpresa = new TarefaBatchGerarArquivoTextoContasCobrancaEmpresa(processoIniciado.getUsuario(),
																																											funcionalidadeIniciada.getId());
				...
				tarefaBatchGerarArquivoTextoContasCobrancaEmpresa.addParametro("nomeAbreviadoEmpresa", sistemaParametro.getNomeAbreviadoEmpresa());

				if (sistemaParametro.getNomeAbreviadoEmpresa().equalsIgnoreCase(SistemaParametro.EMPRESA_COMPESA)) {
					tarefaBatchGerarArquivoTextoContasCobrancaEmpresa.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, ids);
				} else {
					Collection colecaoUnidadeNegocio = getControladorCobranca().obterUnidadeNegocioEmpresaCobrancaConta(idsRegistros);

					tarefaBatchGerarArquivoTextoContasCobrancaEmpresa.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoUnidadeNegocio);
				}
	...
	public Integer inserirProcessoIniciadoRelatorioPagamentosContasCobranca(
			RelatorioPagamentosContasCobrancaEmpresaHelper helper, int opcaoRelatorio, Usuario usuario)
																										throws ControladorException {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				TarefaBatchGerarRelatorioPagamentosContasCobrancaEmpresa tarefaBatchGerarRelatorioPagamentosContasCobrancaEmpresa = new TarefaBatchGerarRelatorioPagamentosContasCobrancaEmpresa(	processoIniciado.getUsuario(),
																																																	funcionalidadeIniciada.getId());
	 ...
	 * 
	 */
	public Integer inserirAtualizarMovimentacaoHidrometroIdsBatch(MovimentoHidrometroHelper helper)
																									throws ControladorException {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				TarefaBatchGerarMovimentoHidrometro tarefaBatchGerarMovimentoHidrometro = new TarefaBatchGerarMovimentoHidrometro(	processoIniciado.getUsuario(),
																																	funcionalidadeIniciada.getId());

				// Seta os parametros para rodar a funcionalidade
				tarefaBatchGerarMovimentoHidrometro.addParametro("helper", helper);

				tarefaBatchGerarMovimentoHidrometro.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, helper.getColecaoHidrometroSelecionado());
	...
	public Integer inserirProcessoAtualizarConjuntoHidrometro(String fixo, String inicialFixo, String finalFixo,
		...
		try {
			...
			TarefaBatchAtualizarConjuntoHidrometro tarefaBatchAtualizarConjuntoHidrometro = new TarefaBatchAtualizarConjuntoHidrometro(	processoIniciado.getUsuario(),
																																		funcionalidadeIniciada.getId());
	...
	public void iniciarProcessoRelatorioControleAutorizacao(TarefaRelatorio tarefaRelatorio) throws ControladorException {
		try {
			...
			// O usu�rio n�o pode agendar relat�rios caso exista mais de
			// dois
	...
	public void autorizarProcessoIniciado(ProcessoIniciado processoIniciado, Integer processoSituacao,
			Integer funcionalidadeSituacao) throws ControladorException {
		try {
	 ...
	 * 
	 */
	public Integer inserirProcessoIniciadoPagamentosContasCobranca(Integer idEmpresa, Integer referenciaInicial,
			Integer referenciaFinal, Usuario usuario) throws ControladorException {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				TarefaBatchGerarArquivoTextoPagametosContasCobrancaEmpresa tarefaBatchGerarArquivoTextoPagametosContasCobrancaEmpresa = new TarefaBatchGerarArquivoTextoPagametosContasCobrancaEmpresa(	processoIniciado.getUsuario(),
																																																		funcionalidadeIniciada.getId());

				Collection colecaoUnidadeNegocio = getControladorCobranca().obterUnidadeNegocioPagamentosEmpresaCobrancaConta();

				tarefaBatchGerarArquivoTextoPagametosContasCobrancaEmpresa.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoUnidadeNegocio);
	 ...
	 * 
	 */
	public Integer inserirProcessoGerarTxtContasProcessosEspeciais(String anoMes, Integer idCliente, Usuario usuario)
																														throws ControladorException {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				TarefaBatchGerarArquivoTextoContasProjetosEspeciais tarefaBatchGerarArquivoTextoContasProjetosEspeciais = new TarefaBatchGerarArquivoTextoContasProjetosEspeciais(	processoIniciado.getUsuario(),
																																													funcionalidadeIniciada.getId());
	 ...
	 * 
	 */
	public Integer inserirProcessoGerarTxtDeclaracaoQuitacaoDebitos(Integer idGrupoFaturamento, Usuario usuario)
																												throws ControladorException {
		...
		try {
			...
			Integer qtdRotasAtivas = this.pesquisarQuantidadeRotasAtivasFaturamentoGrupo(idGrupoFaturamento);

			if (qtdRotasAtivas != null && qtdRotasAtivas.intValue() == 0) {

				throw new ControladorException(	"atencao.faturamento_grupo_sem_rota_ativa",
												null,
												idGrupoFaturamento.toString());
			}
			...
			while (iterator.hasNext()) {
				...
				case Funcionalidade.GERAR_DECLARACAO_QUITACAO_ANUAL_DEBITOS:
					...
					filtroRota.adicionarParametro(new ParametroSimples(	FiltroRota.FATURAMENTO_GRUPO_ID,
																		idGrupoFaturamento));
					...
					tarefaBatchGerarDadosDeclaracaoQuitacaoAnualDebitos.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoRotasParaExecucao);
				...
				case Funcionalidade.GERAR_ARQUIVO_TEXTO_DECLARACAO_QUITACAO_ANUAL_DEBITOS:
					...
					tarefaBatchGerarArquivoTextoDeclaracaoQuitacaoAnualDebitos.addParametro(ConstantesSistema.COLECAO_UNIDADES_PROCESSAMENTO_BATCH, colecaoEmpresas);
	 ...
	 * @throws ControladorException
	 */
	public Integer inserirProcessoIniciadoParametrosLivresAguardandoAutorizacao(Map parametros, int idProcesso,
			Usuario usuario) throws ControladorException {
		Integer codigoProcessoIniciadoGerado = null;
			...
			while (iterator.hasNext()) {
				...
				try {
					...
					case Funcionalidade.GERAR_CARTA_TARIFA_SOCIAL:
						...
						TarefaBatchGerarCartaTarifaSocial tarefaBatchGerarCartaTarifaSocial = new TarefaBatchGerarCartaTarifaSocial(processoIniciado.getUsuario(),
																																	funcionalidadeIniciada.getId());
					...
					case Funcionalidade.EMISSAO_ORDENS_SELETIVAS:
						...
						// [SB0002]-Gerar TXT
						TarefaBatchGerarTxtOsInspecaoAnormalidade tarefaBatchGerarTxtOsInspecaoAnormalidade = new TarefaBatchGerarTxtOsInspecaoAnormalidade(processoIniciado.getUsuario(),
																																							funcionalidadeIniciada.getId());
	...
	public Integer verificarAutorizacaoBatch(Integer processoId) {
		...
		filtroProcesso.adicionarParametro(new ParametroSimples(	FiltroProcesso.ID,
																processoId));
	...
	/**
	 * Metodo responsavel por inserir um processo batch a partir de uma
	 ...
	 * @param usuario
	 */
	public Integer inserirProcessoBatchCancelarGrupoFaturamento(Usuario usuario, Integer idGrupoFaturamento,
			Integer anoMesReferencia, Integer anoMesReferenciaGrupoMenosUmMes) throws ControladorException {
		...
		try {
			...
			processo.setId(Processo.CANCELAR_GRUPO_FATURAMENTO);
			...
			while (iterator.hasNext()) {
				...
				TarefaBatchCancelarGrupoFaturamento cancelarGrupoFaturamento = new TarefaBatchCancelarGrupoFaturamento(	processoIniciado.getUsuario(),
																														funcionalidadeIniciada.getId());
	...
	/**
	 * Retorna a data/hora em que um processo foi finalizado em um determinado
	 * m�s.
	 * 
	 * @author Hugo Azevedo
	 * @date 17/10/2011
	 ...
	 * @throws ErroRepositorioException
	 */
	public Integer pesquisarQuantidadeRotasAtivasFaturamentoGrupo(Integer idFaturamentoGrupo) throws ControladorException {

		try {

			return repositorioBatch.pesquisarQuantidadeRotasAtivasFaturamentoGrupo(idFaturamentoGrupo);
	...
	/**
	 * verificar se a funcionalidade de gerar dados di�rios de arrecada��o
	 * esta executando
	 * 
	 * @author Rodrigo Cabral
	 * @date 07/12/11
	...
	public void verificarExecucaoFuncionalidade(Integer idFuncionalidade) throws ControladorException {
		...
		FiltroFuncionalidadeIniciada filtroFuncionalidadeIniciada = new FiltroFuncionalidadeIniciada();
		filtroFuncionalidadeIniciada.adicionarParametro(new ParametroSimples(	FiltroFuncionalidadeIniciada.ID,
																				ultimaFuncionalidadeIniciada));
		Collection colecaoFuncionalidadeIniciada = getControladorUtil().pesquisar(filtroFuncionalidadeIniciada, FuncionalidadeIniciada.class.getName());
		...
		/*
		 * Caso a funcionalidade esteja emprocessamento ou em espera envia uma
		 * mensagem ao usu�rio negando o acesso a consulta.
		 */
	...
	public void verificarExecucaoProcesso(Integer idProcesso) throws ControladorException {
		...
		if (ultimoProcesso != null) {
			...
			filtroProcessoIniciado.adicionarParametro(new ParametroSimples(	FiltroProcessoIniciado.ID,
																			ultimoProcesso));
			Collection colecaoProcessoIniciado = getControladorUtil().pesquisar(filtroProcessoIniciado, ProcessoIniciado.class.getName());

			processoIniciado = (ProcessoIniciado) Util.retonarObjetoDeColecao(colecaoProcessoIniciado);
		}

		/*
		 * Caso o processo esteja emprocessamento ou em espera envia uma
		 * mensagem ao usu�rio negando o acesso a consulta.
		 */
		if (processoIniciado != null
				&& processoIniciado.getProcessoSituacao() != null
				&& (processoIniciado.getProcessoSituacao().getId() == ProcessoSituacao.EM_PROCESSAMENTO || processoIniciado.getProcessoSituacao().getId() == ProcessoSituacao.EM_ESPERA)) {
