...
//			BigDecimal debitoTotalAtualizado = new BigDecimal("0.00");
		...
		BigDecimal descontoSobreDebitoTotal = new BigDecimal("0.00");
		BigDecimal descontoDebitoTipo = new BigDecimal("0.00");
		BigDecimal descontoMulta = new BigDecimal("0.00");
		BigDecimal descontoJuros = new BigDecimal("0.00");
		BigDecimal descontoAtualizacaoMonetaria = new BigDecimal("0.00");
		Short entradaEhExtratoDebito = ConstantesSistema.NAO;
		BigDecimal valorDescontoEntrada = new BigDecimal("0.00");
		...
		if ( calculaOpcaoParcelamento == null ){
			...
			if( efetuarParcelamentoDebitosActionForm.get("valorEntradaInformado") != null
				&& !efetuarParcelamentoDebitosActionForm.get("valorEntradaInformado").equals("") 
				&& !marcadaEP ){
				valorEntradaDebitos = Util.formatarMoedaRealparaBigDecimal((String) 
						efetuarParcelamentoDebitosActionForm.get("valorEntradaInformado"));
			}else{
			...
			descontoSobreDebitoTotal = opcoesParcelamento.getValorDescontoSobreDebitoTotal();
			descontoDebitoTipo = opcoesParcelamento.getValorDescontoDebitoTipo();
			descontoMulta = opcoesParcelamento.getValorDescontoMulta();
			descontoJuros = opcoesParcelamento.getValorDescontoJuros();
			descontoAtualizacaoMonetaria = opcoesParcelamento.getValorDescontoAtualizacaoMonetaria();
			entradaEhExtratoDebito = opcoesParcelamento.getEntradaEhExtratoDebito();
			valorDescontoEntrada = opcoesParcelamento.getValorDescontoEntrada();
			
			// Inicia o valor da Entrada com a Miníma caso não tenha marcardo nenhuma conta
			if( marcadaEP ){
				if(entradaEhExtratoDebito != null && entradaEhExtratoDebito.equals(ConstantesSistema.SIM)){
					//Caso a entrada do parcelamento = extrato de débito
			...
			habilitarValorEntrada(sessao,opcoesParcelamento);
...
//			BigDecimal valorEntradaMinima = opcoesParcelamento.getValorEntradaMinima();
			...
			if (valorEntradaInformado != null && valorEntradaInformado.compareTo(
					valorEntradaMinimaPermitida.setScale(Parcelamento.CASAS_DECIMAIS, Parcelamento.TIPO_ARREDONDAMENTO)) == -1) {
				// -----------------------------------------------------------
				...
				if(!temPermissaoValMinimoEntrada){
					throw new ActionServletException("atencao.valor.entrada.deve.ser.maior.igual.minima", null,
							Util.formatarMoedaReal(valorEntradaMinimaPermitida));
				}
			}

			//comentado por Vivianne Sousa - 05/07/2012
			//não utiliza a valorEntradaMinima depois, n precisa setar nada nela
//			if (valorEntradaInformado != null && valorEntradaInformado.compareTo(
...
//			}
		...
		valorTotalDescontos = valorTotalDescontos.add(descontoDebitoTipo);
		...
		efetuarParcelamentoDebitosActionForm.set("descontoSobreDebitoTotal",Util.formatarMoedaReal(descontoSobreDebitoTotal));
		
		efetuarParcelamentoDebitosActionForm.set("descontoDebitoTipo",Util.formatarMoedaReal(descontoDebitoTipo));
		efetuarParcelamentoDebitosActionForm.set("descontoMulta",Util.formatarMoedaReal(descontoMulta));
		efetuarParcelamentoDebitosActionForm.set("descontoJuros",Util.formatarMoedaReal(descontoJuros));
		efetuarParcelamentoDebitosActionForm.set("descontoAtualizacaoMonetaria",Util.formatarMoedaReal(descontoAtualizacaoMonetaria));
		efetuarParcelamentoDebitosActionForm.set("valorDescontoEntrada", Util.formatarMoedaReal(valorDescontoEntrada));
...
//        }
		...
		efetuarParcelamentoDebitosActionForm.set("entradaEhExtratoDebito",entradaEhExtratoDebito.toString());
	...
	private void habilitarValorEntrada(HttpSession sessao,
			NegociacaoOpcoesParcelamentoHelper negociacaoOpcoesParcelamentoHelper){
		
		String habilitaValorEntrada = "1";
		if (negociacaoOpcoesParcelamentoHelper.getEntradaPodeSerGuia() != null 
			&& negociacaoOpcoesParcelamentoHelper.getEntradaPodeSerGuia().equals(ConstantesSistema.NAO)) {
			habilitaValorEntrada = "2";
		} else if (negociacaoOpcoesParcelamentoHelper.getColecaoContasParaParcelamento() != null && !negociacaoOpcoesParcelamentoHelper.getColecaoContasParaParcelamento().isEmpty()) {
			Iterator contaValores = negociacaoOpcoesParcelamentoHelper.getColecaoContasParaParcelamento().iterator();
		...
		sessao.setAttribute("habilitaValorEntrada", habilitaValorEntrada);
	}
}