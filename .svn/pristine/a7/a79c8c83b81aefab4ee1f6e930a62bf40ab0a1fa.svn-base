...
public class RepositorioGerencialHBM implements IRepositorioGerencial {
	...
	public BigDecimal pesquisarArquivoPlanejamentoResumoArrecadacao(int anoMes,Integer idGerencia,
		...
		try{
			...
			String sql = "SELECT " 
				...
				+ ") AS valorArrecadacao "
				+ "FROM arrecadacao.un_resumo_arrecadacao resumo " 
				+ "INNER JOIN cadastro.g_gerencia_regional gerencia on (gerencia.greg_id = resumo.greg_id and gerencia.greg_id = :idGerencia) "
				+ "INNER JOIN cadastro.g_unidade_negocio unidade ON ( unidade.uneg_id = resumo.uneg_id and  unidade.uneg_id = :idUnidade)  "
				+ "INNER JOIN cadastro.g_localidade loc on (resumo.loca_id = loc.loca_id and loc.loca_id = :idlocalidade and loc.loca_cdcentrocusto = :centroCusto) "
				+ "WHERE resumo.rear_amreferencia = :anoMes "
				+ "GROUP BY gerencia.greg_id,unidade.uneg_id,loc.loca_id,loc.loca_cdcentrocusto "
				+ "ORDER BY gerencia.greg_id,unidade.uneg_id,loc.loca_id,loc.loca_cdcentrocusto ";
				...
				retorno = (BigDecimal) session.createSQLQuery(sql).
					...
					setString("centroCusto",codigoCentroCusto).
					setMaxResults(1).
					uniqueResult();
	 ...
	 * @param String codigoCentroCusto
	 * 
	 * @return Object[1] valorContasAReceber
	 * 
	 * @throws ErroRepositorioException
	 */
	public BigDecimal pesquisarArquivoPlanejamentoResumoPendencia(int anoMes,Integer idGerencia,
		...
		try{
			...
			String sql = "SELECT " 
				...
				+ ") AS valorContasAReceber "
				+ "FROM cobranca.un_resumo_pendencia resumo " 
				+ "INNER JOIN cadastro.g_gerencia_regional gerencia on (gerencia.greg_id = resumo.greg_id and gerencia.greg_id = :idGerencia) "
				+ "INNER JOIN cadastro.g_unidade_negocio unidade ON ( unidade.uneg_id = resumo.uneg_id and  unidade.uneg_id = :idUnidade)  "
				+ "INNER JOIN cadastro.g_localidade loc on (resumo.loca_id = loc.loca_id and loc.loca_id = :idlocalidade and loc.loca_cdcentrocusto = :centroCusto) "
				+ "WHERE resumo.rpen_amreferencia = :anoMes "
				+ "AND resumo.rpen_icvencido = 1 "
				+ "GROUP BY gerencia.greg_id,unidade.uneg_id,loc.loca_id,loc.loca_cdcentrocusto "
				+ "ORDER BY gerencia.greg_id,unidade.uneg_id,loc.loca_id,loc.loca_cdcentrocusto ";

				//Faz a pesquisa
				retorno = (BigDecimal) session.createSQLQuery(sql).
					addScalar("valorContasAReceber", Hibernate.BIG_DECIMAL).
					setInteger("anoMes",anoMes).
					setInteger("idGerencia",idGerencia).
					setInteger("idUnidade",idUnidade).
					setInteger("idlocalidade",idLocalidade).
...
//				 INDICE 89 - Quantidade Economias, Grupo Subcategoria 101				    
	...
	throws ErroRepositorioException {
		...
		try{
			...
			String consulta = "SELECT  localidade.codigoCentroCusto "
				+ "FROM GLocalidade as localidade " 
				+ "GROUP BY localidade.codigoCentroCusto ORDER BY localidade.codigoCentroCusto ";
	...
	public Collection<Object[]> pesquisarArquivoPlanejamentoResumoLigacaoEconomia(int anoMes) 
		...
		try{
			String sql = "SELECT  "
				...
				+ "gerencia.greg_nmregional as nomeGerencia, "
				+ "unidade.uneg_id as idUnidade, "
				+ "unidade.uneg_nmunidadenegocio as nomeUnidade, "
				+ "loca.loca_cdcentrocusto as codigoCentroCusto, "
				+ "loca.loca_id as idLocalidade, "
				+ "loca.loca_nmlocalidade as nomeLocalidade, "
				+ "SUM(CASE WHEN resumo.catg_id = 1 and sub.scat_icrural = 2 and las.last_icativaagua = 1 THEN rele_qteconomias ELSE 0 END) AS totalEconomiasResidenciaisAtivasAgua, " 
				+ "SUM(CASE WHEN resumo.catg_id = 1 and sub.scat_icrural = 2 and les.lest_iccadastradaesgoto = 1 THEN rele_qteconomias ELSE 0 END) AS totalEconomiasResidenciasCadastradasEsgoto, " 
				+ "SUM(CASE WHEN resumo.catg_id = 1 and sub.scat_icrural = 1 THEN rele_qteconomias ELSE 0 END) AS totalEconomiasResidenciasRurais " 
				+ "FROM cadastro.un_res_lig_econ resumo "
				+ "INNER JOIN atendimentopublico.g_ligacao_agua_situacao las ON ( resumo.last_id = las.last_id ) " 
				+ "INNER JOIN atendimentopublico.g_lig_esgoto_sit les ON ( resumo.lest_id = les.lest_id ) " 
				+ "INNER JOIN cadastro.g_subcategoria sub ON ( resumo.scat_id = sub.scat_id ) " 
				+ "INNER JOIN cadastro.g_localidade loca ON ( loca.loca_id = resumo.loca_id ) "
				+ "INNER JOIN cadastro.g_gerencia_regional gerencia ON ( gerencia.greg_id = resumo.greg_id ) "
				+ "INNER JOIN cadastro.g_unidade_negocio unidade ON ( unidade.uneg_id = resumo.uneg_id ) "
				+ "WHERE resumo.rele_amreferencia = :anoMes " 
				+ "GROUP BY gerencia.greg_id,gerencia.greg_nmregional, " 
				+ "unidade.uneg_id,unidade.uneg_nmunidadenegocio,loca.loca_cdcentrocusto,loca.loca_id,loca.loca_nmlocalidade "
				+ "ORDER BY gerencia.greg_id,unidade.uneg_id,loca.loca_cdcentrocusto,loca.loca_id ";
			...
			retorno = (Collection<Object[]>) session.createSQLQuery(sql).
				...
				addScalar("totalEconomiasResidenciaisAtivasAgua", Hibernate.INTEGER).
				addScalar("totalEconomiasResidenciasCadastradasEsgoto", Hibernate.INTEGER).
				addScalar("totalEconomiasResidenciasRurais", Hibernate.INTEGER).
				setInteger("anoMes",anoMes).
				list();
	...
	public Object[] pesquisarArquivoPlanejamentoResumoFaturamento(int anoMes,Integer idGerencia,
		...
		try{
			...
			String sql = "SELECT " 
				+ "SUM(CASE WHEN (resumo.catg_id = 1 AND resumo.rfat_nnsequenciatipolancamento = 1110) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) AS faturamentoAguaResidencial, " // 0
				+ "SUM(CASE WHEN (resumo.catg_id = 2 AND resumo.rfat_nnsequenciatipolancamento = 1110) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) AS faturamentoAguaComercial, " // 1
				+ "SUM(CASE WHEN (resumo.catg_id = 3 AND resumo.rfat_nnsequenciatipolancamento = 1110) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) AS faturamentoAguaIndustrial, " // 2
				+ "SUM(CASE WHEN (resumo.catg_id = 4 AND resumo.rfat_nnsequenciatipolancamento = 1110) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) AS faturamentoAguaPublico, " // 3
				+ "SUM(CASE WHEN (resumo.catg_id in (1,2,3,4) AND resumo.rfat_nnsequenciatipolancamento = 1110) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) AS aguaDireta, " // 4					
				+ "SUM(CASE WHEN (resumo.catg_id in (1,2,3,4) AND resumo.rfat_nnsequenciatipolancamento = 1130) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) AS aguaIndireta, " // 5
				+ "SUM(CASE WHEN (resumo.catg_id = 2 AND resumo.rfat_nnsequenciatipolancamento = 1120) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) AS faturamentoEsgotoComercial, " // 6	
				+ "SUM(CASE WHEN (resumo.catg_id = 3 AND resumo.rfat_nnsequenciatipolancamento = 1120) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) AS faturamentoEsgotoIndustrial, " // 7
				+ "SUM(CASE WHEN (resumo.catg_id = 4 AND resumo.rfat_nnsequenciatipolancamento = 1120) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) AS faturamentoEsgotoPublico, " // 8
				+ "SUM(CASE WHEN (resumo.catg_id in (1,2,3,4) AND resumo.rfat_nnsequenciatipolancamento = 1120) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) AS esgotoDireto, " // 9
				+ "SUM(CASE WHEN (resumo.lctp_id in (3,4,5,11) and resumo.lict_id in (7,8,9,11) ) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) + "
				+ "SUM(CASE WHEN (resumo.lctp_id in (15,16) and resumo.lcit_id =2 ) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) + "
				+ "SUM(CASE WHEN (resumo.lctp_id in (15,16) and resumo.lict_id in (7,8,9,11) ) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) - "
				+ "SUM(CASE WHEN (resumo.lctp_id in (7,8,9) and resumo.lict_id in (7,8,9,11) ) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) - "
				+ "SUM(CASE WHEN (resumo.lctp_id in (18,19) and resumo.lcit_id =2 ) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) - "
				+ "SUM(CASE WHEN (resumo.lctp_id in (18,19) and resumo.lict_id in (7,8,9,11) ) THEN resumo.rfat_vlitemfaturamento ELSE 0 END) "
				+ "as esgotoIndireto "				
				+ "FROM financeiro.resumo_faturamento resumo " 
				+ "INNER JOIN cadastro.gerencia_regional gerencia on (gerencia.greg_id = resumo.greg_id and gerencia.greg_id = :idGerencia) "
				+ "INNER JOIN cadastro.unidade_negocio unidade ON ( unidade.uneg_id = resumo.uneg_id and  unidade.uneg_id = :idUnidade)  "
				+ "INNER JOIN cadastro.localidade loc on (resumo.loca_id = loc.loca_id and loc.loca_id = :idlocalidade and loc.loca_cdcentrocusto = :centroCusto) "
				+ "WHERE resumo.rfat_amreferencia = :anoMes "
				+ "GROUP BY gerencia.greg_id,unidade.uneg_id,loc.loca_id,loc.loca_cdcentrocusto "
				+ "ORDER BY gerencia.greg_id,unidade.uneg_id,loc.loca_id,loc.loca_cdcentrocusto ";

				//Faz a pesquisa
				retorno = (Object[]) session.createSQLQuery(sql).
					addScalar("faturamentoAguaResidencial", Hibernate.BIG_DECIMAL).
					addScalar("faturamentoAguaComercial", Hibernate.BIG_DECIMAL).
					addScalar("faturamentoAguaIndustrial", Hibernate.BIG_DECIMAL).
					addScalar("faturamentoAguaPublico", Hibernate.BIG_DECIMAL).
					addScalar("aguaDireta", Hibernate.BIG_DECIMAL).
					addScalar("aguaIndireta", Hibernate.BIG_DECIMAL).
					addScalar("faturamentoEsgotoComercial", Hibernate.BIG_DECIMAL).
					addScalar("faturamentoEsgotoIndustrial", Hibernate.BIG_DECIMAL).
					addScalar("faturamentoEsgotoPublico", Hibernate.BIG_DECIMAL).
					addScalar("esgotoDireto", Hibernate.BIG_DECIMAL).
					addScalar("esgotoIndireto", Hibernate.BIG_DECIMAL).
					setInteger("anoMes",anoMes).
					setInteger("idGerencia",idGerencia).
					setInteger("idUnidade",idUnidade).
					setInteger("idlocalidade",idLocalidade).
	...
	public Object[] pesquisarRelatorioOrcamentoSINPTotalVolumeFaturadoEsgoto(
		...
		try{
			...
			String sql = "SELECT " 
				+ "SUM(CASE WHEN rfat.catg_id = 1 THEN rfat.rece_vofaturado ELSE 0 END) AS esgotototalvolumefaturadoresid,"
				+ "SUM(CASE WHEN rfat.catg_id = 2 THEN rfat.rece_vofaturado ELSE 0 END) AS esgotototalvolumefaturadocomer,"
				+ "SUM(CASE WHEN rfat.catg_id = 3 THEN rfat.rece_vofaturado ELSE 0 END) AS esgotototalvolumefaturadoindus,"
				+ "SUM(CASE WHEN rfat.catg_id = 4 THEN rfat.rece_vofaturado ELSE 0 END) AS esgotototalvolumefaturadopubli,"
	 ...
	 * 
	 *  Object[0] - aguaTotalVolumeFaturadoMedido
	 *  Object[1] - aguaTotalVolumeFaturadoEstimado
	 *  Object[2] - aguaTotalVolumeFaturadoResidencial
	 *  Object[3] - aguaTotalVolumeFaturadoComercial
	 *  Object[4] - aguaTotalVolumeFaturadoIndustrial
	 *  Object[5] - aguaTotalVolumeFaturadoPublico
	 *  Object[6] - aguaTotalVolumeFaturadoRural
	 *  Object[7] - aguaTotalVolumeFaturadoGeral
	 *   
	 * @throws ErroRepositorioException
	 */
	public Object[] pesquisarRelatorioOrcamentoSINPTotalVolumeFaturadoAgua(
		...
		try{
			...
			String sql = "SELECT " 
				+ "SUM(CASE WHEN rfat.reca_ichidrometro = 1 THEN rfat.reca_voconsumofaturado ELSE 0 END) AS aguaTotalVolumeFaturadoMedido,"
				+ "SUM(CASE WHEN rfat.reca_ichidrometro = 2 THEN rfat.reca_voconsumofaturado ELSE 0 END) AS aguatotalvolumefaturadoestimad,"
				+ "SUM(CASE WHEN rfat.catg_id = 1 AND scat_id not in (13,62,63,64,70,71,77,78) THEN rfat.reca_voconsumofaturado ELSE 0 END) AS aguatotalvolumefaturadoresiden,"
				+ "SUM(CASE WHEN rfat.catg_id = 2 THEN rfat.reca_voconsumofaturado ELSE 0 END) AS aguatotalvolumefaturadocomerci,"
				+ "SUM(CASE WHEN rfat.catg_id = 3 THEN rfat.reca_voconsumofaturado ELSE 0 END) AS aguatotalvolumefaturadoindustr,"
				+ "SUM(CASE WHEN rfat.catg_id = 4 THEN rfat.reca_voconsumofaturado ELSE 0 END) AS aguaTotalVolumeFaturadoPublico,"
				+ "SUM(CASE WHEN rfat.catg_id = 1 AND scat_id in (13,62,63,64,70,71,77,78) THEN rfat.reca_voconsumofaturado ELSE 0 END) AS aguaTotalVolumeFaturadoRural,"
	 ...
	 * @throws ErroRepositorioException
	 */
	public void gerarResumoPendenciaBD() throws ErroRepositorioException, SQLException {
		...
		try {
			...
			sql = " select cobranca.sp_resumo_pendencia(null) ";
