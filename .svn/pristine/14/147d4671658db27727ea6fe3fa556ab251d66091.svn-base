...
public class InformarParametrosSistemaAction extends GcomAction {
	...
	private void montarSistemaParametro1Aba(
		...
		// Numero Telefone Atendimento de Serviços de esgoto
		...
		if (validaCampo(form.getIdClienteResponsavelProgramaEspecial())) {
			if(Util.validarCaracteresEspeciais(form.getIdClienteResponsavelProgramaEspecial())){
				...
				throw new ActionServletException("atencao.campo_texto.caracter_invalido", "Cliente Responsável Programa Especial");
			}
		}
		...
		if (validaCampo(form.getPercentualConvergenciaRepavimentacao())) {
			...
			if(!Util.validarValorNaoNumericoComoBigDecimal(form.getPercentualConvergenciaRepavimentacao())){
				...
				throw new ActionServletException("atencao.campo_texto.caracter_invalido", "Percentual de Convergência de Repavimentação");
			}
		}
		...
		if (validaCampo(form.getValorExtratoFichaComp())) {
			...
			if(!Util.validarValorNaoNumericoComoBigDecimal(valorAux)){
				...
				throw new ActionServletException("atencao.campo_texto.caracter_invalido", "Valor Extrato Ficha Compensação");
			}
		}else{
		...
		if (validaCampo(form.getNumeroDiasBloqueioCelular())) {
			if(Util.validarCaracteresEspeciais(form.getNumeroDiasBloqueioCelular())){
				...
				throw new ActionServletException("atencao.campo_texto.caracter_invalido", "Número de Dias Bloqueio Celular");
			}
		}

		// Valor para Emissão de Guia de Pagamento no Formato Ficha de Compensação
		if (form.getValorGuiaFichaComp() != null && !form.getValorGuiaFichaComp().equals("")) {
			...
			if(!Util.validarValorNaoNumericoComoBigDecimal(valorAux)){
				...
				throw new ActionServletException("atencao.campo_texto.caracter_invalido", "Valor Guia Ficha Compensação");
			}
		} 

		// Valor para Emissão de Demonstrativo de Parcelamento no Formato Ficha de Compensação
		if (form.getValorDemonstrativoParcelamentoFichaComp() != null
			...
			if(!Util.validarValorNaoNumericoComoBigDecimal(valorAux)){
				...
				throw new ActionServletException("atencao.campo_texto.caracter_invalido", 
					"Valor Demonstrativo de Parcelamento Ficha de Compensação");
			}
		} 
		...
		//Quantidade de dias para guardar ocorrências de paralisação
		 ...
		 * @date 02/07/2014
		 * 
		 * RM 11444 - [UC0060] - Informar Parametros Sistema e [UC0061] - Consutar Parâmetros Sistema
		 */
	...
	private void montarSistemaParametro3Aba(
		...
		if (validaCampo(form.getPercentualEntradaMinima())) {		
			...
			if(form.getPercentualEntradaMinima().matches(".*[a-zA-Z].*")){
				throw new ActionServletException("atencao.campo.invalido", null, 
						"Percentual de Entrada Mínima para Financiamento");
			}
		...
		if (validaCampo(form.getMaximoParcelas())) {
			...
			if(!Util.validarStringNumerica(form.getMaximoParcelas().toString())){
				throw new ActionServletException("atencao.campo.invalido", null, 
						"Máximo de Parcelas para um Financiamento");			
			}
		...
		if (validaCampo(form.getPercentualMaximoAbatimento())) {
			...
			if(form.getPercentualMaximoAbatimento().matches(".*[a-zA-Z].*")){
				throw new ActionServletException("atencao.campo.invalido", null, 
						"Percentual Máximo para Abatimento de um Serviço");
			}
		...
		if (validaCampo(form.getNumeroMaximoParcelaCredito())) {
			...
			if(!Util.validarStringNumerica(form.getNumeroMaximoParcelaCredito().toString())){
				throw new ActionServletException("atencao.campo.invalido", null, 
						"Número Máximo para Parcela de Crédito");
			}
		...
		if (validaCampo(form.getPercentualCalculoIndice())) {
			...
			if(form.getPercentualCalculoIndice().matches(".*[a-zA-Z].*")){
				throw new ActionServletException("atencao.campo.invalido", null, 
						"Percentual da Média do Índice para Cálculo do Parcelamento");
			}				
		...
		if (validaCampo(form.getNumeroMesesPesquisaImoveisRamaisSuprimidos())) {
			...
			if(!Util.validarStringNumerica(form.getNumeroMesesPesquisaImoveisRamaisSuprimidos().toString())){
				throw new ActionServletException("atencao.campo.invalido", null, 
					"Número meses para pesquisa de imóveis com ramais suprimidos");
			}
		...
		if (validaCampo(form.getNumeroAnoQuitacao())) {
			...
			if(!Util.validarStringNumerica(form.getNumeroAnoQuitacao().toString())){
				throw new ActionServletException("atencao.campo.invalido", null, 
					"Número de anos para geração da declaração de quitação de débitos");
			}
		...
		if (validaCampo(form.getNumeroMesesAnterioresParaDeclaracaoQuitacao())) {
			...
			if(!Util.validarStringNumerica(
				...
				throw new ActionServletException("atencao.campo.invalido", null, 
					"Quantidade de meses anteriores geração declaração de quitação de débitos");
			}
		 ...
		 * Data: 04/07/2012   
		 * [UC0060] Informar Parâmetros do Sistema
		 */
...
//			}
	...
	private void montarSistemaParametro4Aba(
		 ...
		 * Inclusão de campo para informar 
		 * número de dias úteis para Vencimento das Contas da Entrada de Parcelamento
		 */
		 ...
		 * Inclusão de campo para informar 
		 * número de dias úteis para Cancelamento da Entrada de Parcelamento
		 */
		 ...
		 * Incluisão indicador de forma a verificar se a remoção da restrição do cliente 
		 * no SPC/SERASA só seja excluída após o pagamento da entrada do parcelamento
		 */
		 ...
		 * Autor: Diogo Luiz
		 * Data: 23/09/2014
		 * 1.2.2.38. Indicador de Permissão de Alteração de Dados ou Vínculo de Cliente Negativado 
		 */
	...
	private void montarSistemaParametro5Aba(
		...
		// Situação de Água na Exclusão de Imóvel
		...
		//Situação de Esgoto na Exclusão de Imóvel
		...
		// RM5759 Indicador Exigir RA no cancelamento do débito
		...
		// Descrição do Regulamento para Loja Virtual
